...
import { getExamQuestions, getPartQuestions, isCpeReadingWritingExam, type ReadingExam } from '../examData'
...
import CpeRwPartContent from './CpeRwPartContent'
import '../ketRw/readingKetRw.css'
import '../caeRw/readingCaeRw.css'
import './readingCpeRw.css'

const STORAGE_PREFIX = 'exam-reading-draft:'

export default function ReadingCpeRwTest() {
  ...
  if (!exam || !isCpeReadingWritingExam(exam)) {
    return (
      <div className="flex h-full items-center justify-center">
        <p style={{ color: 'var(--text-muted)' }}>Không tìm thấy đề CPE C2.</p>
      </div>
    )
  }
  ...
  return (
    <div className={`ket-rw-shell cpe-rw-shell${reviewMode ? ' is-review' : ''}`} style={fontStyle}>
      {reviewMode && (
      ...
      {confirmSubmit && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-labelledby="cpe-rw-submit-title"
            onClick={e => e.stopPropagation()}
          >
            <h3 id="cpe-rw-submit-title" className="ket-rw-modal__title">
