/**
 * Build offline dictionary Part 4: +6000 more EN→VI entries from GitHub.
 * Skips words already in Part 1 + Part 2 + Part 3.
 *
 * Source: samuraitruong/open-vn-en-dict (MIT)
 *   https://github.com/samuraitruong/open-vn-en-dict
 *
 * Output: apps/web/src/features/dictionary/data/offlinePart4.json
 *
 * Run: node scripts/build-dict-part4.mjs
 */
...
const outPath = path.join(root, 'apps/web/src/features/dictionary/data/offlinePart4.json')
...
async function main() {
  ...
  const existing = new Set(
    [...(part1.cards || []), ...(part2.cards || []), ...(part3.cards || [])]
      .map(c => String(c.phrase || '').toLowerCase().trim())
      .filter(Boolean),
  )
  console.log('already have', existing.size, 'words (part1+2+3)')
