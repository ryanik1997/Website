/**
 * Đóng gói Tainguyen/<bundle>/ → ZIP để Import thủ công Reading.
 * Run: node scripts/pack-reading-bundle.mjs cae-Reading-test1
 */
...
async function main() {
  const bundleDir = process.argv[2] ?? 'ket-reading-test1'
  ...
  console.log('  → Luyện thi → Import thủ công Reading → chọn ZIP này')
}
