/**
 * Build offline dictionary Part 2: +6000 EN→VI entries from GitHub.
 *
 * Source: samuraitruong/open-vn-en-dict (MIT)
 *   https://github.com/samuraitruong/open-vn-en-dict
 * Word priority: google-10000-english (first20hours) then remaining goodWords.
 *
 * Output: apps/web/src/features/dictionary/data/offlinePart2.json
 *
 * Run: node scripts/build-dict-part2.mjs
 */
...
const outPath = path.join(root, 'apps/web/src/features/dictionary/data/offlinePart2.json')
...
async function main() {
  ...
  const existing = new Set(
    (part1.cards || []).map(c => String(c.phrase || '').toLowerCase().trim()).filter(Boolean),
  )

  // candidates: frequency first, then alphabetical goodWords
  const candidates = []
  const seen = new Set()
  ...
  // prefer medium length content words
  rest.sort((a, b) => a.length - b.length || a.localeCompare(b))
  for (const w of rest) {
    if (candidates.length >= TARGET * 4) break
    candidates.push(w)
  }
