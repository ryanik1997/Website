...
→ O2 ${res.finalO2.toFixed(2)}% · CO2 ${res.finalCO2.toFixed(2)}% · RH ${res.finalRH.toFixed(0)}% · t90 ${res.t90 ? res.t90.toFixed(1) + 'h' : '>72h'}`;
  ...
  function m2Scan() {
    ...
    setTimeout(() => {
      const results = E.findAllConfigs(p, db2);
      ...
      $('m2Body').innerHTML = rows;
      $('m2ScanSummary').textContent = results.length
        ? `Tìm được ${results.length} cấu hình đạt mục tiêu (quét trên ${Object.keys(db2.mang_OPP).length} màng × 5 D × 0-400 lỗ).`
        : 'Không cấu hình nào đạt cả O2 lẫn CO2 — thử film khác, khối lượng thấp hơn, hoặc nhiệt độ thấp hơn.';
