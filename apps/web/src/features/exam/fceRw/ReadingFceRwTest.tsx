...
import { getExamQuestions, getPartQuestions, isFceReadingWritingExam, type ReadingExam } from '../examData'
...
import FceRwPartContent from './FceRwPartContent'
import '../ketRw/readingKetRw.css'
import './readingFceRw.css'

const STORAGE_PREFIX = 'exam-reading-draft:'

export default function ReadingFceRwTest() {
  ...
  const examDurationMinutes = exam ? readingExamDurationMinutes(exam) : 75
  ...
  if (!exam || !isFceReadingWritingExam(exam)) {
    return (
      <div className="flex h-full items-center justify-center">
        <p style={{ color: 'var(--text-muted)' }}>Không tìm thấy đề FCE B2.</p>
      </div>
    )
  }
  ...
  return (
    <div className={`ket-rw-shell fce-rw-shell${reviewMode ? ' is-review' : ''}`} style={fontStyle}>
      {reviewMode && (
      ...
      {confirmSubmit && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-labelledby="fce-rw-submit-title"
            onClick={e => e.stopPropagation()}
          >
            <h3 id="fce-rw-submit-title" className="ket-rw-modal__title">
            ...
            <p className="ket-rw-modal__text">
              ...
              {answeredCount === 0
                ...
                : ' Reading chấm tự động; Writing (Part 8–9) chấm bằng AI sau khi nộp.'}
            </p>
