    ...
    <div className="ss-shell ss-list-page ss-history-page">
      ...
      <main className="ss-history-page__main">
        ...
        {history.length === 0 ? <div className="ss-hub-empty">Chưa có cấu trúc nào được hoàn thành.</div> : (
          <div className="ss-history-page__list">{history.map((entry, index) => <article className="ss-history-page__entry" key={`${entry.completedAt}-${index}`}><span className="ss-history-page__check"><Check size={15} /></span><div><h2>{entry.title}</h2><p>{entry.source === 'ai' ? 'AI chấm đạt' : 'Khớp đúng mẫu'}</p></div><time>{new Date(entry.completedAt).toLocaleString('vi-VN')}</time></article>)}</div>
        )}
      </main>
    </div>
  )
