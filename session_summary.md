...
- Integration branch `integration/consolidate-20260728-190243` now contains the FCE repair, KET crawl/layout, and manually resolved `feat/fix-reading-layout` work. Its 16 original conflict stages remain saved under the backup directory.
...
- Remaining work: run the full Reading/Writing/DB/FCE validation suite before updating `main`; no branch has been deleted.
...
### FCE B2 Reading corpus repair 28626 �� Commit 2 deterministic parser/recrawl ready; overall task NOT COMPLETE

- Branch: `fce-b2-corpus-repair-28626`; Commit 1: `ffec9f5 test(fce): audit corpus and lock regressions`.
- Deterministic source recovery now converts Source Tests 1-26 to App Tests 2-27 with exactly 26 tests, 182 parts, and 1,352 questions; strict conversion failures: 0.
- Final package audit: `SOURCE_OK=182`, `PARSER_BROKEN=0`, `RECRAWL_REQUIRED=0`, `AI_REPAIR_VERIFIED=0`.
- Audit-driven recrawl recovered 27 genuinely incomplete pages across 20 source tests. Final crawler writes full HTML plus normalized `pages/part-M.full.json` metadata for refreshed pages.
- Parser fixes cover raw inline controls, Q14 split alternatives, table Part 3, semantic Part 4, Part 5 pages without `<h2>`, unique lowercase Part 6 IDs, and dynamic A-D/A-E Part 7 sections.
- Package-only semantic validation PASS (26 exams); package-only answer validation PASS (182 part records); targeted regressions PASS.
- No `bootstrap-v1`, `bootstrap-*`, or `ai-generated` content appears in regenerated package files.
- Runtime files are intentionally not rebuilt yet: Commit 3 must first quarantine 172 bootstrap repair caches and make repair merging verified-only/fail-closed. Full package+runtime validators, build, typecheck, tests, Playwright, screenshots, and final status report remain Commit 4 work.

### Next session start prompt

Continue task3_28626 with Commit 3 only after Commit 2 is committed. First harden `scripts/reading/merge-fce-b2-ai-repairs.mjs`, quarantine all 172 bootstrap caches, add `--only-unrecoverable`, and prove zero AI repairs are required because all 182 source parts are recoverable. Do not run the production catalog build until bootstrap caches cannot merge.
...
- Upload file `part1.mp3` (4.19 MB) t�? `D:\App-English-Ryan\Crawl\Import_KET_A2_Listening\test-16\` lên Supabase storage `exam-media/catalog/listening-publish/listening-import-ket-a2-practice-16/part1-audio.mp3` �? upsert thay th�? file cũ.
...
- Root cause còn sót sau bản vá hydrate trước: `Object.assign` thay toàn b�? `parts`, trong khi các màn Listening/Reading đã memo hóa danh sách/câu hỏi theo tham chiếu DTO cũ; báo cáo có answer key nhưng paper vẫn đọc object câu hỏi cũ có `answer` rỗng.
- `promoteHydratedExamForReview` nay đồng b�? sâu tại ch�?, gi�? nguyên tham chiếu object/array hiện hữu đ�? mọi memoized question nhận answer key trước khi bật review mode.
- Regression test xác nhận c�? root exam lẫn các tham chiếu `parts`, `questions`, `question` đã capture đều thấy đáp án `B`. Test 1/1, TypeScript và `git diff --check` PASS.

### Session 2026-07-19 �? Fix Q9/Q10 KET Book 3 Test 1 nhập `1` vẫn đúng

- D�? liệu gốc đúng: Q9 = `8.15`/`eight fifteen`, Q10 = `10.50`; ID hai câu không trùng.
- Root cause: matcher gap-fill cuối cùng dùng substring hai chiều, nên `8.15`.includes(`1`) và `10.50`.includes(`1`) đều tr�? true.
- Tách matcher thuần đ�? test không kéo catalog/DB; đáp án chứa ch�? s�? nay ch�? khớp chính xác hoặc s�? nguyên tương đương (`8` = `08`), không chấp nhận ch�? s�? con trong gi�?/giá.
- Feedback loop trước fix RED đúng triệu chứng; sau fix scoped tests 3/3, TypeScript và `git diff --check` PASS.

### Session 2026-07-19 �? Audit đồng b�? matcher toàn b�? Luyện thi

- Audit các đường chấm t�? động xác nhận Listening và Reading gap-fill/sentence-completion đều từng dùng substring hai chiều; không ch�? s�? `1`, fragment ch�? như `a` cũng có th�? khớp sai với `radio`. Multiple-choice, matching và TID Reading dùng exact comparison nên không mắc lỗi này.
- Loại b�? hoàn toàn fuzzy substring �? matcher Listening/Reading. Ch�? exact normalized answer, answer variants/`acceptableAnswers`, và s�? nguyên tương đương (`8` = `08`) được chấp nhận.
- Tách pure matcher cho c�? hai skill đ�? regression test không kéo catalog/DB. Feedback loop trước fix RED �? c�? Listening và Reading; sau fix exam scoped tests 14/14, TypeScript, scan không còn pattern substring và `git diff --check` PASS.

### Session 2026-07-19 �? Đồng b�? khung ảnh PET B1 Part 1 với KET A2
...
- Composite image, ba ảnh rời, contain frame, radio, footer audio/unsure và responsive vì vậy có cùng layout; các Part PET khác gi�? nguyên.
...
- Root cause lỗi `sync_server_time ... schema cache`: frontend hardening đã gọi RPC t�? migration 031, nhưng migrations 031�?034 vẫn đang �? staging/chưa có trên database hiện tại.
- Thêm `getSyncServerTime`: ch�? fallback sang timestamp local khi PostgREST báo đúng lỗi RPC/schema thiếu; lỗi mạng/quyền khác vẫn throw. Fallback được đánh dấu non-authoritative nên sync chính, exam progress và check-in không ghi tiến cloud cursor, tránh clock client làm b�? sót d�? liệu.
- Main sync cũng coi bảng `sync_tombstones` chưa tồn tại là danh sách rỗng đ�? database pre-031 không làm chết toàn b�? sync; delete ledger đầy đ�? s�? t�? hoạt động sau khi migration được áp.
...
- Feedback loop production xác nhận relay URL tr�? HTTP 429 + `X-Vercel-Mitigated: challenge`/Security Checkpoint; Vercel Function logs trống, nên request chưa tới `youtube-captions-relay` và thông báo hiện tại quy lỗi cho YouTube sai tầng.
- Supabase remote có c�? `VERCEL_AUTOMATION_BYPASS_SECRET` và `YOUTUBE_CAPTIONS_RELAY_SECRET`; Vercel project có relay secret production. Firewall có `Challenge Scrapers` và `Block AI Bots`; browser-like UA vẫn b�? system challenge.
- Project Hobby không h�? tr�? IP/System Bypass (`vercel firewall overview` tr�? unavailable). Không t�? pause system mitigations vì đây là thay đổi bảo mật rộng 24 gi�?.
- Hướng x�? lý cần quyền/chọn lựa: rotate + đồng b�? lại Automation Bypass secret Vercel→Supabase rồi smoke, hoặc tách relay sang endpoint/project riêng không bật checkpoint nhưng vẫn gi�? `x-relay-secret`.

### Session 2026-07-18 �? Hardening local-first sync cho tải 1000 user

- Chuyển sync chính, exam progress và check-in sang pull incremental có watermark server, phân trang 500 bản ghi, th�? t�? ph�? ổn định và tombstone truyền xóa; cursor cloud/local ch�? tiến sau khi push thành công. Cursor local tách khỏi thời gian server đ�? thiết b�? lệch đồng h�? vẫn phát hiện thay đổi offline.
- Batch upsert, timestamp LWW do Postgres ghi, trigger `updated_at`, retry full-jitter, login/reconnect jitter, debounce 4 giây và chu k�? sync 5�?6 phút; dọn timer retry/reconnect đ�? tránh burst trùng.
- Thêm migrations 031�?034 cho index sync/RLS, tombstone, server time, atomic rate/usage counters; Edge Functions dùng RPC counter nguyên t�? và timeout 8 giây cho Resend.
- Dexie v16 thêm compound index SRS/review log, bulk/LRU audio cache có quota 400 MB hoặc 20% browser quota và b�? qua blob đơn vượt ngân sách.
- Thêm b�? k6 200�?1000 VU, fixture token, script EXPLAIN sync và tài liệu chạy an toàn. Chưa chạy lên production và chưa push migration/deploy function.
- Verify: typecheck PASS; hardening 14/14 PASS; full suite 157/158 PASS, lỗi baseline ngoài phạm vi vẫn là `catalogCamReading.test.ts` k�? vọng 47 nhưng catalog hiện có 48. Production build đã qua TypeScript/Vite transform nhưng command tổng b�? timeout khi render chunk sau bước rebuild catalog.
...
- Audited Home `Th�? đến hạn (due: N)` and the `current/target` value. `due: N` now uses one shared valid-due query across Home, SRS popup and Notification Center, excluding orphan SRS rows whose card/deck no longer exists (a likely cause of 14 in the inbox vs 1 selectable card). The progress numerator no longer counts every vocab mode/duplicate retry; it counts unique card IDs reviewed in actual SRS mode. “Today�? now follows local timezone instead of UTC. TypeScript and diff checks pass.
...
- Nguyên nhân: publish upload lên private bucket `exam-media` với `upsert: true`, nhưng migration 019 ch�? có admin INSERT/UPDATE/DELETE �? **không có SELECT**. Storage upsert cần SELECT đ�? pre-check / return row �? RLS fail.
...
- User action: hard refresh, publish lại đ�?; nếu vẫn lỗi admin �? kiểm tra `profiles.is_admin = true` rồi logout/login.

## 2026-07-17 �? Deploy frontend security lên Vercel production

- Vercel CLI login đúng `ryanik1997`; project liên kết `ryanenglishv2`.
- Deploy đầu `dpl_959LwP33UBGWig26T327GSkx8pKz` lỗi do `.vercelignore` loại private `apps/web/public/catalog` nhưng `build-catalog.mjs --if-present` vẫn yêu cầu thư mục này đ�? skip.
- Fix `scripts/build-catalog.mjs`: Vercel dùng committed `packages/catalog/data/manifest.json` khi không có `Tainguyen`; private media không b�? đưa lại vào deploy.
...
- Xác nhận code Security HIGH có đ�? Phase 1.1, 1.2, 1.3, 2.1, 2.2, 2.3, Phase 4 và migration 023.
- B�? verify được báo cáo: `phase1Hardening`, `phase2Hardening`, `phase4Legal`, `BookReaderPage` tổng 10/10 PASS; `tsc --noEmit` PASS.
- Đối chiếu Git: working tree không hoàn toàn sạch vì còn `.claude/settings.local.json` untracked; HEAD thực t�? lúc kiểm tra là `b1db15f7`, không phải `86e26916`.
- Chạy lại `pnpm db:push` production: Supabase project `ntcagvtkwxwsmlxlumfo` tr�? `Remote database is up to date`; migrations 017�?023 đã có trên remote.
- Redeploy `content-sign` thành công lên project `ntcagvtkwxwsmlxlumfo`. CLI lần đầu thiếu token; lần hai ch�? nạp `SUPABASE_ACCESS_TOKEN` t�? `.env.deploy` trong process, không ghi/in secret.
...
- Frontend production đã deploy Ready �? `https://ryanenglishv2.vercel.app`; HTTP smoke `/`, `/terms`, `/privacy` PASS.
- Còn ngoài code: smoke browser Turnstile/OAuth/signed media/admin publish; tạo và review Vercel Firewall draft; x�? lý CAE audio >50MB; cấu hình kênh gửi security alert/PITR/legal review nếu cần.

## 2026-07-17 �? Signup legal consent + email security alerts

- Biến tab `Đăng ký` t�? trang trí thành signup email/password thật; dùng Turnstile hiện có, yêu cầu password >= 8 ký t�? và bắt buộc `TermsConsentCheckbox`.
- Consent version `2026-07-16` được gửi trong Auth metadata. Migration `024_signup_consent_and_security_email.sql` m�? rộng `handle_new_user()` đ�? ghi server timestamp vào `profiles` ngay c�? khi bật email confirmation/chưa có session.
- Với signup có session và Google OAuth signup, app lưu pending version ngắn hạn rồi gọi RPC `accept_legal_terms`; pending được xóa khi thành công hoặc signup lỗi đ�? không ghi nhầm �? login sau.
- Alert quota >=300 request/24h dùng Resend trong `content-sign`. RPC `claim_content_security_alert_email` claim nguyên t�? một email/user/ngày; nếu Resend lỗi thì release claim đ�? request sau retry. DB queue vẫn gi�? nguyên.
- Migration 024 đã push production; `content-sign` mới đã deploy production.
- Frontend commit `3321c983` đã deploy Vercel production Ready tại deployment `ryanenglishv2-okqsjcn1x-ryanenglish.vercel.app`; alias chính gi�? `https://ryanenglishv2.vercel.app`.
- HEAD smoke vào alias chính tr�? HTTP 429 t�? lớp Vercel, nên chưa xác nhận UI signup bằng production browser.
- Blocker email production: Supabase project chưa có các secret `RESEND_API_KEY`, `ADMIN_EMAIL`, `APP_ORIGIN`; `.env.deploy` cũng không có. Cho đến khi set secret, function ch�? cảnh báo log + lưu DB, chưa gửi email thật.
- Verify: scoped security/auth 13/13 PASS; `tsc --noEmit` PASS; production build PASS + strip private media; `git diff --check` PASS.
- Full web suite: 117/118 PASS. Lỗi duy nhất ngoài patch: `catalogCamReading.test.ts` hardcode 47 nhưng catalog hiện có 48 đ�?.

## 2026-07-17 �? Writing subpages: nền lưới đồng b�?

- Mọi route con `/app/writing/*` dùng backdrop `grid`, không có ribbon; `/app/writing` hub gi�? style ribbon hiện có.
- Thêm `.writing-shell` vào CSS transparent layer của backdrop đ�? editor Writing không che ô lưới xanh nhạt.
...
- Entitlement/retention: admin + Pro còn hạn + Lifetime dùng Speaking AI không giới hạn; Free/Trial/Basic gi�? quota 600 giây/ngày. API tr�? access metadata đ�? UI hiện đúng quyền.
- Migration `027_speaking_ai_entitlements_retention.sql`: cron chạy hằng ngày, xóa message/usage quá 30 ngày và conversation rỗng; function dọn d�? liệu không cho anon/authenticated gọi trực tiếp.
- UI Speaking AI hiển th�? `Không giới hạn` cho Pro/admin và thông báo lịch s�? t�? xóa sau 30 ngày; web bump v0.2.6.
...
- Fix tiếp: thêm i18n `nav.speakingAi` cho VI/EN đ�? toolbar không hiện raw key; regression test 1/1 và `tsc --noEmit` PASS.
- Thêm route riêng `/app/speaking-ai` trong sidebar AppShell; trang được lazy-load và dùng backdrop grid/ribbon chung.
- Trang chọn level A1–C1, 7 mode, 6 topic và m�? lại các phiên lịch s�? đã lưu; trạng thái rõ `Ready �? Recording �? Processing �? AI Speaking`; có transcript, correction, natural alternative, giải thích VI, vocabulary, replay, 0.75x/1x/1.25x, nói chậm và retry/error.
- `useSpeakingRecorder`: MediaRecorder gi�? audio cục b�? đ�? replay; Web Speech API (`SpeechRecognition`, `en-US`) tạo transcript trực tiếp trên Chrome/Edge; giới hạn 60 giây, permission error rõ và cleanup stream/object URL.
- Edge Function `speaking-ai` chuyển sang DeepSeek Chat Completions JSON mode (`deepseek-v4-flash` mặc định): ch�? nhận transcript, không upload audio; JWT user bắt buộc, timeout 25s, conversation ownership check và không log/l�? API key.
...
- Verify bản DeepSeek: Speaking AI tests 3/3 PASS; `tsc --noEmit` PASS; production web build PASS + strip private media; full suite 120/121 PASS. Lỗi duy nhất ngoài patch vẫn là catalog Reading k�? vọng 47 nhưng hiện có 48.
- Migration `026_speaking_ai_deepseek.sql` đổi provider mặc định sang `deepseek`; client ch�? gửi transcript + metadata, không còn FileReader/base64/audioData.

### Next session start prompt

Review/chạy migrations 031�?034 trên staging, chạy `scripts/load/explain-sync.sql`, sau đó k6 bằng 1000 token staging và kiểm tra p95/error/correctness trước production. B�? sung transaction duy nhất cho Speaking AI usage + message persistence và lịch prune tombstone/rate counters.

Smoke Speaking AI bằng Chrome/Edge: permission, record 5�?10s, live transcript, reply, TTS, correction, close/reopen history và quota. Safari/Firefox có th�? không h�? tr�? Web Speech API đầy đ�?.

Set Supabase secrets `RESEND_API_KEY`, `ADMIN_EMAIL`, `APP_ORIGIN`, redeploy `content-sign`, rồi test một alert có kiểm soát. Smoke signup email-confirmation + Google consent và kiểm tra `profiles.terms_accepted_at/terms_version/privacy_accepted_at`. Sửa baseline catalog test 47�?48 sau khi xác nhận đ�? th�? 48 hợp l�?.
...
## 2026-07-16 �? Security audit + k�? hoạch nâng bảo mật mức HIGH

- Audit toàn b�? lớp bảo mật trước deploy Vercel: 19 migrations, `content-sign` edge function, `vercel.json`, `.vercelignore`, `strip-public-media-from-dist.mjs`, publish flow.
- **2 l�? hổng CRITICAL phát hiện, CHƯA VÁ �? bắt buộc fix trước deploy:**
  1. `reading_exam_published` / `listening_exam_published` có policy `for select using (true)` không giới hạn role �? anon key (nằm trong bundle) crawl được toàn b�? đ�? **kèm đáp án** (`parts` jsonb chứa `answer` + `explanation`; publish flow ch�? strip `imageKey`).
  2. `books/the-song-of-achilles.pdf` (sách có bản quyền) s�? public trên Vercel �? `.vercelignore` có dòng `!apps/web/public/**/*.pdf` re-include, strip script ch�? xóa `catalog/`+`data/`. Rủi ro DMCA. `ielts-wizard/` (8.4MB ảnh đ�?) cũng public tương t�?.
- Điểm mạnh xác nhận: Mode A Fortress đúng chuẩn (4.3GB catalog private Storage + signed URL 90s + plan gate + rate limit 45/user/phút), Mode D answer vault, RLS user-data đầy đ�? (015), headers/CSP/robots tốt, BYOK không l�? key server.
- **K�? hoạch chi tiết 6 phase đã ghi tại `Security/SECURITY_HARDENING_PLAN.txt`** �? th�? t�? thi công: Phase 1 (vá 2 l�? trên: migration `020_harden_published_exams.sql` + strip answers khi publish + chuyển books/ielts-wizard vào signed flow) �? Phase 5.1 (Vercel firewall) �? Phase 2 (daily quota + Turnstile) �? Phase 4 (Terms/copyright) �? Phase 3 (UI, kèm ghi chú trung thực: layout/UI không chặn tuyệt đối được bằng k�? thuật, ch�? bằng pháp lý + anti-bot).
...
## 2026-07-16 �? Rebuild PDF loading: fetch buffer thay vì PDF.js t�? tải URL (fix lỗi 204)

- User báo `Unexpected server response (204)` khi m�? `/books/the-song-of-achilles.pdf` trong reader.
- Chẩn đoán: file PDF hợp l�? (1MB, header `%PDF-1.4`), dev server tr�? 200 + 206 range đúng qua curl �? lỗi nằm �? tầng transport của PDF.js trong browser. **User xác nhận nguyên nhân: Internet Download Manager bắt range request của PDF.js** (trùng pattern IDM đã ghi nhận session trước).
- Rebuild theo yêu cầu user (thay th�? thay vì vá): `BookReaderPage` gi�? t�? `fetch(pdfUrl, { cache: 'no-store' })` �? kiểm tra `ok`/204/buffer rỗng �? đưa `data: Uint8Array` cho `pdfjs.getDocument` thay vì `url`. PDF.js không còn t�? m�? network request nên không còn b�? intercept.
- Error message rõ ràng khi server tr�? lỗi: `Không tải được PDF (HTTP xxx).`
- Test cập nhật: (1) fetch đúng URL + `getDocument` nhận `data` Uint8Array, không nhận `url`; (2) test mới: server tr�? 204 �? hiện lỗi `HTTP 204`, không gọi `getDocument`.
- Verify: 2/2 BookReaderPage tests PASS; `pnpm --filter web exec tsc --noEmit` PASS.
- Lỗi còn tồn tại: chưa verify bằng browser thật (browser automation không expose); nếu IDM vẫn bắt c�? `fetch()` XHR thì cần user thêm `localhost` vào IDM exclusion list.

### Next session start prompt

Hard refresh `/app/reading-corner/sach/read/cv01`; xác nhận không còn lỗi 204, trang 1 render trên canvas, b�? đếm `1 / 278`. Nếu vẫn lỗi �? kiểm tra IDM: Options �? File types �? thêm `localhost` vào "Don't start downloading from the following sites".

## 2026-07-16 �? Import audio IELTS Listening theo Part (Desktop Dethi)

- Nguồn: `C:\Users\ADMIN\OneDrive\Desktop\Dethi\Đ�? thi IELTS` �? **188/192** file Section/Part MP3 (map Cam 9�?20).
...
- Ảnh user cho thấy iframe native PDF Viewer m�? nhưng báo `0 trên 0`, vùng tài liệu trống.
- CLI parser `unpdf/getDocumentProxy` đọc file thành **278 trang**, chứng minh file nguồn hợp l�?; native blob iframe là lớp gây lỗi trong môi trường hiện tại.
- Loại b�? hoàn toàn `blob:` iframe/native PDF Viewer khỏi `BookReaderPage`.
- Reader mới: fetch `arrayBuffer` �? `resolvePDFJSImport` �? `getDocumentProxy` �? `renderPageAsImage` trang hiện tại thành data URL.
- Ch�? render một trang mỗi lần �? scale 1.6 đ�? gi�? hiệu năng cho 278 trang; thêm nút trang trước/sau, b�? đếm `page / 278`, loading/render/error states.
- Cleanup abort fetch, hủy PDF proxy và b�? kết qu�? render cũ khi đổi trang nhanh.
- Regression test yêu cầu PDF.js render trang 1, ảnh data URL, b�? đếm `1 / 278` và tuyệt đối không có iframe; test đ�? trước fix �? xanh.
...
Hard refresh `/app/reading-corner/sach/read/cv01`; xác nhận trang 1 hiển th�? dưới dạng ảnh, b�? đếm `1 / 278`, nút trang sau render trang 2, không còn toolbar PDF `0/0` và không hiện IDM.

## 2026-07-16 �? Reader PDF nội b�? tránh IDM bắt download

- Ảnh user xác nhận click đã hoạt động nhưng Internet Download Manager bắt URL `.pdf` và m�? dialog download thay vì browser reader.
- Regression test yêu cầu action “Đọc sách�? tr�? route app `/app/reading-corner/sach/read/cv01`, không tr�? trực tiếp file PDF; test đ�? trước fix.
- Thêm lazy route `reading-corner/sach/read/:bookId` và `BookReaderPage`.
- Reader tìm metadata trong books catalog, `fetch()` PDF, tạo `blob:` URL bằng `URL.createObjectURL`, rồi gắn blob URL vào iframe PDF viewer; IDM không nhận direct `.pdf` navigation đ�? chặn.
- Reader có toolbar tên sách/tác gi�?, nút quay lại k�?, loading/error state, cleanup AbortController + revokeObjectURL.
- Nút trong modal tr�? reader route `_self`; FLIP/pointer fixes gi�? nguyên.
...
Hard refresh `/app/reading-corner/sach`, m�? The Song of Achilles và bấm “Đọc sách�?; xác nhận chuyển tới `/app/reading-corner/sach/read/cv01`, không hiện IDM, PDF hiển th�? trong iframe blob và nút “K�? sách�? quay lại đúng.

## 2026-07-16 �? Fix lần 2 hit-test nút Đọc sách trong không gian 3D

- User xác nhận ch�? tắt pointer của bìa vẫn chưa click được; nguyên nhân tiếp theo được khóa bằng regression: `.book-inside` vẫn �? `translateZ(-1px)` và z1, nằm trên mặt phẳng 3D âm.
- Khi modal open, nâng `.book-inside` lên `z-index: 3`, `translateZ(1px)`, gi�? `pointer-events: auto`.
- Chính `[data-book-preview-action]` có `position: relative`, `z-index: 5`, `pointer-events: auto`, `translateZ(8px)`; active state gi�? translateZ khi scale.
- Bìa open tiếp tục `pointer-events: none`; link PDF tiếp tục `_self`.
- Regression CSS kiểm tra đ�? cover pointer-none, inside pointer-auto/z3/Z+1 và action pointer-auto/z5/Z+8; test đ�? trước fix �? xanh.
- Verify: 7 Reading Corner tests PASS; `pnpm --filter web exec tsc --noEmit` PASS; diff-check PASS; live CSS có đ�? Z layers; PDF endpoint 200.
- Lỗi còn tồn tại: chưa click thật bằng browser automation vì browser control không được expose trong phiên này.

### Next session start prompt

Hard refresh `/app/reading-corner/sach`, m�? The Song of Achilles, đợi animation xong và click “Đọc sách�?; kiểm tra hover/click link trên Chrome/Electron và xác nhận chuyển tới PDF.

## 2026-07-16 �? Fix nút Đọc sách b�? bìa 3D block click

- Ảnh user cho thấy nút “Đọc sách�? hiển th�? nhưng không click được; regression CSS xác nhận bìa đã `rotateY(-150deg)` nhưng hitbox vẫn gi�? `pointer-events`, nằm z2 trên trang trong z1.
- Thêm `pointer-events: none` cho `.book-modal-content.is-open .book-cover`; khi bìa m�? xong, lớp bìa không còn chặn click.
- `.book-inside` vốn đã chuyển `pointer-events: auto` �? trạng thái open, nên link PDF nhận click trực tiếp sau fix.
- Gi�? link PDF `target="_self"` đ�? không ph�? thuộc popup/tab mới.
- Regression test kiểm tra đồng thời cover open = pointer none và inside open = pointer auto; test đ�? trước fix �? xanh sau fix.
...
Hard refresh `/app/reading-corner/sach`, m�? The Song of Achilles, đợi bìa xoay m�? rồi click “Đọc sách�?; xác nhận nút nhận hover/click và tab hiện tại chuyển đến PDF.

## 2026-07-16 �? Fix lần 2 nút Đọc sách: điều hướng cùng tab

- User xác nhận native anchor `target="_blank"` vẫn không m�? trong môi trường hiện tại; kết luận môi trường chặn tab/cửa s�? mới, không phải lỗi PDF vì endpoint luôn 200 `application/pdf`.
- Regression test đổi yêu cầu của action PDF sang `target="_self"`; test đ�? khi code còn `_blank`.
...
- Clone trong FLIP modal gi�? nguyên `_self` và tabIndex 0; người dùng dùng nút Back của browser đ�? quay lại k�?.
- Verify: regression test đ�? �? xanh; 6 Reading Corner tests PASS; `pnpm --filter web exec tsc --noEmit` PASS; diff-check PASS; PDF endpoint 200 `application/pdf`.
- Lỗi còn tồn tại: chưa click navigation thật bằng browser automation vì browser control không được expose trong phiên này.

### Next session start prompt

Hard refresh `/app/reading-corner/sach`, m�? The Song of Achilles, click “Đọc sách�?; xác nhận tab hiện tại chuyển thẳng đến PDF và nút Back quay lại k�?.

## 2026-07-16 �? Fix nút Đọc sách không m�? PDF

- User xác nhận click “Đọc sách�? không m�? gì dù PDF endpoint đã 200; nguyên nhân đáng tin cậy nhất là cơ ch�? `window.open()` bằng JS b�? trình duyệt/in-app environment chặn.
- Regression test yêu cầu action của sách đã import phải là `HTMLAnchorElement` native với `href`, `target="_blank"` và `rel="noopener noreferrer"`; test đ�? khi action còn là button.
- `BilingualBooksPage` render `<a>` native cho sách có `pdfUrl`; sách chưa có PDF tiếp tục render `<button>` “Đọc thử�?.
- Preview controller ch�? gắn fallback handler “đang biên tập�? cho `HTMLButtonElement`; anchor PDF được clone nguyên vẹn, tabIndex chuyển 0 và đ�? trình duyệt x�? lý navigation trực tiếp.
- CSS action b�? sung inline-flex/center/text-decoration none đ�? anchor gi�? đúng giao diện nút cũ.
- Verify: regression test đ�? �? xanh; 6 Reading Corner tests PASS; `pnpm --filter web exec tsc --noEmit` PASS; diff-check PASS; live module có native href; PDF endpoint tiếp tục 200.
- Lỗi còn tồn tại: chưa click native anchor bằng browser automation vì browser control không được expose trong phiên này.

### Next session start prompt

Hard refresh `/app/reading-corner/sach`, m�? The Song of Achilles và click “Đọc sách�?; xác nhận browser m�? `/books/the-song-of-achilles.pdf` �? tab mới. Nếu môi trường vẫn chặn tab mới, chuyển `target` sang `_self`.

## 2026-07-16 �? Import The Song of Achilles PDF vào k�? sách
...
- `BookCover` h�? tr�? `pdfUrl`; preview của sách có PDF hiển th�? nút “Đọc sách�?, sách chưa có file vẫn là “Đọc thử�?.
- Preview controller đọc `data-book-preview-url` và m�? PDF bằng tab mới với `noopener,noreferrer`; fallback “đang biên tập�? gi�? nguyên cho sách chưa import.
- Thêm regression test catalog→DOM và test controller m�? đúng PDF.
- Verify: 6 Reading Corner tests PASS; `pnpm --filter web exec tsc --noEmit` PASS; diff-check PASS; Vite HEAD `/books/the-song-of-achilles.pdf` tr�? 200, `application/pdf`, đúng 1,018,904 bytes.
- Lỗi còn tồn tại: chưa click PDF bằng browser automation vì browser control không được expose trong phiên này.

### Next session start prompt

Hard refresh `/app/reading-corner/sach`, click “The Song of Achilles�?, đợi animation m�? sách, bấm “Đọc sách�? và xác nhận PDF m�? �? tab mới; smoke-test đóng modal/drag shelf vẫn bình thường.

## 2026-07-16 �? Fix StudySession grid làm xuyên bảng Vocabulary

- Feedback ảnh xác nhận `.vocab-study-shell` transparent làm toàn b�? bảng t�?, toolbar và ch�? phía dưới xuyên qua màn hình học, gây chồng lớp.
- Đổi study shell t�? transparent sang background kín gồm màu `--reading-corner-bg` + hai linear-gradient grid 32px.
...
- Regression test đổi yêu cầu t�? transparent sang opaque grid surface: phải có background color, grid line và background-size 32px; test đ�? trước fix �? xanh sau fix.
...
Hard refresh `/app/vocab`, m�? SRS khi bảng deck đang hiện phía dưới; xác nhận ch�? thấy nền xanh grid + UI học, không còn hàng t�?/toolbar xuyên qua, rồi smoke-test 8 mode còn lại và light/mid/dark.

## 2026-07-16 �? Grid cho toàn b�? ch�? đ�? học Vocabulary

- Áp dụng ô lưới của `/app/vocab` cho c�? 9 mode dùng chung `StudySession`: SRS, Quiz, Type, Listen & Type, Speaking, Weak Words, Review, Stats và Notebook.
- Nguyên nhân lớp grid b�? che trong mode học: `.vocab-study-shell` là overlay `absolute inset-0 z-40` và có `background: var(--vs-shell-bg)`.
- Trong `.app-shell--grid`, ép riêng `.vocab-study-shell` v�? transparent; stat bar, mode tabs, flashcard, quiz card, input, bảng thống kê và notebook card vẫn gi�? surface riêng.
- Không render ribbon vì `/app/vocab` tiếp tục �? backdrop mode `grid`.
- Regression assertion đã đ�? trước fix vì thiếu selector study shell, sau fix xanh.
...
- Lỗi còn tồn tại: chưa có screenshot rendered t�? động vì browser control không được expose trong phiên này.

### Next session start prompt

Hard refresh `/app/vocab`; m�? lần lượt SRS, Quiz, Type, Listen & Type, Speaking, Weak Words, Review, Stats, Notebook và xác nhận grid 32px hiện phía sau, không có ribbon, card/input/panel vẫn rõ �? light/mid/dark.

## 2026-07-16 �? Fix grid Vocabulary b�? CardPanel che

- Repro theo trạng thái `activeDeckId`: route `/app/vocab` đã đúng mode `grid`, nhưng `CardPanel` full-height vẫn ph�? `var(--bg-primary)` nên người dùng không thấy ô lưới khi một deck đang được nh�?/m�?.
- Thêm hook `.vocab-card-panel` cho c�? trạng thái deck đang tải/chưa có và trạng thái deck đã m�?.
- Trong `.app-shell--grid`, ép riêng `.vocab-card-panel` v�? `background: transparent !important`; header, bảng, card và modal bên trong gi�? surface riêng.
- Thêm regression test render CardPanel thật với store/query mock, đồng thời kiểm tra selector CSS grid-mode tồn tại.
- Verify: regression loop đ�? 2/2 trước fix �? xanh; tổng 63 tests PASS; `pnpm --filter web exec tsc --noEmit` PASS; diff-check PASS; live Vite CSS/module PASS; `/app/vocab` HTTP 200.
- Lỗi còn tồn tại: chưa có screenshot rendered t�? động vì browser control không được expose trong phiên này.

### Next session start prompt

Hard refresh `/app/vocab` khi đang �? danh sách deck và khi một deck đã m�?; xác nhận grid 32px đều hiện, không có ribbon, header/bảng/card vẫn d�? đọc �? light/mid/dark.

## 2026-07-16 �? Grid-only cho Writing subpages và Vocabulary

- Tách backdrop AppShell thành 3 mode: `none`, `grid`, `ribbon`; mode `grid` render ô lưới nhưng không tạo ba phần t�? ribbon.
- Chuyển `/app/vocab` t�? ribbon sang grid-only.
...
- Dùng lớp chung `.app-shell--backdrop` đ�? g�? nền ngoài của Writing layout, `.cb-hub`, `.wd-page` và Vocabulary `.app-page-surface`; card/form/header bên trong vẫn gi�? surface riêng.
...
Hard refresh `/app/vocab`, `/app/writing/translate/grammar_basic`, `/app/writing/practice/task2`, `/app/writing/cambridge/c2`, `/app/writing/dashboard`; xác nhận ch�? có grid 32px, tuyệt đối không có ribbon, card/form vẫn rõ �? light/mid/dark.

## 2026-07-16 �? Grid + ribbon cho Exam Track, Shadowing lesson và Sentence catalog

- M�? rộng `hasAppRibbonBackdrop()` cho đúng các route Exam Track IELTS/Cambridge được yêu cầu, gồm level A2–C2 và các trang Listening/Reading.
...
- Bật backdrop cho mọi Sentence Structure ID dạng `catalog:ss:*`, h�? tr�? c�? dấu `:` trực tiếp và `%3A` URL-encoded.
- G�? nền đặc ch�? �? container ngoài: `.exam-hub-page`, `.exam-skill-picker`, `.shadowing-detail`, `.ss-shell`; card, player, transcript và panel vẫn gi�? surface riêng.
- Tắt dot texture `ss-shell::before` đ�? không chồng lên grid 32px dùng chung.
- Verify: 41 matcher tests PASS; `pnpm --filter web exec tsc --noEmit` PASS; `pnpm --filter web build` PASS; 4 URL đại diện HTTP 200; live Vite CSS chứa đ�? selector mới.
- Lỗi còn tồn tại: chưa có screenshot rendered t�? động vì browser control không được expose trong phiên này. `pnpm build` đã chạy `build:catalog` và làm mới các file catalog sinh t�? động; gi�? nguyên, không t�? ý hoàn tác worktree.

### Next session start prompt

Hard refresh một route mỗi nhóm: `/app/exam/track/cambridge/c2/reading`, `/app/exam/track/ielts/listening`, `/app/shadowing/28EFRJaA2JQ?mode=shadowing`, và một `/app/sentence-structure/catalog:ss:*`; xác nhận grid/ribbon hiện sau nội dung, card/player vẫn rõ �? c�? light/mid/dark.

## 2026-07-16 �? Thêm ô lưới cho các trang con Reading Corner

- `/bao` và article reader gi�? grid dùng chung `.snb-ribbon-grid` đã có.
- `/sach` b�? sung `.library-camera::before`: grid 32×32px màu amber nhạt 14%, opacity .42, soft-light.
...
Hard refresh `/app/reading-corner/sach` và `/bao`; kiểm tra ô lưới 32px đ�? nh�? trên ảnh thư viện và rõ trên nền xanh, không che card/bookcase.

## 2026-07-16 �? Thêm nút Quay lại cho Reading Corner hub

- `/app/reading-corner` thêm `Link.rc-hub-back` �? đầu nội dung, điều hướng v�? `/app/home`.
- Nút có icon mũi tên trái, glass surface theo theme tokens, blur 10px, hover dịch trái nh�? và focus-visible.
- Nút nằm trong flow trước header nên không che tiêu đ�? trên mobile.
...
Hard refresh `/app/reading-corner`; kiểm tra nút Quay lại trên desktop/mobile và xác nhận điều hướng v�? `/app/home`.

## 2026-07-16 �? G�? mouse tilt, thay bằng ambient background drift

- Xóa toàn b�? `sceneRef`, `cameraRef`, preview tilt refs, mousemove/mouseleave listeners, RAF lerp và CSS vars/rotateX/rotateY của camera.
- `.library-camera` gi�? lại làm wrapper tĩnh cho background/scene; `stage.is-preview-open` vẫn gi�? vì ch�? khóa shelf khi FLIP modal m�?.
- `.library-bg` chạy `ambient-drift` độc lập: scale 1�?1.03, 30s ease-in-out alternate infinite, transform-origin center bottom.
- Không thêm scroll parallax/listener đ�? tránh chi phí và xung đột drag shelf.
- Reduced-motion tắt ambient animation, transform và will-change.
- Xóa regression test camera tilt; gi�? test không có RAF lúc render và toàn b�? drag/click/FLIP tests.
...
- Feedback loop đ�? xác nhận 3 vấn đ�? cùng tồn tại: bottom fade `position:fixed`, focus shadow áp trực tiếp lên `.book`, shelf occlusion dùng 88% shadow.
- Focus halo chuyển sang `.book::before`, inset -8px và radius 8px; `:focus-visible` ch�? bật opacity. Ring dùng spread 6px 50% + glow 24px/4px 30%, tạo khoảng h�? t�? nhiên.
- Focused book z11 đ�? halo không b�? shelf front z8 che; contact shadow tiếp tục dùng `.book::after`.
- Bottom fade chuyển t�? `.library-scene::before` fixed sang `.library-camera::after` absolute bottom, nên ch�? xuất hiện �? cuối thật của toàn b�? scene thay vì đáy viewport/hàng 2.
- Shelf-front shadow và lower-row occlusion giảm 18�?16px, shadow mix 88%�?60% (effective khoảng 24% t�? base rgba .4).
...
Hard refresh `/app/reading-corner/sach`; Tab vào sách kiểm tra halo cách bìa ~8px, cuộn qua hàng 2 xác nhận không còn dải ch�? nhật tối và kiểm tra fade ch�? xuất hiện �? cuối scene.

## 2026-07-16 �? Warm UI rings, glass pills và bottom fade

- Ch�? sửa `readingCorner.css`; không đổi DOM/handlers/animation sách.
- Book `:focus-visible` b�? outline xanh/tím, dùng amber ring 3px 70% + glow 20px 40%; vẫn gi�? keyboard accessibility.
- Nút “Góc đọc�? và “Đọc Báo Song Ngữ�? dùng walnut glass 50%, blur 10px, amber border 30%, cream text và shadow 4px/12px.
- Hover pill pha nh�? `--library-accent`, selector có specificity cao hơn rule trắng legacy.
- `.library-scene::before` tạo bottom fade fixed 100�?150px, transparent �? dark walnut 96% �? 90%; z850, pointer-events none. Grain tiếp tục �? `::after` z900.
...
Hard refresh `/app/reading-corner/sach`; dùng Tab kiểm tra amber focus ring, hover hai glass pill và cuộn xuống cuối đ�? kiểm tra fade không che click/drag sách.

## 2026-07-16 �? Header depth + dust motes cho library scene

- Thêm `.library-header-depth` cao 280�?480px phía sau intro, `backdrop-filter: blur(2.5px) brightness(.85)` và mask fade xuống dưới; bookcase ngoài vùng này gi�? nguyên nét.
...
- Thêm đúng 18 `.library-dust__particle` deterministic; size 2�?4px, duration 8�?15s, negative delay, drift riêng; animation ch�? transform/opacity.
- Dust nằm trong `.library-camera`, cùng tilt với scene nhưng pointer-events none; reduced-motion tắt animation và will-change.
- Mobile gi�? vertical padding intro đ�? mask/glow không b�? cắt; thêm WebKit mask fallback.
...
Hard refresh `/app/reading-corner/sach`; kiểm tra header feather/glow, blur ch�? �? vùng trên, 18 dust motes, camera tilt và reduced-motion/mobile; xác nhận bookcase vẫn sắc nét.

## 2026-07-16 �? Camera tilt + spine depth tương tác

- Thêm `.library-camera` bên trong `.library-scene`; scene gi�? scroll/viewport ổn định, camera chứa background + intro + bookcase và nhận transform 3D.
- Mousemove fine-pointer chuẩn hóa -1..1, target rotateX tối đa ±4°, rotateY ±6°; RAF lerp h�? s�? .08 và ch�? tiếp tục khi còn sai s�?, không có loop idle.
- Touch/coarse pointer và `prefers-reduced-motion` không đăng ký tilt; CSS reduced-motion tắt transform/will-change camera.
- Khi FLIP modal m�?: `previewOpenRef`, class `.is-preview-open` và reset callback đưa target v�? 0; CSS ép camera 0° ngay đ�? nội dung đọc ổn định.
- Mỗi `.book` có `--book-cover-y` deterministic theo ID trong khoảng -6..6° và `--book-cover-image`.
- Shelf `.book-cover` dùng preserve-3d/rotateY; `::before` tạo gáy 8�?12px rotateY(90°), lấy chính ảnh bìa và brightness .7. Modal không k�? thừa vars outer book nên vẫn m�? phẳng.
- Occlusion gi�? shelf front z8 cao hơn book z7; shadow overlay tầng trên z10 tiếp tục che t�? nhiên đầu sách tầng dưới.
- Regression test mới kiểm tra RAF lerp, camera vars, preview reset class và toàn b�? cover angle range.
...
Hard refresh `/app/reading-corner/sach`; rê chuột đến 4 góc, kiểm tra tilt �?6°, drag shelf, hover sách, spine depth và camera reset khi modal m�?; th�? thêm touch/mobile/reduced-motion.

## 2026-07-16 �? Đồng chất ảnh thật và CSS bookcase

- Root scene thêm class `.library-scene`; `::after` ph�? SVG `feTurbulence` noise 180×180, opacity .05, blend overlay lên c�? ảnh nền và bookcase.
- `.bookcase-container` có color grade `saturate(.92) contrast(1.05) brightness(.98) sepia(.05)`, shadow blend lớn 100px/40px đ�? mềm viền.
- Container overlay kết hợp side vignette 30%, warm radial highlight �? giữa-trên và orange grade 8%�?5%, `mix-blend-mode: soft-light`.
...
- Contact shadow mỗi sách đổi thành dải 5px opacity .5 với box-shadow 2px/3px, sát mặt k�?; hover vẫn bù translate đ�? bóng �? lại shelf.
...
Hard refresh `/app/reading-corner/sach`; kiểm tra warm grade/noise/vignette, cạnh bookcase hòa nền, contact shadow và đ�? sắc nét bìa trong modal trên desktop/mobile.
...
- `BilingualBooksPage` thêm `.library-bg`, `.library-bg-overlay` và `.bookcase-container`; bookcase/shelf-row/book DOM bên trong gi�? nguyên.
- Background fixed/cover/center-bottom với fallback `#3d2b1f`, thay hoàn toàn nền grid xanh trên route `/reading-corner/sach`.
- Overlay dùng radial vignette tối nhất �? trung tâm + gradient dọc đ�? giảm chi tiết ảnh thật phía sau bookcase.
...
- Intro chuyển sang text trắng/muted có text-shadow đ�? đọc rõ trên ảnh nền.
- Không bật mousemove parallax trong patch này đ�? tránh tranh pointer với drag-to-scroll từng shelf và gi�? hiệu năng ổn định.
- Regression test b�? sung xác nhận 3 lớp background/container; 3 hover/click/FLIP tests vẫn PASS.
...
Hard refresh `/app/reading-corner/sach`; kiểm tra ảnh cover trên desktop/mobile, vignette trung tâm, text contrast, shadow/blend bookcase và FLIP sau khi scroll ngang. Ch�? thêm parallax nếu drag shelf vẫn ổn.

## 2026-07-16 �? Tăng chiều sâu 3D CSS cho bookcase

- Ch�? sửa `readingCorner.css`; không đổi component, drag handlers hay FLIP controller.
- Side-left/right dùng clip-path hình thang và `perspective(800px) rotateY(±7deg)` với transform-origin �? mép ngoài, tạo cảm giác vách m�? v�? phía người xem.
...
- Shelf front z8 và `::after` tạo bóng 18px xuống compartment k�? tiếp; thêm overlay tương ứng trên đỉnh `.shelf-books` t�? row th�? hai đ�? bóng chạm phần đầu bìa.
- Top/bottom/side frame có repeating-linear-gradient vân g�? opacity thấp; rim light đồng nhất t�? trên-trái, cạnh dưới/phải tối hơn.
- B�? `filter: blur(5px)` khỏi shelf shadow; toàn b�? lớp mới là static transform/gradient/box-shadow, không thêm animation loop.
...
Hard refresh `/app/reading-corner/sach`; kiểm tra side panels xiên, đường gấp shelf-top/front, vân g�?, bóng trên đầu sách tầng dưới và rim light trên-trái �? desktop/mobile.

## 2026-07-16 �? Nâng cấp các k�? rời thành bookcase hoàn chỉnh

- Tham chiếu `Crawl/Giaodien/sheft.jpg`: t�? thư viện walnut tối, khung crown dày, back panel kín và ánh sáng ấm t�? trên.
...
- Back panel `#4a3728` có vân dọc nh�?, inset side shadow và bóng radial/linear riêng �? góc trên từng shelf-row.
- Shelf front tăng lên 14�?17px, kéo sát mép trong hai tr�?; b�? support rời của từng row đ�? toàn b�? thanh ngang tr�? thành một phần của khung t�?.
- Light overlay ph�? interior t�? sáng nh�? phía trên xuống tối phía dưới; pointer-events none nên drag/click sách không b�? ảnh hưởng.
- Responsive dưới 700px: frame side 18px, top 30px, bottom 32px; ẩn divider đ�? gi�? diện tích sách và horizontal scroll.
- Regression test b�? sung xác nhận back/top/bottom, 2 side và 2 divider; click/FLIP tests vẫn PASS.
...
Hard refresh `/app/reading-corner/sach`; đối chiếu `sheft.jpg`: kiểm tra crown frame, panel sau kín, shelf nối tr�?, divider, ánh sáng top-down, horizontal scroll và FLIP trên desktop/mobile.

## 2026-07-16 �? Nâng cấp shelf bar thành k�? g�? hai lớp

- Ch�? sửa `readingCorner.css`, không đổi HTML/React/controller.
...
- `.shelf-row` có hai giá đ�? dọc 18×36px bằng layered background, cùng tone g�? tối.
- `.book::after` tạo bóng ellipse dưới từng cuốn; khi hover bóng bù `translateY` đ�? nằm gần mặt k�? trong lúc bìa được rút lên.
- Điều chỉnh stacking: sách z7, hover z9, shelf z6 đ�? bóng nằm trên mặt g�? nhưng k�? vẫn đ�? đúng đáy sách.
...
Hard refresh `/app/reading-corner/sach`; kiểm tra mặt trên sáng, mặt trước tối, hai giá đ�? và bóng ellipse �? c�? ba theme; xác nhận hover/click FLIP không đổi.

## 2026-07-16 �? Fix click sách trên k�? không m�? FLIP preview

- Root cause: drag-to-scroll gọi `setPointerCapture()` ngay t�? `pointerdown`, khiến browser có th�? retarget click t�? `.book` sang `.shelf-track`; handler `openBook()` không chạy.
- Fix: `pointerdown` ch�? lưu trạng thái; ch�? capture pointer và bật `is-dragging` sau khi di chuyển ngang vượt threshold 6px.
- Click bình thường tiếp tục vào `bookPreviewController.openBook()`; kéo ngang vẫn gi�? pointer sau khi xác định đúng gesture.
...
Hard refresh `/app/reading-corner/sach`; click trực tiếp bìa đ�? kiểm tra FLIP + lật m�?, sau đó kéo ngang trên cùng bìa đ�? xác nhận không m�? nhầm modal.

## 2026-07-16 �? Đổi coverflow thành k�? sách thư viện tại /reading-corner/sach

- B�? hoàn toàn orbit 3D, `requestAnimationFrame`, góc quay tích lũy và auto-rotate; không còn transform định v�? tuyệt đối từng sách.
- Chia 27 cuốn thành 3 `.shelf-row`, mỗi hàng 9 cuốn; `.shelf-books` dùng Flexbox, đáy sách chạm `.shelf-bar`.
- Thanh k�? dùng gradient/shadow theo CSS theme tokens; nền trang chuyển sang `--reading-corner-bg` và grid token nên h�? tr�? Sáng/Tối vừa/Tối.
- 3/27 cuốn (11%) nghiêng deterministic 4�?8 đ�?; hover rút riêng sách đang chọn lên 24px và scale 1.05 bằng transform.
...
- Khối text “Tính năng đang được cập nhật�? chuyển thành intro phía trên toàn b�? k�?.
- Gi�? nguyên FLIP + lật bìa: controller tiếp tục lấy `getBoundingClientRect()` trực tiếp t�? v�? trí sách trên k�? và tr�? đúng v�? đó khi đóng.
- Regression test `BilingualBooksPage.test.tsx`: 3 k�?, 3 shelf bar, 27 sách, 3 sách nghiêng và không khởi chạy auto-rotate.
...
M�? `/app/reading-corner/sach`, kiểm tra desktop/mobile: sách chạm k�?, hover không b�? clip, kéo ngang không m�? nhầm modal, FLIP đi/v�? đúng v�? trí sau khi hàng đã scroll.

## 2026-07-16 �? Preview m�? sách bằng FLIP tại Reading Corner /sach

- Mỗi cuốn trong coverflow có cấu trúc `.book-modal-content` gồm `.book-cover` và `.book-inside`; phần trong dùng metadata thật t�? catalog, có placeholder mô t�? và nút “Đọc thử�?.
- Thêm controller DOM thuần `bookPreviewController.ts` với `openBook(bookElement)` / `closeBook()`: clone sách, overlay fixed + blur, FLIP transform v�? giữa màn hình, lật bìa `rotateY(-150deg)`, rồi đảo animation v�? đúng v�? trí gốc.
- Đóng được bằng nút X, click vùng tối hoặc Escape; khóa scroll, quản lý focus, h�? tr�? Enter/Space và `prefers-reduced-motion`.
- Coverflow chuyển sang góc quay tích lũy đ�? pause thật khi modal m�?, không nhảy v�? trí lúc resume.
- Animation ch�? thay đổi `transform` và `opacity`; kích thước modal được xác lập một lần trước transition.
- Regression test `bookPreviewController.test.ts` kiểm tra clone, m�? ruột sách, khóa body và phục hồi source khi đóng.
...
Smoke-test trực quan `/app/reading-corner/sach`: click nhiều v�? trí trên vòng cung, kiểm tra FLIP đi/v�?, lật bìa, Escape/click ngoài và resume auto-rotate trên desktop/mobile.

## 2026-07-16 �? Fix nền/ribbon khi chuyển theme Tối và Tối vừa

- Root cause: 5 token nền Reading Corner/ribbon ch�? được khai báo �? theme Sáng, nên theme `mid` và `dark` vẫn k�? thừa nền xanh nhạt cùng ribbon sáng, gây sai tương phản.
- Thêm palette riêng cho `--reading-corner-bg`, `--reading-corner-grid-line`, `--reading-ribbon-soft/mid/core` trong c�? `[data-theme="mid"]` và `[data-theme="dark"]`.
- Reading Corner `/app/reading-corner` và `/bao` dùng `var(--reading-corner-bg)` thay màu nền hardcode; tiêu đ�?, mô t�? và eyebrow có override theme-aware đ�? gi�? đ�? đọc.
- Thêm regression test `styles/themeBackdropTokens.test.ts`, bắt buộc theme Tối/Tối vừa khai báo đ�? 5 token.
- Verify: 19 tests PASS; `pnpm --filter web exec tsc --noEmit` PASS. `git diff --check` ch�? còn dòng trắng cuối `listeningTest.css` thuộc thay đổi có sẵn, không liên quan bản vá này.
- Lỗi còn tồn tại liên quan bản vá: chưa ghi nhận.
...
## 2026-07-16 �? Fix CTA Landing không còn m�? Google OAuth trực tiếp

- Root cause: `LandingPage.startFree()` vẫn gọi `signInWithGoogle()` khi user chưa đăng nhập, nên hai nút “Vào lớp học�? / “Bắt đầu miễn phí�? b�? qua màn hình login custom.
- Fix: mọi CTA miễn phí điều hướng tới `/app`; `ProtectedRoute` render `LoginPage` giống mockup khi chưa đăng nhập. Google OAuth ch�? chạy sau khi bấm nút đăng nhập bên trong `LoginPage`.
- Tắt `VITE_DEV_AUTH_BYPASS` trong `apps/web/.env.local`; Vite dev đang phục v�? giá tr�? `"0"` nên `/app` không còn b�? qua login.
- Đồng thời b�? màu hardcode còn sót trong `loginPage.css`, thay bằng CSS variables.
...
- Chuyển động nổi map theo mascot Tổng quan và h�? tr�? `prefers-reduced-motion`.
...
- `AuthContext` thêm `signInWithPassword()` dùng Supabase Auth; nút “Đăng nhập ngay�? submit email/mật khẩu, nút Google gi�? OAuth riêng.
- Typography trang Login đổi t�? Instrument Serif sang `Segoe UI Variable Display` + `Segoe UI Variable`; thêm token `--color-danger` cho đ�? light/mid/dark.
...
- `rc-hub` và Login cùng dùng nền xanh nhạt, lưới trắng 32px; Login map thêm lớp noise nh�? giống `rc-hub-ambient`.
- B�? radial tím cũ �? Login đ�? màu nền không lệch `/app/reading-corner`.
...
- Thêm 3 ribbon chéo c�? định phía sau card Login, map đúng geometry `280vmax`, góc 45°, offset, opacity và thời lượng animation t�? `/app/reading-corner/bao`.
- Tách palette ribbon thành token dùng chung `--reading-ribbon-soft/mid/core`; Reading Corner và Login cùng s�? dụng.
- `prefers-reduced-motion` tắt ribbon animation; form và mascot gi�? z-index phía trên.
- Regression test xác nhận Login render đ�? 3 ribbon.
- Verify: LoginPage test PASS; ribbon mapping invariant PASS; live Vite module PASS; `pnpm --filter web exec tsc --noEmit` PASS.
- Sau feedback, riêng ribbon Login đổi sang `rotate(-45deg)` đ�? hướng t�? trái dưới lên góc phải màn hình; live Vite CSS PASS.

## 2026-07-16 �? Nền xanh + ribbon cho 9 hub AppShell

- Thêm backdrop chung tại `AppShell`: nền xanh Reading Corner, lưới trắng 32px, noise nh�? và 3 ribbon `-45deg`.
- Ch�? bật �? đúng route gốc: `/app/home`, `/app/vocab`, `/app/writing`, `/app/listening`, `/app/shadowing`, `/app/exam`, `/app/sentence-structure`, `/app/settings`, `/app/admin`.
- Không bật �? Reading Corner vì có nền riêng; không bật �? bài học, bài luyện và exam player full-screen.
- Làm trong suốt outer surface của Home, Vocab, Writing, Listening, Shadowing, Exam, Sentence Structure, Settings và Admin; card/sidebar/toolbars gi�? nền riêng.
...
- Fix b�? sung `/app/writing`: `WritingLayout` có 2 wrapper inline background ph�? backdrop; thêm class `writing-layout` / `writing-layout__content` và override trong suốt ch�? khi route hub active. Verify: 16 route tests PASS; live WritingLayout PASS; tsc PASS.
...
- Hub Góc đọc thêm đ�? 3 ribbon trên nền xanh lưới; outer `rc-hub--ribbon` chuyển transparent đ�? backdrop hiện đúng.
- Ribbon `/bao` đổi t�? `rotate(45deg)` sang `rotate(-45deg)`, đồng nhất hướng trái dưới lên góc phải.
- Article reader vẫn dùng ch�? đ�? grid-only, không ribbon.
...
- `ListeningTest`: IELTS không dùng overlay/shell cũ �? TidShell t�? quản Play + layout.
- Gi�? catalog đ�? + map options + note form (`ListeningIeltsPartView`), draft/submit/review.
...
- **Fix runtime:** `listeningLetterOptions.ts` + Map/Diagram blocks suy A–H t�? instruction/answers.
...
- Matching block hiển th�? `partImageUrl` khi có hình; `examMediaUrl` encode path segment.
...
- Layout TID: header logo + title + timer + **Kiểm Tra**, part banner `#f1f2ec`, footer part tabs; gi�? PartView (note/map/MC/matching), draft, submit, review, audio local.
...
- Nút Landing “Vào lớp học�? / “Bắt đầu miễn phí�? gọi `/app`; route index `/app` đổi t�? `/app/vocab` sang `/app/home`.
- OAuth callback sau Google login đổi t�? `/app/vocab` sang `/app`, đ�? người dùng mới cũng vào đúng màn Tổng quan.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? Rollback visual redesign Tổng quan

- G�? các override `Premium dashboard layer` và `CTA landing destination` trong `homePage.css`; Tổng quan quay v�? layout Home cũ thay vì hero H1 quá lớn.
- Không đổi route `/app` hoặc logic đa ngôn ng�?.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.
...
- Khi user chưa đăng nhập và bấm CTA Landing vào `/app`, `ProtectedRoute` không redirect v�? Landing nữa mà render `LoginPage`.
...
- IELTS và Cambridge Listening dùng chung nút tốc đ�? cạnh nút Play.
- Chu k�? tốc đ�?: **1× �? 0.75× �? 0.5× �? 1×**; thay đổi ngay trên audio đang phát.
...
- Listening media upload trước đây không có timeout; nếu Supabase Storage không phản hồi, nút Publish gi�? loading vô hạn.
- Thêm timeout 120 giây cho từng upload media và reset progress khi bắt đầu Publish mục mới.
- Verify: `pnpm -C apps/web build` PASS.

## 2026-07-16 �? Giai đoạn 1 đa ngôn ng�? giao diện
...
- Thêm lựa chọn ngôn ng�? trong Settings �? Giao diện.
- AppShell t�? cập nhật nhãn navigation theo ngôn ng�? đã chọn.
- Nội dung bài học/đ�? thi chưa dịch; ch�? dịch khung giao diện chính trong giai đoạn 1.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? Giai đoạn 2 m�? rộng ngôn ng�? giao diện

- M�? rộng `language.tsx` t�? 2 lên đ�? **17 ngôn ng�?**: English, Arabic, German, Greek, Spanish, Indonesian, Japanese, Korean, Malay, Portuguese, Russian, Thai, Turkish, Ukrainian, Vietnamese, Simplified Chinese, Traditional Chinese.
- Thêm nhãn bản địa và bản dịch navigation + các nhãn chính của Settings cho từng ngôn ng�?.
- Gi�? cơ ch�? lưu preference localStorage + Dexie `settingsRepo`; ngôn ng�? chưa có bản dịch �? khu vực khác s�? fallback v�? Vietnamese.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? Giai đoạn 3 RTL và locale

- Arabic t�? đặt `dir="rtl"`; các ngôn ng�? còn lại dùng `dir="ltr"`.
- Cập nhật `document.documentElement.lang` khi đổi ngôn ng�?.
- Thêm `formatLocaleDate()` với locale tương ứng cho ngày tháng.
- Tab Settings dùng nhãn dịch theo ngôn ng�? đã chọn.
...
- Quick actions trên HomePage lấy nhãn t�? i18n nên đổi theo ngôn ng�?: Vocabulary, Writing, Listening, Translate, MindMap.
- Gi�? nguyên d�? liệu thống kê, streak và nội dung học tập.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? B�? sung dịch text còn sót �? AppShell/Home

- Chuyển thêm fallback user, đăng xuất, sidebar expand/collapse, trạng thái đồng b�?, theme label và lifetime plan sang i18n.
- Chuyển nhãn thống kê Home và tiêu đ�? nhóm “Học ngay�? sang i18n.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? Map toàn b�? giao diện trang Tổng quan

- Chuyển greeting, subtitle, mascot message, quick-action descriptions và thống kê Home sang i18n.
- Chuyển StudyActivityGrid: tiêu đ�?, mô t�?, legend, aria-label và active-day text.
...
- Chuyển StreakCelebration: tiêu đ�? và thông báo streak.
- Với ngôn ng�? chưa có bản dịch riêng cho key mới, fallback dùng English thay vì Vietnamese đ�? tránh trộn ngôn ng�?.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? Map giao diện Vocabulary

- Chuyển trang `/app/vocab` sang i18n cho tiêu đ�?, sửa deck trùng, notebook, tạo deck, loại t�? và trạng thái repair.
- Chuyển DeckGrid cho b�? lọc, confirm/error xoá deck.
- Chuyển CardPanel cho chọn deck, export/import, thêm t�? và empty state.
- D�? liệu deck/card không thay đổi.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-16 �? B�? sung 120 guide Task 2 TID Writing

- Thêm `scripts/fill-tid-task2-guides.mjs` đ�? sinh guide HTML tĩnh theo 6 genre Task 2.
- B�? sung guide + outline + useful language + thesis direction + model answer cho **120/120** đ�? thiếu.
- Cập nhật `apps/web/public/catalog/writing/tid/tasks.json`: **356/356** đ�? có `guideHtml`; Task 1 không thay đổi.
- Không gọi AI/API.
...
- Tách SVG mặt trời màn kết qu�? �? `components/SunnyMascotSvg.tsx` (ExamPracticeResultReport tái dùng, UI không đổi).
- Header `/app` (HomePage) hiện mascot + bubble lời thoại theo gi�?/streak (`getMascotLine`); CSS `home-sun-mascot*` trong homePage.css; mobile ẩn bubble. Verify: web typecheck PASS.

## 2026-07-15 �? Fix undefined questions khi m�? Import Listening
...
- Orb tím «FOCUS / 01» �? `/app/exam` �? mascot **mặt trời** (ban ngày) / **mặt trăng** (18h�?6h) tái dùng `LegacySunMascot` t�? landing.
...
- Fix: thêm `options.prune`; `Publish mục mới` dùng `{ prune: false }`, ch�? `Publish tất cả` mới prune.
- Khôi phục cloud KET practice 02�?44 t�? nguồn `Crawl/Import_KET_A2_Listening` (đã publish thành công 02�?44 qua các batch).
...
- Hard-refresh `/app/sentence-structure` đ�? thấy đ�? 365.
...
- Admin publish t�? lấy `exam-listening-whisper:{examId}:{partNumber}` t�? localStorage và đưa vào `parts` JSONB của `listening_exam_published`.
- User đọc đ�? published s�? thấy transcript cloud trong panel; không thay đổi câu hỏi/đáp án.
- Verify: web typecheck PASS.

## 2026-07-15 �? Xóa toolbar Sao chép trùng trong Exam

- Global `TextSelectionToolbar` không còn hiển th�? trong vùng `[data-exam-highlight-zone]`.
- Transcript/Exam ch�? còn một toolbar Reading/Exam với một nút `Sao chép`.
- Verify: web typecheck PASS.
...
- Ch�? đặt phía trên khi gần cuối viewport đ�? tránh b�? cắt.
- Verify: web typecheck PASS.

## 2026-07-15 �? Fix toolbar overlap khi chọn transcript

- `ReadingHighlightToolbar` t�? chuyển xuống dưới đoạn chọn nếu đoạn chọn gần mép trên viewport.
- Tránh toolbar Sao chép/Highlight đè lên dòng transcript.
- Verify: web typecheck PASS.

## 2026-07-15 �? Đồng b�? Highlight/Note transcript với Reading/Exam

- Transcript panel dùng trực tiếp `ReadingHighlightToolbar` và `ReadingHighlightableText`.
- Toolbar có Sao chép, Tô sáng, Note, B�? tô sáng và Xóa note giống Reading/Exam.
- Annotation lưu local theo `examId + Part`, dùng đúng `readingHighlightUtils` và màu `var(--exam-highlight-bg)`.
- Verify: web typecheck PASS.

## 2026-07-15 �? B�? Highlight/Note transcript

- Bấm trực tiếp vào đoạn highlight đ�? b�? highlight.
- Mỗi note có nút `B�? note`.
- Màu highlight dùng `var(--exam-highlight-bg)`, đồng b�? Reading/Exam.
- Verify: web typecheck PASS.

## 2026-07-15 �? Highlight/Note cho transcript có sẵn

- Transcript t�? `q.ttsText`/audioscript gi�? cũng chọn được đ�? Highlight hoặc Note, không ch�? transcript Whisper.
- Dùng chung lưu tr�? local theo đ�?/Part.
- Verify: web typecheck PASS.

## 2026-07-15 �? Note/Highlight transcript Whisper

- Transcript Whisper trong Listening h�? tr�? bôi chọn đoạn văn.
- `Highlight` tô vàng và lưu local theo `examId + Part`.
- `Note` hỏi nội dung ghi chú, lưu local và hiển th�? lại dưới transcript.
- Verify: web typecheck PASS.

## 2026-07-15 �? Áp dụng Whisper transcript cho IELTS Listening

- Mọi đ�? `examType: 'ielts'` không còn đọc transcript DeepSeek cũ trong localStorage.
- IELTS Listening dùng chung nút Whisper `base.en`, transcript lưu theo `examId + partNumber`.
- Câu hỏi và đáp án IELTS không b�? thay đổi.

## 2026-07-15 �? Xóa transcript DeepSeek sai khỏi KET A2 catalog

- `ListeningTranscriptSidePanel.tsx` t�? xóa transcript map cũ trong localStorage với mọi `catalog-listening-*`.
- KET A2 Test 1 không còn hiển th�? transcript DeepSeek cũ; ch�? dùng `ttsText` chuẩn hoặc transcript Whisper local mới.
...
- Gửi audio URL của Part hiện tại tới `POST /api/stt`, lưu transcript tại localStorage theo `examId + partNumber` và hiển th�? lại khi m�? panel.
- Transcript Whisper ch�? là văn bản tham khảo toàn Part; không dùng đ�? suy đoán câu hỏi/đáp án.
- Server typecheck PASS. Web typecheck còn lỗi catalog type có sẵn �? `listeningExamCatalogMerge.ts`/`packages/catalog/src/builtinExams.ts`.

## 2026-07-15 �? Mount local STT router

- `server/src/index.ts` import và mount `sttRouter` tại `/api/stt`.
- Trang `/` công b�? `ttsHealth`, `tts`, `sttHealth`, `stt`.
...
- Server STT mặc định dùng `faster-whisper` `base.en` thay cho `tiny.en` đ�? nhận dạng tốt hơn s�?, tên riêng và spelling trong Listening Cambridge A2–C2.
- Có th�? ghi đè bằng biến môi trường `WHISPER_MODEL`; không thay đổi việc tạo câu hỏi/đáp án.

## 2026-07-15 �? Khóa tạo đ�? KET A2 bằng AI

- `ImportReadingPdfModal.tsx`: không cho chạy DeepSeek/OpenAI khi import PDF Cambridge A2; nút Phân tích b�? khóa và hướng dẫn dùng ZIP chuẩn có `exam.json` + `answer-key`.
- Lý do: Whisper `tiny.en` ch�? nhận dạng lời nói thành transcript, không th�? dựng chính xác câu hỏi/đáp án/hình ảnh Cambridge; Test 1 KET A2 chuẩn đã có catalog.
- IELTS và các luồng AI khác vẫn gi�? nguyên.
- Cần verify: `pnpm --filter web exec tsc --noEmit`.

## Trạng thái hiện tại

- **Transcript AI lưu vĩnh viễn (2026-07-15):** `examListeningTranscriptStorage.ts` sessionStorage �? **localStorage** (migrate t�? động); panel transcript có nút «Tạo transcript bằng AI» (Sparkles) khi part thiếu transcript �? merge với map cũ, tạo 1 lần không gọi lại. `tsc` pass
- **Transcript split khi làm bài (2026-07-15):** `ListeningTranscriptSidePanel.tsx` mới �? nút «Transcript» trên header 4 test runner (KET/PET/FCE-CAE-CPE/IELTS) m�? panel fixed bên phải, kéo cạnh trái resize (280�?720px, lưu localStorage), Esc đóng; nguồn: `q.ttsText` (Audioscript import) + AI map sessionStorage, lọc theo part hiện tại; CSS `.listening-transcript-panel*` trong listeningTest.css. `tsc` pass
- **MS PET stack dọc (2026-07-15):** `.listening-pet-mc__question` b�? grid 2 cột (câu trái / options phải) �? flex column, options nằm dưới câu hỏi
- **Bìa sách Cambridge library (2026-07-15):** `getCambridgeBrandBookCoverColor` (cambridgeLibraryGrouping.ts) �? b�? `color-mix` với base brand (ra toàn nâu); dùng `BOOK_PALETTE` 14 màu đa dạng như IELTS, offset theo brand; Book 1 gi�? màu brand. `tsc` pass
- **Auto-play part audio (2026-07-15):** đ�? Listening import chia `part1..part5.mp3` �? bấm chuyển part trong `goToPart` t�? phát audio part đó (KET/PET/FCE-CAE-CPE: helper `autoPlayPartAudio`; IELTS: inline, key `part-{id}`). Ch�? khi không dùng 1 MP3 shared, không �? review/submitted, tôn trọng play limits (exam mode); gọi trong click gesture nên không vướng autoplay policy. `tsc --noEmit` pass
- **UI MS (2026-07-15):** redesign MS/MC listening trong `apps/web/src/features/exam/listeningTest.css` �? KET A2 (`.listening-ket-p3__*`), PET B1 (`.listening-pet-mc__*`), FCE/CAE/CPE B2–C2 (`.listening-fce-mc__*`, `.listening-fce__num`), KET Part 4/fallback (`.listening-ket-cambridge__question .listening-exam-option*` + `__qnum`): card double-bezel/gradient, badge s�? pill gradient, custom radio spring (tham chiếu `Crawl/Giaodien/not.jpg`); components không đổi
- **Branch:** `project_14726` / `feat/reading-part-picker` (git repo `D:/App-English-Ryan/Website`)
- **Phase:** Import batch **KET A2 Listening practice** (44 đ�?) �? **published 02�?44 cloud**
...
- **Dev:** `pnpm dev` �? hard refresh đ�? nạp `listening_exam_published`

### Bundle đ�? sẵn trong `Tainguyen/`
| K�? năng | Level | File | Trạng thái |
|---------|-------|------|------------|
...
| Listening | IELTS Cam 9�?20 | `IELTS/Listening/Listening IELTS_Test*_Cam*` (48 đ�?) | **Builtin** `catalog-listening-ielts-cam{X}-test{Y}` �? restored 2026-07-12 |
| Reading | IELTS Cam 9�?11 (một phần) | `IELTS/Reading IELTS_Test*_Cam*` | **Builtin 9 đ�?** có `exam.json` (Cam9 T1�?4, Cam10 T1�?4, Cam11 T3); 39 folder còn PDF/scaffold |
...
### Nguyên nhân “mất�?
- 2026-07-04/05: user request “Xóa sạch 48 đ�? mẫu�? �? disable `discoverIeltsListeningBundles`, empty `generatedIeltsListening.ts`, xóa 48 JSON catalog.
...
- [x] Discover Reading IELTS ch�? folder có `exam.json` �? **9 đ�?** + `generatedIeltsReading.ts`
...
- [x] **Auto-backup đ�?** (2026-07-12): Dexie `examBackups` v15, OPFS, auto-download JSON khi Lưu Wizard/Import; Settings toggle; full app backup v4 gồm examBackups
...
- [ ] User: hard refresh production đ�? catalog v24 nạp lại
...
�?   �?   ├── pages/
�?   �?   �?   ├── landing/LandingPage.tsx  �? Trang ch�? public + animated sun
...
- **Nguyên nhân:** (1) Vite dev serve `VocabularyPage.tsx` rỗng (Content-Length 0) �? `React.lazy` render `undefined`; (2) UI import tĩnh `vocabSeedDecks` + ~7MB JSON seed; (3) `DeckGrid` N× `useLiveQuery` (100+ deck) d�? treo main thread
...
- [x] Lazy VocabularyPage retry nếu module rỗng; tắt SW register �? DEV
- [x] `examVocabDecks` bắt race Dexie ConstraintError (StrictMode)
- [x] Verify Playwright mock-auth: có «B�? t�? vựng» + tabs; `tsc --noEmit` pass

### T�? điển offline Part2�?5 (2026-07-15)
...
### Vocab seed 100 t�? đơn + 100 cụm + IPA/example (2026-07-15)
- [x] singles + phrases (~31200 th�?)
- [x] `enrich-preset-vocab.mjs` �? IPA US/UK (CMUdict) + example gắn phrase; seed **v4**
- [x] `seedPresetCards` **patch** th�? cũ (IPA/example thiếu hoặc generic)
- [x] Hard refresh `/app/vocab` �? thấy IPA + ví d�? đầy đ�?

### Vocab thêm b�? preset rỗng (2026-07-15)
- [x] Lần 1�?3: +6/nhóm mỗi lần �? **26 b�?/nhóm** (156 preset)
- [x] M�? `/app/vocab` �? seed deck mới t�? tạo (idempotent)

### Vocab 2 cấu trúc: T�? đơn | Cụm t�? (2026-07-15) �? HOÀN THÀNH
- [x] `vocabUnitKind.ts` �? phân loại: có khoảng trắng / POS cụm �? phrase; còn lại single
- [x] Tab cấp 1 trên `/app/vocab`: **T�? vựng đơn l�?** | **Cụm t�? vựng**
- [x] Lọc s�? đếm deck, danh sách th�?, SRS/Quiz/Type/Nghe/Speak, stats/weak/review theo `unitKind`
- [x] `tsc --noEmit` pass

### Vocab preset seed 100 t�?/nhóm (2026-07-15) �? HOÀN THÀNH
- [x] `scripts/gen-preset-vocab-seed.mjs` �? sinh 600 th�? (IELTS/Oxford/TOEIC/Academic/SAT/TOEFL × 100)
...
- [x] Fix: `seedPresetDecks` trước đây return sớm nếu `admin_published_vocab_version > 0` �? không nạp th�?
- [x] Prune publish **gi�?** th�? `sourceLabel` `preset-seed*`; sau `mergeVocab` gọi lại `seedPresetCards`
- [x] `tsc --noEmit` pass
- [ ] User: hard refresh `/app/vocab` �? m�? b�? (vd. IELTS �? Môi trường) �? ~12�?13 t�?/b�?
...
- [x] `study/DeckStatBar.tsx` �? Total/Leared/Progress/T�? mới/Cần ôn/Đã vào lịch/Lần ôn k�? tiếp
- [x] `study/StudyModeTabs.tsx` �? 3 mode active + placeholder tabs (Listening, T�? yếu�?)
- [x] `study/useDeckStudyStats.ts` �? live stats t�? Dexie SRS
- [x] `study/speakPhrase.ts` �? Kokoro local TTS (lang `a` US); fallback Web Speech; SRS auto-phát khi lật v�? mặt trước
...
- [x] Danh sách t�? (`CardPanel`) �? cột **Trạng thái** (T�? mới / Cần ôn / Đã học) + badge **t�? loại** (Danh t�?, Động từ�?)
- [x] **Nghe & Gõ** (`ListenTypeMode`) �? mode mới theo `Giaodien/Nghe & Gõ lại t�? vựng.html`: TTS + tốc đ�?, gõ lại, gợi ý/b�? qua/xem đáp án, panel hướng dẫn + tiến đ�?
- [x] **SRS popup nhắc ôn** �? `SrsReviewReminderModal`: chim animated, đếm t�? due, chọn deck �? SRS; hiện khi vào `/app` (F5/login) + mỗi 30 phút
- [x] Fix popup sau F5: `useSrsReviewPopup` dùng `useLiveQuery` + re-check sau `syncState === 'done'`
- [x] Import t�? gán `pos` qua `posLabels.ts` (infer + chuẩn hóa noun/verb/adj �? tiếng Việt)
- [x] **Học lại** �? `StudyDoneActions` trên màn hình hoàn thành (SRS/Quiz/Type/Listen): nút Học lại + Quay lại/Xong
- [x] **SRS flip 3D** �? Space/click lật th�? với animation `rotateY` (`.vs-flip-scene` / `.vs-flip-inner.is-flipped`)
- [x] **T�? yếu** (`WeakWordsMode`) �? bảng t�? yếu (lapses/ease), nút Ôn SRS t�? yếu; `isWeakWord()` + `studyFilter: 'weak'`
- [x] **Ôn tập** (`ReviewHubMode`) �? hero đến gi�? ôn, stat pills, lịch ôn sắp tới, ch�? đ�? ôn nhanh
- [x] **Thống kê** (`StatsMode`) �? bento dashboard: hoạt động 14 ngày, phân b�? rating/mode/trạng thái, top t�? yếu
- [x] `StudyModeTabs` �? 7 tab active (không còn placeholder disabled); `CardPanel` thêm nút Ôn tập / T�? yếu / Thống kê
...
- [x] `AppendSentencesModal.tsx` �? thêm 1 câu hoặc dán văn bản (tách câu t�? động)
- [x] `lessonRepo.appendSentences()` �? append vào bài đã có
- [x] Seed `listening_seed_v2` �? b�? sung bài có cấu trúc Test/Part
- [x] Xóa/ẩn **Bài Cambridge (cũ)** �? `purgeLegacyCambridge()` xóa bài flat khỏi DB, UI ch�? hiện bài có `book/test/part`

### Listening �? Import & Phiên âm (MP3 �? text) �? HOÀN THÀNH
- [x] `packages/core/src/ai/transcribeAudio.ts` �? Whisper API (OpenAI `whisper-1` / Groq `whisper-large-v3`), dùng API key t�? Cài đặt AI
...
- [x] `ListeningLibraryPage` �? nút **Import & Phiên âm** m�? modal, sau tạo redirect vào bài mới
- [x] Cần API key **Groq** hoặc **OpenAI** (DeepSeek/Gemini chưa h�? tr�? STT)

### Copy text trong app �? HOÀN THÀNH
- [x] `lib/copyToClipboard.ts` �? Clipboard API + `execCommand` fallback
- [x] `components/TextSelectionToolbar.tsx` �? toolbar "Copy" khi bôi đen text (toàn app, tr�? input/textarea)
- [x] `components/CopyButton.tsx` �? nút copy tái s�? dụng
...
- [x] `SrsMode` �? cho phép chọn text khi đã lật th�?
- [x] `DictionaryModal` �? Copy toàn b�? kết qu�? tra t�?
...
- [x] `ListeningSidebarCards.tsx` �? **xóa badge `ListeningTtsStatusBadge` ("Kokoro local")** �? card "Luyện phát âm" (theo `Giaodien/7.jpg`)
...
- [x] `POST /api/tts` �? tr�? `audio/wav` thật, filesystem cache SHA-256 (text+voice+speed)
...
- [x] SQL migration chạy thành công (6 bảng + RLS + triggers)
  ...
  - Row Level Security: mỗi user ch�? thấy data của mình
  - Trigger `handle_new_user`: t�? tạo profile khi user đăng nhập lần đầu
...
- [x] Protected routes (`/app/*`) �? redirect v�? `/` nếu chưa login
- [x] AppShell sidebar với user info + logout + theme switcher
- [x] Dexie schema đầy đ�? (local-first)
...
- Sun animation: xoay tia (12s), nổi lên xuống (4s), chớp mắt, lắc smile, chat bubble "bạn c�? việc focus... đã có Ryan lo!"
...
- Footer: brand + liên h�? email + anchor links (#features, #pricing)
- Toàn b�? UI dùng CSS variables (không hardcode gray/indigo)

### Plan Management + Admin Page �? HOÀN THÀNH (session 2026-06-30)

#### Kiến trúc (Hướng B �? Supabase-based):
- Schema `profiles` đã có sẵn `plan` + `plan_expires_at` t�? migration 001
- Migration 002 ch�? cần thêm `is_admin` + admin RLS policies
...
- [x] `features/auth/usePlanSync.ts` �? hook đọc plan+is_admin t�? Supabase sau login �? lưu vào db.settings
- [x] `features/admin/AdminPage.tsx` �? trang quản lý user: stats 4 ô + search + bảng users + UpgradeModal
- [x] `App.tsx` �? route `/app/admin`
- [x] `AppShell.tsx` �? gọi `usePlanSync()` + nav item Admin (ch�? hiện nếu is_admin=true)

#### Notes quan trọng:
- `usePlanSync` chạy 1 lần sau login, lưu plan vào db.settings �? `canUse(plan, feature)` dùng ngay
- is_admin được set TH�? CÔNG trong Supabase SQL Editor: `UPDATE profiles SET is_admin=true WHERE email='...'`
...
- RLS helper function `is_current_user_admin()` dùng SECURITY DEFINER đ�? tránh recursion
...
- [x] `NewMindmapModal.tsx` �? nhập t�? trung tâm �? tạo root node
...
- Collapse: toggle node.collapsed �? layout b�? qua subtree đó
...
- [x] `DictionaryFAB.tsx` �? FAB c�? định bottom-right, detect text selection khi click
...
- [x] `AppShell.tsx` �? gắn FAB + Modal vào layout chính (available toàn b�? /app pages)
...
- API key/provider lấy t�? db.settings (dùng chung với Writing module)
- SaveToDeckModal: z-[60] (trên DictionaryModal z-50), inline create deck không cần modal con
- Collocations + Synonyms đều clickable �? tra tiếp t�? liên quan
- TTS: speechSynthesis.speak() inline (không import t�? listening module)
...
- [x] `packages/db` �? WritingDoc.type m�? rộng: 'ielts_task1'|'ielts_task2'|'master' (+ backward compat 'ielts')
...
- [x] `DocListPanel.tsx` �? danh sách bài, badge loại/t�? s�?
- [x] `NewDocModal.tsx` �? chọn Task 1/Task 2/Free + nhập đ�? bài
...
- [x] `WritingEditor.tsx` �? textarea với auto-save 1.5s debounce + word count bar + grade flow đầy đ�?
...
- Cache: cùng text �? tr�? ngay t�? writingHistory.textHash, không gọi AI lần 2
- Rate limit: Free/Basic=0, Trial=5/ngày, Pro=20/ngày, Lifetime=�?
- Plan default: 'pro' (t�? chọn trong AiSettings, dùng th�? ch�? không enforce)
- Tất c�? 4 providers dùng OpenAI-compatible API format
- response_format: { type: 'json_object' } �? AI buộc tr�? JSON
- Feedback viết bằng tiếng Việt (system prompt ch�? định)

### Module Listening �? HOÀN THÀNH + PORT T�? P15.8.302 (session 2026-06-30)
...
- [x] `ListeningPracticeTab.tsx` �? Ô ch�?/Cloze per-word inputs, audio progress bar, word diff live, cloze count +/�?
- [x] `ListeningShadowingTab.tsx` �? pitch contour canvas, Web Speech recognition, mic YIN capture
- [x] `practiceUtils.ts`, `BlankInputMode.tsx`, `WordDiffPanel.tsx` �? logic blank/cloze/diff port t�? reference
- [x] `useListeningPlayback.ts` �? TTS + progress bar ước lượng theo tốc đ�? đọc
...
- Cloze: `clozeCount=0` = ẩn tất c�? t�? eligible; +/- điều chỉnh s�? ô trống
- SRS sentences được lưu embedded trong `lesson.sentences[]` (không dùng bảng `srs`)
- Cambridge packs ch�? seed 1 lần (check `count > 0` trước)
...
- [x] `packages/db/src/index.ts` export đầy đ�? repos
- [x] `vocabStore` (Zustand) �? activeDeckId + studyMode state
- [x] `DeckPanel` �? danh sách deck (live query), tạo/xóa deck, badge "X ôn"
- [x] `CardPanel` �? bảng t�? (live query), thêm/sửa/xóa t�?, 3 nút học
- [x] `DeckEditorModal` �? tạo/sửa b�? th�?
- [x] `CardEditorModal` �? thêm/sửa t�? (phrase/meaning/example/IPA), "Thêm & tiếp"
- [x] `StudySession` �? overlay toàn màn hình, tab chuyển ch�? đ�?
...
#### Notes quan trọng:
- `cardRepo.add()` t�? init SRS state (dueAt = now �? all new cards immediately due)
...
- [x] `apps/web/vercel.json` �? SPA rewrite: tất c�? route (tr�? `/assets/*`) �? `index.html`
- [x] `apps/web/vite.config.ts` �? `base: '/'`, `build.outDir: 'dist'` (explicit)
- [x] `apps/web/public/_redirects` �? Netlify fallback: `/* /index.html 200`
- [x] `apps/web/.env.example` �? template env vars (không có giá tr�? thật)
- [x] `pnpm --filter web build` �? sạch (tsc + vite build)

#### Vercel setup notes:
- **Deployed:** https://ryanenglishv2.vercel.app/
- **Root Directory: Đ�? TRỐNG** (repo root `Website/`) �? KHÔNG đặt `apps/web`
  - Monorepo cần `packages/*` + `pnpm-workspace.yaml` �? root đ�? build
...
- **QUAN TRỌNG:** `cleanUrls: false` �? `cleanUrls: true` làm rewrite `/index.html` b�? ignore �? 404 SPA
...
- OAuth redirect v�? `/` (không dùng `/auth/callback`) �? tránh 404 Vercel static
- `AuthContext` x�? lý `?code=` PKCE ngay khi app load
...
- **Commit `vercel.json` �? repo root vào Git** nếu deploy qua GitHub
...
- [ ] **KET A2 Listening practice test-02�?44** �? ch�? user đ�? media (~1h) �? `node scripts/ket-practice-csv-to-exam.mjs 2-44` (hoặc `all`)
- [ ] **Listening Ô CH�? trên mobile iOS** �? user từng báo ~75% fix; **ch�? user hard refresh production và xác nhận**
- [x] **Listening thanh cuộn ngang/dọc thừa** (dưới tabs) �? fix layout shell + ẩn scrollbar
- [ ] **Web Audio trên iOS** �? chime/buzz/pháo hoa có th�? cần gesture trước (autoplay policy)
...
- [x] `features/settings/useAiSettings.ts` �? hook load/save/test AI settings (tách t�? AiSettingsModal)
...
- [x] `App.tsx` �? React.lazy() + Suspense cho tất c�? route pages
- [x] `components/PageFallback.tsx` �? spinner loading khi lazy load chunk
- [x] Build production thành công �? main chunk ~384KB (t�? 588KB), pages tách riêng (Vocab 28KB, Writing 22KB...)

#### Notes quan trọng Settings:
- Theme: 3 ch�? đ�? light/mid/dark, lưu localStorage `ryan-theme`, preview card mini UI
- AI tab: tái dùng logic AiSettingsModal, thêm nút "Kiểm tra kết nối" (callAI test JSON)
- Account tab: plan t�? db.settings (sync Supabase qua usePlanSync), hiển th�? features enabled qua canUse()
- Contact: email ryanik1997@gmail.com + hướng dẫn nâng cấp qua admin
- Plan selector trong AI settings đã b�? (plan gi�? sync t�? Supabase, không override th�? công)
...
- [x] `SettingsPage` tab Giao diện �? section "Nhắc nh�? ôn t�? hàng ngày"
- [x] `App.tsx` �? đăng ký SW on load; `AppShell` �? `useNotifications()` interval
- Best-effort: ch�? chạy khi tab m�?, không cần VAPID/server

### AppShell Sidebar Polish �? HOÀN THÀNH (session 2026-06-30)
- [x] Logo mark gradient R + tagline IELTS · AI · SRS
- [x] Nav badges ôn t�? (vocab SRS + listening dueAt) real-time
...
- [x] `PaymentModal` �? nút "Thông báo đã chuyển" (ch�? khi đã login)
...
```
Secrets: `RESEND_API_KEY`, `ADMIN_EMAIL` (tu�? chọn `APP_ORIGIN`)
Chạy `004_payment_requests.sql` trong Supabase SQL Editor trước khi test.

### Supabase Cloud Sync �? HOÀN THÀNH (session 2026-06-30)
- [x] `sync.ts` m�? rộng �? writingDocs + mindmaps push/pull, `isLocalEmpty()`, error handling
...
- [x] `SyncOnLogin.tsx` �? toast khôi phục / đồng b�? xong
- [x] `AppShell` �? sync indicator sidebar; Settings tab Tài khoản �? "Đồng b�? đám mây"
- [x] `pnpm --filter web exec tsc --noEmit` �? pass

#### Notes quan trọng Cloud Sync:
- Thiết b�? mới (local trống) �? `syncCloudToLocal`; có data �? `syncLocalToCloud`
...
- [x] `settingsRepo.ts` �? getSetting / putSetting, export t�? `@ryan/db`
- [x] `useDailyGoal.ts` �? goal words/translations, progress t�? reviewLog hôm nay
...
### Backup/Restore toàn b�? data �? HOÀN THÀNH (session 2026-06-30)
...
- [x] `SettingsPage` tab Tài khoản �? section "D�? liệu & Backup" (xuất/nhập JSON)
...
- Fuzzy compare: normalize lowercase, b�? dấu câu, match t�? (cho phép lệch 1 ký t�?)
- Highlight: đúng = `--color-primary`, thiếu = `--color-accent`, thừa = `--text-muted`
- SRS đơn giản: D�? +3 ngày, Ổn +1 ngày, Khó ôn lại ngay
- Seed check `translationRepo.count() > 0` trước khi insert mẫu

### Import/Export t�? vựng �? HOÀN THÀNH (session 2026-06-30)
- [x] `features/vocab/importExport.ts` �? export CSV/JSON (RFC 4180), parse CSV/JSON (version 1), template CSV
- [x] `features/vocab/ImportModal.tsx` �? drop zone, preview 5 dòng, import qua `cardRepo.add()`, cảnh báo >500 t�?
- [x] `CardPanel.tsx` �? nút **Xuất** (dropdown CSV/JSON, disabled khi rỗng) + **Nhập** (m�? ImportModal)
- [x] `pnpm --filter web exec tsc --noEmit` �? pass
...
- Import ch�? thêm vào deck hiện tại (không tạo deck mới t�? JSON)
...
- [x] Đồng nhất left panel width �? `w-60` (240px) �? LessonPanel + TranslationListPanel t�? `w-64` �? `w-60`
- [x] `pnpm --filter web exec tsc --noEmit` �? pass

#### Notes quan trọng Panel unify:
- Header: `px-4 py-3.5`, `text-sm` title, `text-xs` subtitle, CSS variables only
- CardPanel header dùng PanelHeader nhưng gi�? nguyên study/export/import/thêm t�? trong `actions`
- Right panel detail headers (LessonDetail, TranslationDetail...) chưa đổi �? ch�? left list panels + CardPanel

### HomePage Dashboard Polish �? HOÀN THÀNH (session 2026-06-30)
- [x] Dynamic greeting/subtitle theo gi�? + streak
...
- [x] `VocabularyPage.tsx` �? khi chưa có deck: welcome + nút "Tạo b�? th�? ngay" �? DeckEditorModal
- [x] `WritingPage.tsx` �? khi chưa có bài: welcome + "Tạo bài viết đầu tiên" �? NewDocModal; link Settings > AI
- [x] `features/home/useHomeStats.ts` �? stats t�? Dexie: wordsStudied, docCount, streak (reviewLog), onboarding 5 bước
- [x] `HomePage.tsx` �? dashboard: 3 stat cards + 4 quick actions + checklist onboarding (hiện khi < 5/5)
- [x] Build production thành công (tsc + vite build sạch)

#### Notes quan trọng Onboarding:
- Empty state ch�? render khi `useLiveQuery` tr�? v�? mảng rỗng (không flash loading)
- Streak tính t�? `reviewLog` �? ngày liên tiếp có ôn tập SRS
- Onboarding 5 bước: tạo deck �? 10 t�? �? SRS review �? writing doc �? API key
- Writing empty state ẩn DocListPanel; Vocabulary empty state ẩn DeckPanel/CardPanel
- Tất c�? UI dùng CSS variables (`--bg-card`, `--color-primary`...)
...
- [x] `WritingEditor.tsx` �? 2 cột: đ�? bài (trái) + editor (phải), timer, action bar, AI feedback drawer
- [x] `WritingTopicPanel.tsx` �? badge, ảnh đ�?, prompt glass box, hướng dẫn
- [x] Import JPG/WEBP �? `writingImage.ts` (nén, validate), modal tạo bài + panel đ�?
...
- [x] `ScorePanel.tsx` �? hiển th�? điểm Cambridge (0�?5 + level) hoặc band IELTS tùy loại bài
...
- [x] Fix tsc: `WritingEditor` framework merge; `useWritingDashboard` `getCriterionBand` cho c�? 2 framework
- [x] `pnpm --filter web exec tsc --noEmit` �? pass
- [x] Fix Dashboard thống kê trống: layout `AppShell`/`WritingLayout` + `wd-page` height; empty state có UI đầy đ�?; `parseScore` legacy; errorBank sort in-memory
...
- [x] **Cấu trúc câu** �? sidebar `/app/sentence-structure`: danh sách + điền A/B, lật th�?, kiểm tra; seed 6 mẫu; Dexie v7 `sentenceStructures`
...
- [x] IELTS hub 3 bước: `/practice` (Task 1/2/Free) �? `/practice/:track` (line graph, opinion�?) �? `/practice/:track/:genre` (editor); `ieltsCatalog.ts` + `WritingGenre` m�? rộng
...
- [x] **Translation tracks mới:** Cấu trúc cơ bản (8 grammar), Collocations & Vocab (15 ch�? đ�?), Dịch đoạn Band 6.5 (15), Band 8.0 (10), Essay hoàn chỉnh (15), Của tôi

### Translation �? label Đã dịch / Chưa dịch + bấm câu đ�? dịch (session 2026-06-30)
...
- [x] `SentenceRow` �? badge cạnh D�?/TB/Khó: **Đã dịch** (primary) / **Chưa dịch** (muted)
- [x] `applyPracticeRating` gi�? `translatedAt` khi rate SRS
- [x] Bấm bất k�? câu nào trong danh sách �? `startPracticeSentence(id)` m�? luyện dịch 1 câu
- [x] Nút **Luyện tập** vẫn chạy phiên đầy đ�? (ưu tiên câu cần ôn)
- [x] **Practice UI redesign** (`translationPractice.css`) �? badge VN→English, câu VI lớn, chip ẩn t�?
- [x] `translationChips.ts` �? mỗi t�? trong đáp án EN = 1 chip; m�? theo th�? t�? khi gõ đúng t�? (live, k�? c�? t�? lặp)
- [x] Chip khóa hiện `●●●` / `N từ`; m�? xanh + nút nghe TTS; **Hiện tất c�?** / **Ẩn gợi ý**
- [x] Smart Segment panel sau kiểm tra; Enter đ�? nộp; Chấm AI riêng (placeholder)
- [x] `pnpm --filter web exec tsc --noEmit` �? pass

### Session 2026-06-30 �? 2026-07-01 �? Listening Ô CH�? + phản hồi âm thanh/pháo hoa

#### Listening Ô CH�? �? fix mất ch�? khi gõ
- [x] `BlankInputMode.tsx` �? input DOM thuần (port P15.8.302), `forwardRef` + `collectAnswer()` đọc DOM
- [x] `ListeningAudioBar.tsx` �? tách progress TTS, tránh re-render 60fps làm mất ký t�?
- [x] `practiceUtils.collectBlankAnswer` (boxes) �? `join(' ')` trực tiếp, b�? placeholder `…`
- [x] `globals.css` �? width px động; `readOnly` khi khóa (không `disabled`)
- [x] WordDiff live: 1 panel trong `BlankInputMode` khi bật "Hiện kết qu�? ngay"
...
- [x] Đúng �? chime + pháo hoa; Sai / B�? qua / Không biết �? buzz
- [x] SRS **không** có (ch�? đ�? rating 1�?4, không đúng/sai nh�? phân)

#### SRS �? Space phát audio
- [x] `SrsMode.doFlip` �? mỗi lần Space/lật th�? đều `speakPhrase(card.phrase)` (trước ch�? phát khi quay v�? mặt trước)
...
- [ ] Listening Ô CH�? trên **mobile iOS** �? hard refresh production, test gõ "do" và câu dài
...
- [x] `ListeningLessonPage` �? b�? debug MutationObserver/inline style; shell 2 lớp (hidden + scroll)
...
### Luyện thi �? ch�? Reading + Listening (session 2026-07-01) �? HOÀN THÀNH
- [x] IELTS + Cambridge track: b�? Writing khỏi hub (Writing �? module Viết); b�? nút Full Mock IELTS trên `ExamTrackPage`
- [x] Cambridge: Reading thay Writing; import PDF gắn `cambridgeLevel`

### IELTS �? đ�? mẫu Listening 4×10 (session 2026-07-01) �? HOÀN THÀNH
...
- [x] Hiện trên `/app/exam/track/ielts`; `bandHint` hiển th�? cấu trúc đ�?

### Cambridge �? đ�? mẫu theo format thi thật A2–C2 (session 2026-07-01) �? HOÀN THÀNH
...
- [x] `cambridgeReadingSamples.ts` �? Reading đúng s�? Part (A2:5, B1:6, B2:7, C1:8, C2:7); bandHint `Sample X/Y câu`
- [x] `cambridgeListeningSamples.ts` �? Listening đúng s�? Part (A2:5×25, B1:4×25, B2–C2:4×30 câu)
- [x] `cambridgeExamLevels.ts` �? mô t�? format thi trên hub; ExamTrackPage hiện `bandHint`

### Cambridge A2–C2 �? Luyện thi (session 2026-07-01) �? HOÀN THÀNH
- [x] `cambridgeExamLevels.ts` �? 5 cấp A2 (KET) �? C2 (CPE), skills Reading + Listening (Writing �? module Viết)
- [x] `examTracks.ts` �? hub 2 track: IELTS + Cambridge A2–C2 (b�? KET riêng, redirect `/track/ket` �? `/track/cambridge/a2`)
- [x] `ExamTrackPage` �? `/app/exam/track/cambridge` chọn level; `/cambridge/:level` đ�? + import + link Writing
- [x] `ListeningExamType` m�? rộng: `pet` | `fce` | `cae` | `cpe` (B1–C2 dùng UI multi-part như IELTS)
...
- [x] `ExamTrackPage` �? `/app/exam/track/:trackId/:level?`, Full Mock + import + danh sách đ�?
...
- [x] Reading/Listening nộp trong Full Mock hiện `FullMockStageResult` + chuyển k�? năng
- [x] `WritingMockTest` �? Task 1 + Task 2 (đếm t�?, timer, chưa chấm AI)
...
- [x] `ImportListeningModal` �? JSON + MP3/ảnh (q1.mp3, q1-a.jpg, part1.mp3), template tải v�?
...
- [x] `ListeningKetTest` �? tách t�? shell Phase 1, h�? tr�? `examMode`
- [x] Đ�? builtin `ielts-listening-sample-01` (Part 1�?2)
- [x] Backup v3 gồm `listeningExams`
- [x] `ExamHome` �? Import Listening + xóa đ�? import
- [x] `pnpm --filter web exec tsc --noEmit` �? pass

### Listening KET Phase 1 �? card UI (session 2026-07-01) �? HOÀN THÀNH
- [x] `listeningExamData.ts` �? schema + đ�? mẫu `ket-listening-sample-01` (6 câu, `audioKey`/`audioUrl`/`imageUrl` optional)
- [x] `useExamQuestionAudio.ts` �? MP3 t�? `audioRepo` hoặc URL, fallback TTS `ttsText`
...
- [x] `readingHighlightUtils.ts` �? offset-based highlights theo `blockId`, merge/tr�? range, parse Selection
- [x] `ReadingHighlightableText.tsx` �? render `<mark>` cho từng khối text
- [x] `ReadingHighlightToolbar.tsx` �? toolbar chung Highlight / B�? highlight / Copy (passage + câu hỏi)
- [x] `ReadingPassagePanel.tsx` + `ReadingQuestionPanel.tsx` �? highlight passage và panel câu hỏi/đáp án
- [x] `ReadingTest.tsx` �? `highlightsByPart` ch�? trong session (không lưu localStorage); mất khi thoát/F5
- [x] `readingTest.css` �? `.reading-test-highlight` theme-aware; `user-select: text` passage + questions
- [x] `TextSelectionToolbar` �? b�? qua `[data-reading-highlight-zone]` (tránh toolbar trùng)
- [x] `pnpm --filter web exec tsc --noEmit` �? pass
...
- [x] `parseReadingPdfFull()` �? th�? full trước, fallback parse từng part + progress UI
...
2. Upload PDF (�?12MB) �? text layer hoặc scan (Vision OCR t�? động)
3. API key Cài đặt �? AI (OpenAI/Gemini khuyến ngh�? full test + scan)
4. Parse Part 1�?3 �? score tin cậy + preview (lọc câu đáp án đoán) �? **Lưu & làm bài**
5. Backup v2 gồm `readingExams`

#### Giới hạn còn lại
- Vision OCR tốn token/chậm (~20 trang max); chất lượng ph�? thuộc ảnh scan
- Groq không Vision �? scan cần đổi OpenAI/Gemini
- Chưa sync `readingExams` lên Supabase cloud
- Chưa có UI sửa đ�? import sau khi lưu
...
- [x] `parseReadingPdfFull(..., { format: 'ket-a2' })` �? t�? chọn parser KET khi import t�? Cambridge A2
...
- [x] `ParsedReadingPart.partNumber` �? `number` (h�? tr�? part 4�?5)

### Fix KET split �? cắt nhầm Part 5 (session 2026-07-01)
- [x] `splitPdfTextForKetParts` �? b�? regex `writing\s+part` (khớp nhầm footer "Reading and Writing PART 5" �? mất Part 5)
- [x] Ch�? cắt PDF khi có Writing Part 6+ (`part 7`�?)
...
- [x] `parseReadingPdfFull(..., { format: 'pet-b1' })` �? t�? chọn khi import t�? Cambridge B1
...
### G�? Import PDF/OCR Reading (session 2026-07-01)
- [x] B�? nút **Import PDF Reading** khỏi `ExamTrackPage` �? ch�? còn **Import th�? công**
- [x] JSON mẫu KET A2: 5 parts · 30 câu + hướng dẫn trong modal
- [x] Hướng dẫn import trong modal Reading + Listening

### Import PDF OCR �? gi�? ảnh trang/passage (session 2026-07-01) �? ĐÃ G�? KHỎI UI
- [x] `pdfContent.ts` �? `ExtractPdfResult.pages[]` (text + `dataUrl`); `preservePageImages` cho KET/PET
- [x] `pdfVision.ts` �? OCR **từng trang** (detail high), gi�? ảnh gốc �? không còn plain-text-only batch
...
- Part 2�?5: fallback `passage: [{ imageKey }]` khi passage < 80 ký t�?

### Import th�? công Reading + Listening (session 2026-07-01) �? HOÀN THÀNH
- [x] `importReadingManualUtils.ts` �? parse/validate JSON đ�? Reading, lưu ảnh đoạn văn vào `audioRepo`
...
- [x] `ReadingPassageBlock.imageKey` + `ReadingPassagePanel` hiển th�? ảnh đoạn văn
- [x] `ExamTrackPage` �? nút **Import th�? công Reading** (JSON + ảnh) + **Import th�? công Listening** (đã có JSON/ZIP)
- [x] Xóa đ�? import: `reading-pdf-*` + `reading-manual-*`
- [x] `pnpm --filter web exec tsc --noEmit` �? pass

#### Flow Import th�? công Reading
1. Luyện thi �? track IELTS/Cambridge �? **Import th�? công Reading**
2. Tải JSON mẫu �? điền passage (text + `imageFile`) + questionGroups
3. Upload ảnh (`part1-p1.jpg`�?) hoặc ZIP bundle
4. Preview �? **Lưu & làm bài** �? ảnh hiển th�? trong panel passage trái

#### Import Reading Cambridge A2–C2 �? template + prompt chuẩn (session 2026-07-01)
- [x] `cambridgeReadingImportTemplates.ts` �? LEVEL_PARTS đ�? A2–C2; A2 Part 4 = MC chọn t�? (không gap-fill)
- [x] JSON mẫu tải trong modal theo level �? đ�? s�? câu mỗi part (không ch�? 2 câu mẫu)
...
- [x] Claude tr�? folder `C:\Users\ADMIN\Downloads\ket-reading-test1` �? 5 parts · 30 câu · 6 ảnh Part 1
...
- [x] `ReadingPassagePanel` �? B1 Part 2/4: ẩn block `Danh sách A–H` cột trái (passage đã có đ�? nội dung)
- [x] `ReadingQuestionPanel` �? B1 matching: ẩn `List of features/sentences` cột phải (ch�? câu hỏi + pills A–H)
- [x] `scripts/build-pet-b1-test1.py` �? Part 2: passage `label` A–H + `features[]` tên ngắn; Part 4: câu A–H trong passage + `features[]` ch�? text câu (không prefix ch�? cái)
...
- [ ] User **xóa đ�? cũ** + import lại `pet-reading-test1.zip` �? xác nhận Part 2/4 không lặp (a8/a9)

#### Flow Import th�? công Listening (đã có)
1. **Import th�? công Listening** �? JSON/ZIP + MP3/ảnh câu hỏi
2. Tên file: `q1.mp3`, `q1-a.jpg`, `part1.mp3`�?

### Fix Import PDF kẹt / timeout "Đọc PDF quá lâu" (session 2026-07-01)
- [x] `pdfExtract.ts` �? worker c�? định `/pdf.worker.min.mjs`; không `await task.destroy()` (fire-and-forget 2s �? tránh treo 45s)
- [x] Timeout theo bước: m�? PDF 30s, mỗi trang 20s, tổng 120s; `useWorkerFetch: false`
- [x] `vite.config.ts` �? plugin copy `pdf.worker.min.mjs` �? `public/`; `optimizeDeps.exclude: pdfjs-dist`
- [x] Tiến trình: tải pdf.js �? m�? file �? trang X/Y; `preloadPdfJs()` khi m�? modal
...
- [x] Tăng `EDGE_PAD`/`LINE_GAP`, `strokeLinecap: butt` �? tránh nét tròn xuyên vào ch�?
...
Migrations chạy **một lần cho c�? h�? thống** �? không per-user.
...
- [ ] (Tu�? chọn) Extract ảnh Part 1 t�? PDF �? `q1-a.jpg` �? `q5-c.jpg`

#### Flow Import Listening KET A2
1. Luyện thi �? Cambridge �? A2 �? **Import th�? công Listening**
2. Upload `ket-listening-test1.zip` (exam.json + listening.mp3)
3. Preview �? Lưu & làm bài �? UI KET 1 câu/màn, h�? tr�? picture-mc / gap-fill / MC / matching
...
- [x] Builtin đ�?: KET/PET/FCE Reading + KET Listening (ID `catalog-*`) �? mọi user sau deploy
- [x] `GlobalCatalogSync` trong `AppShell` �? upsert Cấu trúc câu catalog (ID c�? định `catalog:ss:*`)
...
2. `pnpm build:catalog` (đ�? thi) + bump `GLOBAL_CATALOG_VERSION` (Dexie seeds)
3. `pnpm deploy:prod`
...
- [x] Import + `build-catalog` + legacy `q1-a.jpg` vẫn h�? tr�?
- [x] `exam.json` KET + HDSD cập nhật

### KET Listening �? Part 2 gap-fill + Part 5 drag-drop (session 2026-07-02) �? HOÀN THÀNH
- [x] `ListeningKetGapFillPartView` �? Part 2 (câu 6�?10): đ�? + audio trái, tất c�? ô điền ch�? trống gộp cột phải
- [x] `ListeningKetMatchingPartView` �? Part 5 (câu 21�?25): tên + ô vuông kéo th�? trái (theo `Giaodien/a1.jpg`), bank A–H phải
- [x] Kéo th�? hoặc chọn đáp án �? bấm ô; mỗi ch�? cái dùng một lần; nút × xóa ô
...
- [x] `ListeningKetTest` / `ListeningIeltsTest` �? b�? `stopPlayback()` khi `partIndex` đổi (KET + IELTS + Cambridge PET/FCE/CAE/CPE)
- [x] `ListeningQuestionCard` �? b�? dừng audio khi đổi câu
- [x] KET timer c�? định **25 phút** (`KET_LISTENING_DURATION_MINUTES`)

### Listening KET �? fix audio không phát (session 2026-07-02) �? HOÀN THÀNH
- [x] `useExamQuestionAudio` �? th�? lần lượt blob Dexie �? `audioUrl` catalog; blob rỗng/hỏng không chặn fallback
- [x] `listeningExamCatalogMerge` �? luôn b�? sung `audioUrl` t�? builtin khi import thiếu
- [x] `listeningExamLoader` �? mọi đ�? `source: import` đều merge media catalog (match title/examType)
...
- [x] `ExamHeaderBack` + `examNavigation.ts` �? nút **Quay lại** khi đang làm bài (mọi ch�? đ�?)
- [x] `KET_READING_DURATION_MINUTES = 30` �? Reading A2 c�? định 30 phút
...
- [x] FCE Part 3 matching (a9.jpg) �? `ListeningLetterMatchingPartView`: A–H bên trái, kéo/th�? ch�? cái vào ô Speaker 19�?23
...
- [x] **Fix Cam 9 Test 1 nội dung đúng** (JOB INQUIRY / SPORTS WORLD / Spiros–Hiroko / Whales) �? trước đó nhầm đ�? khác
- [x] `ListeningIeltsPartView` �? UI một cột theo `Giaodien/a1–a4`: note-completion inline (`gapLead`/`gapTrail`), MC dọc, Choose TWO
- [x] ZIP import: `pnpm pack:listening:ielts-cam9` �? **flat** `exam.json` + `listening.mp3` (không thư mục con/PDF); `importListeningZip` b�? qua PDF
- [x] Catalog: `catalog-listening-ielts-cam9-test1` + `catalog-listening-ielts-cam20-test1`
- [x] `isListeningAnswerCorrect` �? đáp án thay th�? `A/E` (Choose TWO) + gap-fill `/` variants
...
- [x] **Fix Cam 20 đáp án thiếu** (`Giaodien/a5–a10`): `notePassage` P1/P4 đầy đ�? static + `gapLead`/`gapTrail`; Choose TWO P2–P3 `choose_two()` nhãn đầy đ�? (không còn "A A"); rebuild catalog + ZIP
...
- [x] **Cam20 P1 table layout** �? `notePassageLayout: table` + `noteTable` (4 cột như đ�? giấy); `ListeningIeltsNoteTable.tsx`; so `Giaodien/a2` giống `a1` (bảng có viền, ô trống inline)
...
- [x] **Cam9 Test 2 Part 2** �? `exam_part2.json` (a6: bảng Parks + MC Longfield + map Hinchingbrooke); `map.jpg` t�? PDF; đáp án Key: trees, friday, farm, C, B, A, A, I, F, E
...
- [x] Cam9 Test 2 completely removed from builtin samples (per user: "xóa sạch đ�? mẫu"). User will import manually from Tainguyen/IELTS/Listening IELTS_Test2_Cam9/ (P1 = ACCOMMODATION FORM �? STUDENT INFORMATION). Updated generatedIeltsListening.ts, manifest.json, removed forces in loader.
...
- [x] Part 4 dual-task (a10.jpg) �? `ListeningDualLetterMatchingPartView`: TASK ONE 21�?25 + TASK TWO 26�?30, hai bảng A–H riêng, kéo/th�? vào Speaker 1�?5
...
- [x] Áp dụng toàn b�? Listening: `ListeningKetTest`, `ListeningPetTest`, `ListeningIeltsTest` (IELTS + FCE/CAE/CPE)
- [x] Vùng tô sáng: hướng dẫn, đ�? bài, gap-fill notes, MC/matching options; audio/ô nhập có `data-highlight-skip`
- [x] Highlight lưu theo Part; reset khi **Làm lại**
- [x] Theme **light**: tô sáng exam (`--exam-highlight-bg`) màu vàng `#fff3a3`; mid/dark gi�? accent tím

### Luyện thi �? Đã làm + Làm lại trên danh sách đ�? (session 2026-07-02) �? HOÀN THÀNH
...
- [x] `ExamTrackPage` �? badge **Đã làm**, meta `Đúng X/Y câu`, nút **Xem kết qu�?** + **Làm lại** từng đ�? Reading/Listening
- [x] `ExamResult` / `ListeningExamResult` �? **V�? luyện thi** + **Làm lại** cạnh nhau; back v�? đúng track Cambridge/IELTS (`examNavigation.ts`)
- [x] `ListeningKetGapFillPartView` �? ghi chú Part 2 có s�? câu trước ch�? trống (vd. `Send a letter and:(10) …`)

### Luyện thi �? Footer thống nhất + Reset timer (session 2026-07-02) �? HOÀN THÀNH
- [x] `ExamPartFooter.tsx` �? thanh ngang Part + pills s�? câu + Prev/Next câu + Submit (dùng chung Reading/Listening)
- [x] `ExamTimerControls.tsx` + `examTimer.ts` �? đồng h�? + nút reset (RotateCcw) cạnh timer
- [x] `ListeningKetTest.tsx` �? refactor `partIndex` + `activeQuestionId` (migrate draft `questionIndex` cũ); footer giống Reading
- [x] `ListeningIeltsTest.tsx` �? footer dùng `ExamPartFooter`; nav prev/next **câu** (không ch�? Part)
...
## K�? hoạch ngày mai �? Import ~100 đ�? Listening IELTS

**Mục tiêu:** User đưa PDF + Answer Key + MP3 hàng loạt; agent h�? tr�? tạo JSON, validate, gộp đ�?, đưa vào app.
...
Tainguyen/IELTS/Listening IELTS_Test{N}_Cam{9|10|…|20}/
  ...
  Answer key.pdf         (tu�? chọn)
```

### Workflow mỗi đ�? (lặp ×100)
...
| 4 | Import th�? (1 đ�?) | ZIP hoặc builtin catalog |
...
pnpm ielts:validate "IELTS/Listening IELTS_Test2_Cam9" --partial   # dev part l�?
```

### Pilot thành công (mẫu copy)

| Đ�? | ID catalog | Parts | Ghi chú |
|----|------------|-------|---------|
...
- [ ] Batch `build-catalog.mjs`: generate BUNDLES[] t�? manifest hoặc glob (tránh sửa tay 100 dòng)
...
- [x] `build-catalog.mjs` �? auto-discover 25 đ�? IELTS (Cam9�?14 + Cam20 T1) t�? `Tainguyen/IELTS/`
...
- [ ] User: hard refresh (`Ctrl+Shift+R`) hoặc xóa đ�? import cũ �? dùng đ�? builtin; `pnpm deploy:prod` cho production

### IELTS Listening Cam15�?16 batch (session 2026-07-03) �? HOÀN THÀNH
- [x] `scripts/build-ielts-cam15-16-listening.py` �? 8 đ�? × 4 parts × 40 câu
- [x] Cam15 T3 P1 �? PDF đ�? Part 1 (Employment Agency: Possible Jobs); layout form theo đ�? giấy (First Job / Second Job, bullets từng dòng)
...
- [x] `pnpm build:catalog` �? **33 đ�?** IELTS auto-discovered (Cam9�?16 + Cam20 T1)
- [x] `GLOBAL_CATALOG_VERSION` bump **2 �? 3** (sau đó **3 �? 4** khi fix Cam15 T3 P1 t�? PDF gốc)
- [x] `pnpm --filter web exec tsc --noEmit` pass

### IELTS Listening Cam17�?18 batch (session 2026-07-03) �? HOÀN THÀNH
- [x] `scripts/dump-ielts-cam17-18.py` + `ielts-cam17-18-dump.txt`
- [x] `scripts/build-ielts-cam17-18-listening.py` �? 8 đ�? × 4 parts × 40 câu
- [x] Cam18 T2 P1 �? PDF đ�? Q1�?5 (Milo's Restaurants); layout dual noteTables theo đ�? giấy
- [x] Cam18 T2 map �? `map.jpg` t�? PDF page 3 (School, Sports centre, Clinic�?)
- [x] `pnpm ielts:validate` �? 8/8 pass
- [x] `pnpm ielts:pack` �? 8 ZIP
- [x] `pnpm build:catalog` �? **41 đ�?** IELTS (Cam9�?18 + Cam20 T1)
- [x] `GLOBAL_CATALOG_VERSION` bump **4 �? 5** (sau đó **5 �? 6** khi fix Cam18 T2 P1 t�? PDF gốc)

### IELTS Listening Cam19�?20 batch (session 2026-07-03) �? HOÀN THÀNH
- [x] `scripts/dump-ielts-cam19-20.py` + `ielts-cam19-20-dump.txt`
- [x] `scripts/build-ielts-cam19-20-listening.py` �? 8 đ�? × 4 parts × 40 câu (Cam19 T1–T4 + Cam20 T1–T4)
...
- [x] `pnpm ielts:pack` �? 8 ZIP
- [x] `pnpm build:catalog` �? **48 đ�?** IELTS (Cam9�?20)
- [x] `GLOBAL_CATALOG_VERSION` bump **6 �? 7**

### IELTS Part 1 layout fix �? 48 đ�? (session 2026-07-03) �? HOÀN THÀNH
...
- [x] Cam9 T1 bundle (`exam_part1.json`), Cam12 v2 bullets, Cam20 T1 b�? `notePassage` trùng bảng
- [x] Rebuild 48 đ�? + `pnpm build:catalog` �? `GLOBAL_CATALOG_VERSION` **8 �? 9**

### IELTS Part 4 layout fix �? 48 đ�? (session 2026-07-03) �? HOÀN THÀNH
- [x] `normalize_part4()` trong `build-ielts-cam11-12-listening.py` �? auto `notePassageLayout: lecture`, `passageTitle`, b�? section trùng title, bullets `•`/`–`
- [x] `scripts/fix-all-ielts-p1.py` m�? rộng x�? lý c�? `exam_part4.json` (48 đ�?)
...
- [x] `groupNotePassageFormLines()` �? mỗi `static` JSON = một dòng; ch�? gom `static+gap(+trail)`; gap trail �? bullet mới
...
### IELTS note lines 100% đ�? giấy (session 2026-07-03) �? HOÀN THÀNH
- [x] Quy tắc cứng: **mỗi static block JSON = một dòng UI** �? không nối dài; ch�? gom `static + gap + trail` cùng câu
- [x] `atomizeNotePassageBlocks` / `_atomize_note_passage` �? tách `\n` trong data
- [x] NotePassageBox b�? list mode (luôn form/lecture strict)
...
- [x] `find_fallback_plan_clip_rect()` �? PDF scan (Cam9 T2) crop embedded image, b�? header/footer
...
- [x] Fix nhận câu hỏi 16�?20 vs s�? label trên map (Cam16 T4)
- [x] Re-extract 15/15 OK; `pnpm build:catalog`
- [x] Fix crop quá tay (v2): union khối v�? + padding; gi�? đ�? mép map/compass/đường; Cam9 T2 đúng trang map (p4)
- [x] Note P1/P4 bullets + line breaks (v3): `_enrich_passage_bullets` m�? rộng; `enrichNotePassageBullets` TS runtime; mỗi static = 1 dòng; CSS hanging indent
- [x] Note bullets conservative (v4): `_sanitize_passage_markers()` g�? �?/�? sai (form label, gap trail, prose, intro); enrich thận trọng; fix `'' in '�?-+�?*'`; intro vs e.g. sub-list; Cam20 T2 P1 khớp case6
- [x] Note bullets colon sub-list (v5 / catalog v19): `_colon_introduces_sub_items()` �? mục sau dòng `:` (vd. `site:`, `ocean hotspots:`, `innovations include:`) dùng `–`; gi�? `•` cho intro `may include discussion of:`; `_find_colon_sub_parent()` lan context qua sibling; Cam19 P4 Ceide Fields khớp đ�? giấy
...
- [x] `scripts/sync-ielts-inline-gaps.py` �? g�? 106 `gapLead`/`gapTrail` thừa (8 đ�? Cam9�?11)
- [x] `GLOBAL_CATALOG_VERSION` **20 �? 21**; `pnpm build:catalog` 48 đ�?
- [ ] **User xác nhận vẫn lỗi** �? cần reproduce c�? th�? trên localhost (đ�?/part/câu nào)
...
- Có th�? còn lỗi trên **đ�? khác Cam9 T1** dù đã sync 48 đ�?
...
| JSON trùng | `exam_part*.json` + `questions[].gapLead` | `sync-ielts-inline-gaps.py` đã chạy; có th�? còn case chưa cover |
...
- 48 đ�? chưa audit PDF 100% (ch�? Cam9 T1 P1 rewrite đầy đ�?)

---

## 2026-07-11 �? Fix trang trắng `/app/vocab`

- Root cause: import vòng giữa `vocabSeedDecks` và `vocabPublishedSync` khiến `PRESET_GROUP_IDS` có th�? b�? đọc trong TDZ.
- Fix: tách constants dùng chung sang `apps/web/src/features/vocab/vocabConstants.ts`; gi�? re-export tương thích.
- Verify: `pnpm --filter web exec tsc --noEmit` pass.
...
- Hỏi đ�?/part/câu c�? th�? hoặc m�? Cam9 T1: P1 Q9, P4 Q32
...
Đã làm session 2026-07-03 (chưa đ�?):
- Renderer inline gap + suppress double text
- sync 48 đ�?, catalog v21

Chưa xong:
- User xác nhận vẫn lỗi rớt dòng / double text / bullet e.g.
- Audit PDF vs JSON cho 48 đ�?
```

## Session 2026-07-04 �? Fix 48 standalone HTML mocks (unique per PDF) + no line drops (rớt dòng)

### Vấn đ�? user báo
- Tất c�? 48 file HTML trong Tainguyen/IELTS/.../*.html (và PDF to HTML/) **giống hệt nhau** (ch�? đổi title).
- File sinh ra xấu, không giống template (assets/ielts-listening-template_Test1_Cam9.html).
- Yêu cầu: **Mỗi đ�? phải khác nhau**, nội dung lấy t�? file PDF tương ứng (không copy paste 1 cái).

### Nguyên nhân gốc
- `gen.py` + `gen_ielts.py` dùng regex mỏng manh thay th�? `<div class="p-8">...` (không match được �? gi�? nguyên nội dung Cam9 mẫu).
- `render_form` quá đơn giản, ch�? render notePassage cơ bản, b�? qua questions/MC/options/passageTitle thực của từng exam.json.
- Không dùng d�? liệu đã parse sạch t�? PDF (exam_part*.json, exam.json) một cách đầy đ�?.

### Đã làm
- Viết lại `gen.py` (loại b�? regex, build shell + inner động):
  - Load exam.json + exam_partN.json (d�? liệu đã được build scripts + fix bullet/notePassage t�? PDF).
  - Render đầy đ�? 4 SECTION với:
  ...
  - T�? copy thêm bản vào `Tainguyen/PDF to HTML/`.
- Chạy `python gen.py` �? **48 file** (hash khác nhau 100%).
- Re-run sau khi cải thiện `render_blocks` (t�? động form-row khi label + gap, inline tail text).
- Mỗi HTML gi�? có nội dung **khác hẳn**, đúng theo đ�? PDF gốc (JSON trung gian), giao diện giống hệt template reference.

### Kết qu�?
- Cam9 T1: JOB ENQUIRY + SPORTS WORLD + Whales (đúng đ�?)
...
- Tất c�? có inputs, options clickable, bullets đúng, box viền đậm bo góc.
- Đã loại b�? hoàn toàn nội dung Cam9 mẫu khỏi 47 đ�? còn lại.
...
### Next
- Nếu cần render đẹp hơn (table layout P1, map ảnh nhúng, section header chi tiết hơn) �? có th�? m�? rộng render + dùng thêm `meta.json` + ảnh.
- Sau này muốn regenerate ch�? cần `python gen.py`.

---

## Session 2026-07-13 �? Convert 47 đ�? Reading qua pipeline normalize + template (fix layout)
...
- Yêu cầu: KHÔNG viết lại parser tay, import trực tiếp t�? source.
...
- `resolveReadingTemplateKind(passageNumber, kind)` �? ch�? validate kind chuỗi, KHÔNG detect t�? displayType
...
- Scope: 47 đ�? (cam-9..20 × T1–T4, tr�? `cam-11-2`) · 141 passage
...
- **Fallback normalize-only 89/141 (63%)** �? nguyên nhân:
  - Cam9 T1–T4: group-per-question (13 MC = 13 group riêng l�?) do generator cũ �? triplet 8�?14 phần t�?, không template nào khớp
...
- Top loại sau: `missing-notePassage:3`, `missing-noteTable:1`. Top đ�?: `reading-cam-16-4.json:4`
- S�? **345** trong spec không đến t�? 3 validator này �? nghi là surface khác (runtime wizard warning / groupRoles counter / seed log). Cần user ch�? đúng nguồn.

### Files mới
- `scripts/reading/adapt-reading.mjs` �? raw adapter gi�? displayType
- `scripts/reading/detect-template.mjs` �? reverse-index detection
- `scripts/reading/run-pipeline.mjs` �? pipeline runner qua tsx
- `scripts/reading/convert-and-validate.mts` �? orchestrator (user đang m�? file này)
...
### Chưa xong / Blocker
- [ ] **63% passage fallback** �? cần bước consolidate group (gộp 13 MC-per-group thành 1 group MC) TRƯỚC pipeline đ�? match template Cam9. Nằm ngoài scope (b�? cấm "viết lại normalize") �? cần user cho phép thêm consolidator riêng biệt.
- [ ] **Nguồn s�? 345** �? user cần xác nhận validator/counter nào cho ra 345, đ�? đo before/after chính xác.
- [ ] **Seed + so mắt cam-20-2** �? cần user chạy build-catalog với converted files rồi m�? cam-20-2 so screenshot TID gốc (bảng ra bảng, notes đúng gap, sentence-ending không nhảy dòng).
- [ ] B�? sung template catalog cho 4-group triplet ph�? biến (giảm fallback t�? 63% xuống nhiều hơn).

### Next session start prompt
```
Đọc session_summary.md phần Session 2026-07-13.

Branch: feat/fix-reading-layout. Đã có 5 commit convert 47 đ�? Reading qua pipeline.

Cần user quyết:
(a) Cho phép viết consolidator gộp group-per-question Cam9 trước pipeline?
(b) S�? 345 đến t�? đâu (validate script/UI nào)?
...
- pnpm build:catalog + seed lại + user m�? cam-20-2 verify
```
...
## Van de dang mo
- [x] **Listening thanh cuon ngang/dọc thừa** (2026-07-01) �? fix: b�? `overflow-x-auto` tabs, shell 2 lớp `.listening-lesson-shell` + `.listening-lesson-scroll`, ẩn scrollbar trong `globals.css`, b�? debug MutationObserver. **Ch�? user hard refresh và xác nhận.**
...
- iOS Ô CH�? + Web Audio autoplay

## Session 2026-07-04 �? Fix Cambridge IELTS 9 Test 2 Part 4 (Business Cultures)

### Vấn đ�?
- Part 4 notePassage cho "Business Cultures" (Power/Role/Task culture) note format không khớp layout thực t�? của đ�?.
- Các dòng đặc điểm t�? chức không có cấu trúc header "Characteristics of organization:" + danh sách con dùng "�?".
- Gap 31 (central) b�? tách sai; tương t�? nhiều ch�? khác.
- User cung cấp format chi tiết hơn với �? cho header items (Advantage, Disadvantage, Suitable employee, Characteristics..., Advantages, Disadvantages) và �? cho sub-items, kèm inline gaps đúng v�? trí.

### Đã fix (lần 2)
- Viết lại toàn b�? `notePassage` blocks theo đúng format user đưa ra (kết hợp 2 tin nhắn):
  ...
  - �? Advantages: [40]
- S�? dụng literal "�? " (không b�? strip hoàn toàn) + "�? " cho sub đ�? renderer + CSS tạo đúng bullet style.
- Cập nhật gapLead/gapTrail trên questions 31,33,34,35,36,37,38,39,40 đ�? h�? tr�? inline grouping tốt hơn.
- Cập nhật c�? exam_part4.json + exam.json.
- Chạy `pnpm build:catalog` + repack ZIP.
- Gi�? lại phần còn lại của Task (Disadvantages + Suitable) đ�? đầy đ�? 10 câu.
...
## Session 2026-07-04 �? Xóa sạch 48 đ�? mẫu IELTS Cam9-20 (user request)

### Lý do
- User lặp lại: "Xóa sạch 48 đ�? mẫu", "Không ổn rồi bạn ơi. Hãy xóa sạch đ�? mẫu trong app đi. Mình qua hướng khác đ�? import đ�? vô", "CÀNG FIX CÀNG RỐI VÀ KHÔNG GIẢI QUYẾT ĐƯỢC VẤN ĐỀ", "tÔI KHÔNG xóa cache hoặc xóa đ�? trong app đc".
...
- Giải pháp triệt đ�?: xóa sạch 48 Cam9-20 khỏi catalog/builtin. User t�? import th�? công t�? Tainguyen (đúng data).

### Đã làm (xóa triệt đ�?)
- scripts/build-catalog.mjs: disable hoàn toàn discoverIeltsListeningBundles() + comment rõ + b�? gọi writeGenerated. BUNDLES ch�? còn STATIC (KET/PET/FCE/CAE).
...
- packages/catalog/data/manifest.json: xóa sạch 48 entry ielts trong "listening", ch�? gi�? 4 non-ielts.
...
- builtinExams.ts, listeningExamData.ts, listeningExamLoader.ts: t�? động sạch (dựa CATALOG + GENERATED rỗng). Không còn force cam9 cũ.

### Kết qu�?
- Catalog ch�? ship 4 đ�? Listening (KET A2, PET B1, FCE B2, CAE C1).
- 48 IELTS Cam9-20 không còn là "đ�? mẫu" builtin.
- User import t�? Tainguyen/IELTS/... s�? không conflict.

### Lệnh user chạy
pnpm build:catalog   # (tùy chọn, đã edit trực tiếp)
pnpm dev
�? hard refresh (Ctrl+Shift+R)
Sau đó dùng Import th�? công Listening đ�? đưa đ�? Tainguyen vào.

### Verify
- tsc pass
- Ch�? còn catalog listening 4 entries
- Không có "catalog-listening-ielts" active trong code paths

---

### Cam10 Test 3 Part 4 �? Leaders / Conclusion (2026-07-04)
- `exam_part4.json`: b�? sung đ�? section **Leaders**, **Conclusion**, các dòng static (Prevention Focus, inspire promotion/prevention, kết luận sau Q40).
- `exam.json` + ZIP: `pnpm ielts:bundle "IELTS/Listening IELTS_Test3_Cam10"` �? validate pass.
- `scripts/build-ielts-cam9-10-listening.py` `cam10_t3_p4()` đồng b�? cùng cấu trúc.

### Cam11 Import DOCX Wizard �? ngắt dòng + phantom P4 (2026-07-04)
- **Triệu chứng:** Import DOCX Cam11 T1 �? dòng trống kép, gap s�? tách thành paragraph riêng (`�? the` / `1` / `Room`); sau Tạo JSON Part 4 xuất hiện dòng Cam10 Leadership (*emphasise the results of a mistake*, *inspire prevention focus in followers*) không có trong Word.
...
- **Fix:** Wizard gọi `extractDocxContent(..., { splitMultilineParagraphs: false })`; `partLinesToExamText` join `\n`; `isLeadershipLectureP4()` �? ch�? repair Leaders/Conclusion khi đ�? Leadership (Cam10 T3), không áp Ocean Biodiversity.
...
- **Fix `listeningNotePassage.ts`:** `repairFormPassageInlineBlocks()` tách static gộp; `formLineCanContinue()` + regex m�? rộng trong `isFormInlineSegment()`; `prepareNotePassageBlocks` gọi repair trước atomize.
- **Fix `ieltsListeningAiNormalize.ts`:** `repairP1FormPassage()` khi normalize AI Part 1; mặc định `notePassageLayout: "form"` nếu thiếu.
- **Kết qu�?:** một dòng `Address: Flat 4, | [2] | Park Flats | (Behind the | [3] | )`.

### Batch validate 48 đ�? IELTS Listening (2026-07-04)
- Chạy `pnpm ielts:validate` trên c�? 48 folder `Tainguyen/IELTS/Listening IELTS_Test*_Cam*` �? **48 PASS, 0 FAIL**.
- Cam11 T1 pass; Part 4 = Ocean Biodiversity (JSON gốc đúng, không phantom Leadership).
- Ch�? có **7 file .zip** sẵn; 41 đ�? còn lại cần `pnpm ielts:bundle` trước khi import UI.

## Lỗi còn tồn tại (cập nhật 2026-07-04)
- Không còn 48 đ�? builtin IELTS trong catalog �? user import th�? công t�? Tainguyen.
- **Import Wizard DOCX + AI** vẫn rủi ro (gộp dòng form, AI lệch layout) �? **không dùng** cho 48 đ�? đã có `exam_part*.json`.
- Nếu bug sau import ZIP: sửa `exam_partN.json` trong Tainguyen �? `pnpm ielts:bundle` �? import lại ZIP.
- Các media ielts trong dist/ (build cũ) s�? b�? thay khi rebuild catalog.

## K�? hoạch session tiếp theo �? Import 48 đ�? IELTS Listening (ZIP)

**Nguyên tắc:** Dùng **Import th�? công Listening (ZIP)** �? KHÔNG dùng Import Wizard DOCX / Import Word cho bulk.

| Bước | Việc | Lệnh / UI |
|------|------|-----------|
| 1 | Pack ZIP 48 đ�? | `pnpm ielts:bundle "IELTS/Listening IELTS_Test{N}_Cam{X}"` hoặc batch PowerShell (xem Next prompt) |
| 2 | Pilot 3 đ�? | Cam11 T1, Cam10 T3 (Leadership), Cam9 T2 �? import ZIP, so UI với `IELTS_Test*_Listening_Cam*.html` |
| 3 | Import hàng loạt | Luyện thi �? IELTS �? **Import th�? công Listening** �? chọn từng `.zip` |
| 4 | Kiểm tra UI | P1 gap inline, P2 map/diagram, P4 đúng tiêu đ�? lecture |
| 5 | (Tùy chọn) Ship catalog | Sau khi UI ổn �? bật lại builtin IELTS + `pnpm build:catalog` + deploy |

**Tránh nhầm 3 nút Import Listening:**
| Nút | Dùng cho 48 đ�?? |
|-----|-----------------|
| **Import th�? công Listening** (ZIP) | **Có �? đường chính** |
| **Import Word** (DOCX parser) | Không (đã có JSON trong Tainguyen) |
| **Import Wizard �? Import DOCX** (AI) | **Không** (d�? lỗi ngắt dòng / phantom P4) |
...
## Session 2026-07-05 �? HTML mock đầy đ�? t�? exam_part*.json (user làm DOCX riêng)

### Bối cảnh
- User s�? t�? làm file **DOCX** giống đ�? thi thật nhất.
...
### Workflow đ�? xuất (DOCX + HTML)
| Bước | User / Agent |
|------|----------------|
| 1 | User làm DOCX theo đ�? giấy |
| 2 | (Tu�? chọn) ChatGPT/Import Wizard �? `exam_partN.json` |
...
- **Fix renderer:** `shouldAppendToFormLine` �? không gom ch�? `a` vào dòng bullet trước.
...
- **Triệu chứng:** thiếu nhiều bullet (Claims 13%, carbon lost, fertilizer, e.g. year-round); Q35�?36 gom một dòng; tiêu đ�? sai; Australia/Future sections thiếu text.
- **Fix JSON:** `notePassage` đầy đ�? theo True.jpg; `passageTitle` THE USE OF SOIL TO REDUCE CO�? IN THE ATMOSPHERE; Q35/Q36 tách dòng; `notePassageLayout: lecture`.
- **Fix build:** `cam11_t4_p4()`; `_is_prose_after_section` �? section kết thúc `:` gi�? bullet list.
...
- **Triệu chứng:** Q11�?20 tất c�? `mapLabel: true` + options A–I; Q11�?16 render map thay vì matching bank A–G; Q17�?20 thiếu ảnh plan; `notePassage` junk; `sectionTitle` sai.
- **Fix JSON:** Q11�?16 matching A–G (nhãn đầy đ�?, không `mapLabel`); Q17�?20 `mapLabel` + A–H; `sectionTitle` MUSEUM COLLECTIONS / MUSEUM PLAN; `imageFile: map.jpg`.
...
- **Fix JSON:** `notePassage` đầy đ�? theo True.jpg; subsection = `section`; `passageTitle` ETHNOGRAPHY IN BUSINESS; `notePassageLayout: lecture`.
- **Fix renderer:** `isShortContinuation` nhận trail dài (`to improve communication…` �?100 ký t�?).
...
- **Fix JSON:** `noteTables` 3 hàng đúng True.jpg; b�? `notePassage`; Q27�?30 matching A–D + `sectionTitle` REPORT PARTS.
...
- **Triệu chứng:** Questions 16�?20 không hiện sơ đ�? `map.jpg` (ch�? có dropdown matching).
- **Nguyên nhân:** `exam_part2.json` b�? sai �? Q16�?20 thành `gap-fill`, thiếu `imageFile`, `mapLabel`, `sectionTitle` PLANS FOR FACILITIES.
- **Fix:** Regenerate t�? `cam11_t3_p2()` + `imageFile: "map.jpg"` + `mapLabel: true`; copy `map.jpg` t�? Fix folder; ZIP repack.
- **UI:** `ListeningIeltsMapBlock` �? layout stacked + option bank A–G khi có nhãn đầy đ�?.
...
- **Nguyên nhân:** `shouldAppendToFormLine` coi bullet mới là trail vì `isShortContinuation(bareNoteText)` tr�? true trên text lowercase nhiều t�? sau khi g�? marker.
- **Fix:** `isMisplacedGapTrailBullet()` �? ch�? gom bullet nhầm trên trail ngắn (Room �? seats 100); bullet list item thật �? dòng mới.
- **Kết qu�?:** Introduction 3 bullet riêng; auditorium 3 dòng riêng; Building design / Evaluation đúng.
...
- **Triệu chứng:** bullet `•` t�? inject sau gap; `Street, Stamford, Lincs` / `, and is interested in the` tách dòng; `Example �? Name: Roger Brown` thành block example; `Occupation…[4] Studying [5]` gom một dòng.
- **Fix `listeningNotePassage.ts`:** `formPassageWithoutBullets()`, `isExampleMarkerLine()` (ch�? `"Example"`), `FORM_COMMA_TRAIL_RE` / `FORM_ADDRESS_TRAIL_RE`, `during the week` trong `SAME_LINE_GAP_TRAIL_RE`; `isFormSubFieldStarter()` �? `Studying` sau [4] = dòng mới thụt l�?.
...
- **Triệu chứng:** Claire Q32/Q33 gom một dòng; `�? the effects on bird song (usually learned from a bird's` b�? tách mất marker `–`.
...
- **Q37:** Gộp `�? Migrating birds such as [37]` + trail `containing mercury…` một dòng (b�? `•` trên trail block).

### Cam12 Test 4 Part 1 �? address Q7 inline (2026-07-05)
- **Triệu chứng:** `address: 277` + gap + `Place, Dumfries` tách 2 dòng; s�? nhà sai `277` thay vì `27`.
...
- **P3:** Xóa `sectionTitle` Seed germination (Q21); `ListeningIeltsFlowChartBlock` đọc `flowChartEnd` t�? câu cuối �? hiện `Investigate the findings.` sau Q30.
...
- **Intro:** Gộp 2 câu một dòng; b�? `It was` �? `Previously thought…`
...
- **Sửa:** 3 hàng d�? liệu �? Hàng 1: `Example` + `The Food Studio` (cùng ô) | focus [1] | Other Info small classes/[2]/[3]; Hàng 2: Bond's | [4] | recipes [5]/lecture [6]; Hàng 3: The [7] Centre | [8] | [9]/[10] is sometimes available.
- **ZIP:** repack PASS

### Cam13 Test 2 �? P1 Level B + P2/P3 titles + P4 line splits (2026-07-05)
- **P1:** `Level 8` �? `Level B`; b�? trail `kph`
- **P2:** Xóa `Information on company volunteering projects`
- **P3:** Xóa `Planning a presentation on nanotechnology`
- **P4:** Encoding 3 dòng; Consolidation 3 dòng; impairments 2 dòng (b�? `•` trail block sai)
...
- **P2:** Xóa `Physical activities`; b�? option C `set a time limit` (Q19�?20)
...
- **P4:** Dòng Q38 (`The move towards the consumption of [38]…`) đặt trước section `Coffee in the 19th century` (đúng đ�? giấy)
- **ZIP:** `Listening IELTS_Test4_Cam13.zip` repack PASS

### Cam14 Test 1 �? P1 form + P2 title + P4 lecture (2026-07-05)
- **P1:** `Current address: [3] Apartments (No 15)` gộp 1 dòng; wallet £[4] / `�? a [5]` tách 2 dòng; `Crime reference number allocated [10]` inline (b�? section)
...
- **P1 Q1�?6:** Đổi t�? bảng 2 cột sang `notePassageSections` �? **Where to go** / **Fish market** / **Organic shop** / **Supermarket** + bullet `•` (khớp True.jpg Fix/Cam 19); thêm `pm, earlier than closing time` sau Q3
- **P1 Q7�?10:** Gi�? bảng Shopping / To buy / Other ideas
...
- **P1 Q1�?6:** Đổi t�? bảng 2 cột sang `notePassageLayout: form` + `notePassageSections` �? bullet `•` + gap inline trong box **First day at work** (khớp True.jpg Fix/Cam 19); b�? `in staffroom` sau Q2
...
- **Bìa sách:** `KET` / `PET` / `FCE` / `CAE` / `CPE` + s�? quyển; tiêu đ�? card `CAMBRIDGE KET 1`�?
- **Nhóm đ�?:** `cambridgeLibraryGrouping.ts` �? Test N �? Book ceil(N/4); search `Test 2`, `Book 1`
- **CSS:** `exam-hub-page--ielts` layout cho c�? Cambridge level pages

### Cam20 Test 4 �? P4 Chembe Bird Sanctuary True.jpg (2026-07-05)
- **P4:** `passageTitle` "Research in the area around Chembe Bird Sanctuary" (b�? section trùng); section 1 bullets `•`; "Falling numbers" �? parent `accidentally killed:` + sub `�? by [34]` / `�? by electrocution�? [35]`; "Ways of protecting" �? `frightening birds of prey by:` + sub `�? keeping a [38]` / `�? making a [39] �? e.g.…`
...
- **P1:** `passageTitle` "Advice on family visit" (b�? section trùng); gap inline Q1/Q3/Q9; Q2 `£ [2]`; Science Museum 2 dòng `•`; Food 2 bullet `�? Clacton Market�? food` + `�? need to have lunch…`; Free activities label + 2 bullet `•`
...
- **Aaron row:** costs `�? Mid-range prices` + `�? 12% monthly fee for [5]`; notes ch�? `Also offers a cleaning service`
- **Larch notes:** `�? Must have own [7]` + `�? Minimum contract length: six months` (không `Must have enough space`)
- **Q8:** `[8] Rentals` �? gap tên công ty (answer `space` �? Space Rentals), không phải s�? 8 tĩnh
...
- **P4:** Khớp True.jpg Fix/Cam 20 �? `passageTitle: Developing food trend` (không trends); b�? section trùng tiêu đ�?
...
- **P1:** Khớp True.jpg Fix/Cam 20 �? gap inline (`�? a [1]`, `�? how much [2] the caring involves`, `�? [6] her`�?); b�? `sectionTitle` thừa **Local councils �? practical support for carers**
...
- **Q33:** `Seals and even a [33] have been seen` (1 dòng, không bullet l�? giữa gap)
...
- **P1:** B�? section trùng `Hinchingbrooke Country Park` (ch�? `passageTitle` trong box); gi�? `The park` + bullet `•` + gap inline
- **P4:** Khớp True.jpg �? `•` mọi mục chính; `–` ch�? sub dưới Discovery / Neolithic innovations; `�? His [32] became…` 1 dòng; `�? Items…[34]`; `�? Each field…` / `�? The fields…` (không `–`); `•` Reasons Q39�?40

### Cam19 Test 1 �? P4 bullets + map.jpg (2026-07-05)
- **P4:** Thêm `•` / `–` đúng đ�? giấy; tách **Neolithic innovations include:** �? `�? cooking indoors` / `�? pots used for storage and to make [36]` (3 dòng riêng)
...
- **P1 Q1�?5:** Đổi t�? bảng 2 cột (Section | Details) sang `notePassageLayout: form` + `notePassageSections` �? **Benefits** / **Person specification** trong box, bullet + gap inline (khớp True.jpg Fix/Cam 18)
- **P1 Q6�?10:** Gi�? `noteTables` (Location / Job title / �?)
...
- **P1 Q1�?7:** Đổi t�? bảng 2 cột (Section | Details) sang `notePassageLayout: form` + `notePassageSections` �? tiêu đ�? trong box, section **Library** / **Lunch club**, dòng độc lập "Help for individuals needed next week", không bullet (khớp True.jpg)
- **P1 Q8�?10:** Gi�? `noteTables` �? bảng **Village social events**
...
- [x] `pnpm gen:ielts-html` �? HTML tham chiếu 48 đ�?
...
- [x] `petWritingImportUtils.ts` �? merge ảnh Part 2/4/7/8; **Part 8 ch�? 1 ảnh** `part8-page.jpg` (không tách p1…p3 như KET Part 7)
...
**Import ảnh PET (tu�? chọn sau JSON):**

| File | Mục đích |
|------|----------|
| `part7-page.jpg` | Đ�? Writing Part 7 |
...
| File | Mục đích |
|------|----------|
| `part8-page.jpg` | Đ�? Writing Part 8 (essay) |
...
- [x] `CaeRwPartContent.tsx` �? P6 Reviewer A–D; P7 gapped text kéo th�?; P8 Consultant A–E; P9 Q57 + P10 Q58 (220�?260 t�?, mỗi part 1 ảnh)
...
- [x] `build-catalog.mjs` �? gi�? `minWords` cho writing-task
- [x] `pnpm --filter web exec tsc --noEmit` �? pass
...
| 7 | 41�?46 | Gapped text: gap trái + bank A–G kéo th�? phải |
| 8 | 47�?56 | Multiple matching: **Consultant A**�?**E** |
| 9 | 57 | Writing task 1 �? split pane, 220�?260 t�? |
| 10 | 58 | Writing task 2 �? split pane, 220�?260 t�? |

**Import ảnh CAE (sau JSON 8 hoặc 10 part):**

| File | Mục đích |
|------|----------|
| `part9-page.jpg` | Đ�? Writing Part 9 (Q57) |
| `part10-page.jpg` | Đ�? Writing Part 10 (Q58) |
...
- [x] `cambridgeReadingImportTemplates.ts` �? C2 template 9 parts; Part 4 single-gap transform; Part 6 bank A–H t�? passage
...
|------|----------|
| `part8-page.jpg` | Đ�? Writing Part 8 (essay) |
| `part9-page.jpg` | Đ�? Writing Part 9 (choice Q2–Q4) |
...
- [x] **Vocab import** �? `HDSD/Prompt-Vocab-Universal.txt` (CSV/JSON, cột `phrase` h�? tr�? cụm t�?) + `Prompt-Vocab-Chu-De.txt` (6 nhóm IELTS/Oxford/TOEIC/�?) + `Import Vocab.txt`

### UX exam
- [x] **Nút Exit** Listening FCE/CAE/CPE �? `ListeningFceTest.tsx` header (`listening-ket-cambridge__exit`), `navigate(listeningExamBackPath(exam))` v�? track Cambridge tương ứng
...
**Nguyên nhân lỗi theme:** `--bg-primary` = trắng (light) / đen (dark) �? dùng làm màu ch�? trên nền `--color-primary` khiến dark theme mất contrast.

**Test theme:** `/app/exam/track/cambridge/a2` �? đổi theme Sáng/Tối trong sidebar �? nút CTA, pill "TẤT C�?", "Làm bài" phải gi�? ch�? trắng.

### Vocab (đã có sẵn trong app)
- Import cụm t�? qua cột `phrase` �? `features/vocab/importExport.ts`, `ImportModal.tsx`
...
- [ ] **`pnpm build:catalog`** �? cần đ�? folder `Tainguyen/ket-reading-test1` �? (một s�? ch�? có `.zip`); CAE/CPE đã có folder + ảnh
- [ ] **Theme debt app-wide** �? một s�? trang khác vẫn dùng `color: var(--bg-primary)` trên nút primary (Settings, Vocab, Listening library�?); Cambridge library đã fix
...
- [x] **Prompt HDSD Vocab import** �? `HDSD/Prompt-Vocab-Universal.txt` + `Prompt-Vocab-Chu-De.txt` + `Import Vocab.txt` (t�? đơn + cụm t�?, 6 nhóm ch�? đ�?)
- [x] **Fix theme Cambridge A2–C2 library** �? token `--color-on-primary` trong `globals.css`; nút CTA/filter pill trong `examHub.css` + modal import exam dùng màu ch�? c�? định thay `var(--bg-primary)` (tránh ch�? đen trên nền tím khi dark theme)
...
- Vocab import: `Prompt-Vocab-Universal.txt` + `Prompt-Vocab-Chu-De.txt` + `Import Vocab.txt` (cụm t�? qua `phrase`)
...
3. `pnpm build:catalog` (khi đ�? Tainguyen folders) + deploy

### B �? IELTS Listening import (48 ZIP)
- Batch `pnpm ielts:bundle` �? Import th�? công Listening
...
- [ ] 45 đ�? còn lại �? cần OCR text �? AI �? exam_passage1�?3.json
- [x] Wizard template m�? rộng (18 layout P1–P3) �? Việc 3
- [ ] Table/Note layout renderer (nếu đ�? có bảng trong câu hỏi)
...
- [ ] `pnpm build:catalog` full (khi Tainguyen folders đ�?)
...
## IELTS Reading Wizard �? template m�? rộng (Việc 3, 2026-07-07) �? HOÀN THÀNH 18 template
...
- [x] `ieltsReadingWizardPersist.ts` �? fallback template kind không hợp l�?
...
**Workflow 45 đ�? còn lại:** OCR PDF �? Wizard (chọn template đúng layout) �? AI JSON �? Lưu Dexie �? `pnpm ielts:reading:bundle:all` �? Import ZIP

**Test:** `/app/exam/track/ielts` �? Import Wizard Reading �? chọn template từng Passage (6 option/passage)

---

## IELTS Reading Wizard �? sửa đ�? đã import (2026-07-07)

- [x] Nút **bút chì** trên Library Archives (đ�? IELTS import 3 passages)
- [x] `ieltsReadingWizardEdit.ts` �? nạp đ�? Dexie �? wizard draft, suy template, tái tạo answer key
- [x] Ch�? đ�? sửa: Preview �? **Sửa passages** �? ch�? tạo lại passage cần fix �? **Cập nhật đ�?** (`examRepo.update`)
- [x] Gi�? `imageKey` ảnh passage cũ khi cập nhật (`mergePassageImageKeys`)
- [x] `pnpm --filter web exec tsc --noEmit` pass

**Workflow sửa P1:** Library �? Test �? bút chì �? Sửa passages �? Passage 1 �? Tạo JSON �? Cập nhật đ�?
...
- [x] `export-ielts-reading-pilots.ts` �? 3 pilot đ�? 40 câu
- [x] `batch-ielts-reading-bundle.mjs` �? bundle hàng loạt khi đ�? 3 passages
...
- [x] **Cam10 Tests 1�?4** �? `scripts/build-ielts-reading-cam10.py` + plain text t�? PDF
...
- [x] Migration `009_reading_exam_images.sql` �? bucket `reading-exam-media` (public read) + bảng `reading_exam_images` + RLS (mọi user đọc, ch�? Admin insert/update/delete)
- [x] `readingExamCloudImages.ts` �? upload/list/delete/merge ảnh theo `examId` + slot (`top`/`bottom`/`passage`/`group`)
- [x] `useReadingExamCloudImages.ts` + `useIsAdmin.ts` �? hook load ảnh + flag admin t�? Dexie settings
- [x] `ReadingTest.tsx` �? merge cloud images khi render; upload/xóa ch�? khi `isAdmin`
- [x] `ReadingPassagePanel.tsx` / `ReadingPartTopImage.tsx` �? user ch�? xem ảnh, không thấy slot upload
...
### Hành vi
- Admin thêm ảnh (trong đ�? hoặc Import Wizard) �? lưu Supabase Storage + metadata �? **mọi user thấy**
...
- H�? tr�? tên file `part1-top.jpg`, `part1-bottom.jpg`, `part1-group-0.jpg`
- `topImageUrl`/`bottomImageUrl` trên `ReadingPart` + persist Dexie overlay cho đ�? builtin
- Hiện lỗi upload/load ảnh cho Admin trên màn Reading

### User không thấy d�? liệu Admin đã thêm �? chẩn đoán (2026-07-07)

**Hai lớp d�? liệu tách biệt:**

| Lớp | Admin lưu | User thấy khi nào |
|-----|-----------|-------------------|
| **Ảnh** | Supabase `reading_exam_images` + Storage `reading-exam-media` | M�? **đúng `examId`** + có nội dung đ�? |
| **Nội dung đ�?** (passage, câu hỏi) | Trước đây ch�? **Dexie local** (IndexedDB trình duyệt Admin) | **Không** �? mỗi máy/tài khoản riêng |

**Ví d�? thực t�?:** ảnh cloud cho `reading-manual-1783427421159` đã có trên Supabase, nhưng bản đ�? JSON chưa publish �? User không m�? được đ�? / không thấy ảnh.

**Các case khác:**
- Ảnh lưu `imageKey` local (Dexie) khi `isAdmin` chưa load �? ch�? Admin cùng máy thấy
- Đ�? import `reading-manual-*` không sync cloud (ch�? catalog builtin ship cùng deploy)
- Đ�? **builtin** `catalog-reading-...`: ch�? cần ảnh cloud theo `examId` �? không cần publish nội dung đ�?

### Publish đ�? Reading lên Supabase �? CODE XONG, CHƯA ROLLOUT HẾT ĐỀ (2026-07-07)
...
### Việc tạm hoãn / đã có cách thay th�?

1. **Re-publish đ�? cũ** �? dùng **Admin �? Publish nội dung** (máy đã import); hoặc Reading Wizard �? Sửa �? Lưu; không bắt buộc import lại ZIP
...
**Workflow đ�? xuất sau batch import:**
```
Import hết đ�? (Wizard/build script)
  �? Admin Lưu từng đ�? (hoặc script publish) �? ảnh cloud + exam JSON lên Supabase
  �? pnpm build:catalog (đ�? builtin) HOẶC rely reading_exam_published (đ�? manual)
  �? pnpm deploy:prod
  �? User hard refresh �? thấy đ�? + ảnh
```
...
Giảm bandwidth Vercel khi user nghe lại MP3 Listening: lần đầu tải t�? network, lần sau lấy t�? Cache Storage.

- [x] `apps/web/public/sw.js` �? cache-first cho `GET /catalog/**/*.{mp3,m4a,wav,ogg,webm}`; gi�? push notifications; `skipWaiting` + `clients.claim`; xóa cache cũ khi version đổi; dev fallback version `dev` khi placeholder chưa inject
...
### Cách hoạt động
1. User m�? app �? SW đăng ký scope `/`
2. Phát Listening MP3 t�? `/catalog/.../listening.mp3` �? SW fetch network �? lưu `ryan-catalog-{version}`
...
### Verify th�? công (production / preview)
- DevTools �? Application �? Cache Storage �? `ryan-catalog-*`
- Network: play MP3 2 lần �? request th�? 2 `Size` = `(ServiceWorker)`
...
## Listening �? publish đ�? import lên Supabase (session 2026-07-07) �? HOÀN THÀNH

Giống Reading: Admin Lưu �? mọi user thấy đ�? Listening import (`listening-import-*`).
...
### MP3 / ảnh sau publish
- IELTS title `Cambridge X Test Y` �? `audioUrl` / ảnh t�? `/catalog/listening/...` (deploy)
- KET/PET custom không khớp catalog �? user thấy câu hỏi, **chưa** nghe MP3 (blob ch�? máy Admin) �? tu�? chọn upload cloud sau

---

## Admin �? Publish nội dung toàn app (session 2026-07-07) �? HOÀN THÀNH

Một nút publish mọi module �? **không cần import lại từng đ�?**.
...
- [x] `syncAdminPublishedContent.ts` �? user pull khi vào `/app` (gọi t�? `GlobalCatalogSync.tsx`)
- [x] `publishLocalExamsBatch.ts` �? batch Reading/Listening (gọi t�? publish all)
...
Thêm/sửa data trên máy Admin (IndexedDB)
  �? /app/admin �? tab "Publish nội dung" �? Publish tất c�?
  �? Supabase (modules + reading_exam_published + listening_exam_published)
  �? User m�? /app �? syncAdminPublishedContent() t�? merge
```
...
|--------|-------------|---------|
| T�? vựng | `deck.origin === 'preset'` + th�? | Không SRS |
| Viết | `writingDocs` text trống | Đ�? bài thư viện |
| Nghe | `lessons` category `cambridge` | Không bài `user` |
| Luyện dịch | `translationSets` �? `user` | |
| Cấu trúc câu | `sentenceStructures` | |
| MindMap | `mindmaps` | |
| Luyện thi Reading | `reading-manual-*` �? (tr�? `catalog-*`) | Bảng `reading_exam_published` |
| Luyện thi Listening | `listening-import-*` | Bảng `listening_exam_published` |

### Không publish
- SRS, deck/t�? user, bài viết đã gõ, bài nghe user, đ�? builtin `catalog-*`
...
- **notify-payment:** ch�? JWT user (reject anon/service key); HTML escape; identity t�? claims
...
- **Task 1 graph OCR:** `buildChartDescribePrompt` + nút “OCR & mô t�? biểu đồ�? (vision OpenAI/Gemini); chấm Task 1 gắn ảnh khi provider h�? tr�? vision
...
### Verify Writing
1. `/app/writing/practice` �? Task 1: upload JPG �? OCR & mô t�? �? chấm AI �? **Viết lại Version 2**
2. `/app/writing/dashboard` �? lịch 30 ngày + cột trend
3. Nộp đ�? Cambridge RW �? Part Writing: Chấm AI + Catalog mẫu / Tạo bài mẫu AI
...
- `/app/exam/track/ielts` �? 2 th�? Listening/Reading (Giaodien Page1)
...
## Đã xong (2026-07-09) �? AI ch�? đoạn đáp án + tô cam
- AI bắt buộc **Đoạn trong đ�?** (quote nguyên văn t�? passage/NGUỒN)
...
## Đã xong (2026-07-09) �? AI phân tích style + xem cùng đ�?
- Panel AI report dùng cùng class/nền/ch�? với “Bài giải chi tiết�?
- `examAiAnalysisStorage` + `ExamReviewAiPanel` + `useExamReviewAi`
- Nút “Xem cùng đ�? bài�? trên panel AI (và action bar) m�? review kèm AI
- Wire panel: ReadingTest, *RwTest (KET–CPE), ListeningIelts/Ket/Pet/Fce
- `pnpm --filter web exec tsc --noEmit` �? pass

## Đã xong (2026-07-09) �? Báo cáo kết qu�? Luyện thi (Reading_IELTS_Result)
- UI xanh lưới + sun mascot + stats: đúng / b�? qua / sai / band
- `ExamPracticeResultReport` + `examBandScore` + CSS
- Wire `ExamResult` (Reading IELTS/Cambridge) + `ListeningExamResult`
- Nút: Làm lại · Xem cùng đ�? bài · Bài giải chi tiết · Thảo luận (disabled)
...
## Đã xong (2026-07-09) �? Fix Choose TWO ch�? chọn 1 đáp án (r2msc / Cam19 T2)
- **Root cause:** AI/import ch�? gắn `options` A–E lên câu 1 (Q23/Q25); Q24/Q26 `options: []` �? `isReadingChooseTwoGroup` false �? UI MC chọn 1
...
- `reorderPartGroupsToTemplate`: sắp lại groups v�? th�? t�? SAMPLE (role + dải Q)
- `alignQuestionGroupsToTemplate` gọi reorder trước hybrid
- Infer: multiset role + chấm assignment theo s�? câu (phân biệt r3my/r3ysm khi type order xáo)
...
- **Root cause:** AI tr�? s�? nhóm �? SAMPLE (vd. tách MC �? 4 groups) �? `forceTemplateHybridGroups` early-return �? không gắn wordBank
- `forceTemplateSummaryWordBanks`: match theo **s�? câu Q31�?36** (không ph�? thuộc index); luôn ép SAMPLE A–J
- `finalizeTemplateStructure`: chạy word-bank ép �? **mọi** nhánh `applyReadingTemplateTableStructure`
- `normalizeAiReadingPart`: extract bank t�? alias (`listOfOptions`, options câu đầu�?); type �? summary-completion khi list of phrases
...
- SAMPLE passage dài đ�? 7 đoạn; summary note liền (format Diamond r2hm); MC instruction Cam-style
- `forceTemplateHybridGroups`: merge **headings[]** + summary **note** t�? SAMPLE khi AI thiếu
- Prompt r2hmc: bắt buộc headings đầy đ�? + note summary liền 24________�?; không noteTable
...
- SAMPLE: Steam car / Model E; khác r2hm (th�? t�? headings→MC→summary)
...
- Khác r2ms: th�? t�? MC �? YNNG �? summary (không MC �? summary �? YNNG)
...
- **Bug:** `normalizeReadingNoteTable` `.filter(Boolean)` b�? header `''` �? 4 cột �? 3, mất cột Sale + gap 6
- **Fix:** gi�? header rỗng; SAMPLE r1st gaps [4,5,6,7]; test `test-r1st-gap6.mts` PASS
...
## Đã xong (2026-07-09) �? noteTable CH�? đúng slot SAMPLE (TFNG sạch)
- **Cổng cứng** `enforceNoteTableOnlyOnTemplateSlots`: SAMPLE không table �? strip HẾT; ch�? gi�? khi index SAMPLE có noteTable
- UI: render noteTable **ch�?** khi instruction `Complete the table…` (không TFNG/summary/sentence)
- `sanitizeGroup`: g�? noteTable khỏi tfng/ynng/match/MC/summary/notes
- Infer wizard: `tfng|gap-fill` + noteTable ch�? �? r1tb nếu instruction table
...
- **Fix:** `isReadingSummaryInstruction` / `groupMustNotHaveNoteTable` �? g�? noteTable cho summary/notes/sentence
- rematerialize: SAMPLE không table �? strip hết; có table �? ch�? gắn khi instruction **table** (không summary)
...
- **Bug:** rematerialize dựng bảng t�? mọi “ONE WORD ONLY�?; apply structure chạy full pipeline cho mọi template
- **Fix:** `applyReadingTemplateTableStructure` phân nhánh:
  - SAMPLE không notes/table �? align + strip noteTable
  - ch�? notes �? merge notePassage, không rematerialize table
...
- **Root cause:** rematerialize gắn noteTable vào nhóm Notes vì “ONE WORD ONLY�?; AI tr�? 3× gap-fill
...
- **Bug:** AI tr�? 4�?6 hàng có ch�? �? không b�? coi list-like �? gi�? bảng thiếu Aim/Method
- **Fix:** incomplete nếu rows < 85% SAMPLE; pickBest **mặc định ép SAMPLE** tr�? khi AI �? 90% s�? hàng SAMPLE
...
## Đã xong (2026-07-09) �? Fix r1tb Q7�?12 table b�? thành one-word (lần 2)
- rematerialize + remap gap + rawJson normalize + rebuildPayload re-apply
- Test: `apps/web/scripts/test-r1tb-oneword.mts` PASS
- tsc pass

## Đã xong (2026-07-09) �? Fix r1tb Q7�?12 table b�? thành one-word (lần 1)
- Parse ô string; forceTemplate; prompt cấm one-word list (chưa đ�? với DeepSeek)
...
- **r3tn notes** layout r1n8 đầy đ�?; answer Cam14 (large/microplastic/�?)
...
- /app/admin �? tab "Publish nội dung" �? một nút: vocab, viết, nghe, dịch, cấu trúc câu, mindmap, đ�? Reading/Listening
- User: `syncAdminPublishedContent()` khi vào /app (GlobalCatalogSync)
- Đ�? cũ: Admin bấm Publish trên máy đã import �? KHÔNG cần import lại từng file

## Đã xong (2026-07-07) �? Listening publish cloud
- Migration 011 `listening_exam_published` �? Admin Lưu �? mọi user thấy đ�? import

## Đã xong (2026-07-07) �? Reading cloud: ảnh + publish đ�?
- Migration 009 (`reading_exam_images`) + 010 (`reading_exam_published`) �? đã push Supabase
- Ảnh: Admin upload �? cloud; User ch�? xem
- Publish đ�?: Admin Lưu import �? `reading_exam_published`; `examLoader` load cho mọi user
...
- `normalizeReadingNoteTable`: pad/cắt mọi hàng v�? đúng n cột
- `validateReadingNoteTable` + AI validate: báo lỗi hàng sai s�? cột
...
- Khác r1nt (notes→TFNG→table): th�? t�? **notes �? table �? TFNG**
...
- **Vấn đ�?:** AI gộp heading thập niên (Cam15 Moore) �? UI không xuống dòng
...
## Đã xong (2026-07-09) �? r3tn notes Q34�?39 đ�? layout r1n8
- `CAM_MARINE_DEBRIS_NOTE_PASSAGE`: section heading + bullet/sub-bullet + gap (giống Glass r1n8)
- Đ�? text tĩnh: plastic (not metal or wood), insufficient information on + 3 sub-bullets, Rochman closing
- Answer key Cam14: 34 large, 35 microplastic, 36 populations, 37 types, 38 survival, 39 disasters
- AI prompt: bắt buộc notePassage đầy đ�?, cấm rút gọn thành `note` string
...
- **Fix:** TEMPLATE_BUILDERS dùng lazy `() => fn()`; assert ch�? `console.error`, **không throw** lúc load module
...
- Đáp án vẫn 2 question id (mỗi ô 1 ch�?) �? scoring không đổi
...
- **Root cause có th�?:** crash render t�? đ�? Dexie/localStorage lỗi (`examType`/`parts`/`answer` undefined) �? React unmount trắng, không ErrorBoundary
- **Fix:**
...
- `resolveReadingTemplateKind()` �? fallback kind hợp l�? t�? catalog/default khi localStorage hoặc kind lỗi
...
## Đã xong (2026-07-08) �? Reading IELTS: b�? auto chạy theo câu
- `ReadingTest.tsx` �? b�? `useEffect` auto-scroll passage + câu hỏi khi đổi `activeQuestionId`
- `ReadingQuestionPanel.tsx` �? b�? auto chuyển câu tiếp theo sau khi điền paragraph/heading/word bank/feature
...
- **Fix ph�?:** `ExamTrackPage.tsx` �? chuyển redirect `ket` xuống sau tất c�? hooks (tránh Rules of Hooks khi đổi route)
...
- `R1TT_MERGE_TABLE_SAMPLE` �? bảng mẫu "fruits" gộp 5 hàng; vẫn h�? tr�? bảng 2 cột không merge (`CAM13_T1_NZ_WEBSITE_TABLE`)
...
- Nguyên nhân: `syncLocalToCloud` đẩy c�? b�? preset (`preset:group:slug`) lên Supabase �? cột `decks.id` là UUID
- Fix: `packages/db/src/cloud/presetDeck.ts` �? `isPresetDeck()`; sync b�? qua deck/card/SRS preset
- Ch�? deck `origin: user` (UUID) được đồng b�? cloud; preset lấy t�? seed local + Admin publish

## Đã xong (2026-07-08) �? Vocab preset decks b�? double
- Root cause: `seedPresetDecks` dùng UUID ngẫu nhiên + React StrictMode/sync Admin publish tạo bản trùng cùng group+tên
- Fix: `stablePresetDeckId()` + `put` idempotent + `dedupePresetDecks()` gộp th�?/SRS v�? 1 b�?
...
- **Nguyên nhân 2 (console):** `features[]` t�? AI dùng `label` thay `name` �? `splitReferenceText(feature.name)` crash; `ReadingHighlightableText` nhận `text` undefined.
- **Fix:** `readingExamSanitize.ts` �? group type, `features`/`headings`/`wordBank` (map `label`→`name`); load Dexie + published; phòng th�? `splitReferenceText`, `segmentsFromAnnotations`, `ReadingHighlightableText`.
...
- Wizard AI: `mergeTemplateNoteTables` gắn `noteTable` t�? template khi AI thiếu bảng
...
- Admin �? Publish nội dung; batch Cam khi đ�? 3 passages

### B �? K�? thuật còn m�?
- r1tt merge ô E2E browser; SAMPLE passage r1tt �? Coconut nếu AI nhiễu
- Listening Ô CH�? mobile iOS
```

## Đã xong (2026-07-09) �? S�? ghi chú t�? vựng (ch�? đ�? học)
...
- Study SRS: nút **Lưu s�? ghi chú** (`SaveToNotebookButton`) cạnh Hỏi AI / Lật th�?
- Tab **S�? ghi chú** trong study mode + nút trên trang B�? t�? vựng (m�? không cần chọn deck)
- Dictionary modal: **Lưu s�? ghi chú** (song song Thêm vào b�? th�?)
- UI: tìm kiếm, sửa memo, xóa, phát âm �? CSS theme-aware
- `pnpm --filter web exec tsc --noEmit` �? pass

## Next session start prompt (cập nhật 2026-07-10 �? offline dict Part1)
1. T�? điển FAB �? tra `abandon`, `environment`, `look forward to` (offline, không cần Pro)
2. (Tu�? chọn) Part2.json �? `pnpm build:dict:part1` m�? rộng; seed deck vocab t�? JSON
3. Test sync LWW + exam progress nếu chưa verify

## Đã xong (2026-07-10) �? Offline dictionary Part 1 (300 t�?)
- Sửa/hoàn thiện `Tainguyen/TuDien/Part1.json` (file cũ truncated ~105 t�?) �? **300 cards** A2–C2
...
- DictionaryModal: hiển th�? s�? mục offline thật
- `tsc` pass
...
2. Test offline→online: sửa deck/SRS offline �? online �? sync; m�? máy 2 pull LWW
3. Reading/Listening: làm d�? / nộp bài �? Đồng b�? �? thiết b�? khác thấy draft
4. Electron: export Vocabulary v2 / legacy backup �? Settings �? Nhập backup �? Đồng b�? đám mây
5. (Tu�? chọn) notebook cloud sync; verify E2E Reading template
...
- Preset deck vẫn b�? qua sync cloud
...
- Settings: mô t�? h�? tr�? Electron + sync hai chiều

### Verify
- `pnpm exec tsc --noEmit` (apps/web) �? pass
- **Cần `pnpm db:push`** trước khi exam progress sync hoạt động đầy đ�?
...
- **Root cause:** gộp deck preset (`dedupePresetDecks` / admin publish) chuyển th�? theo id khác nhau �? cùng phrase trong 1 deck; import CSV luôn `add` (không unique)
- **Fix:**
  - `cardRepo.dedupeByPhrase` / `dedupeAllDecks` �? gộp theo phrase (case-insensitive), gi�? SRS tốt nhất, merge field thiếu, chuyển reviewLog
...
## Đã xong (2026-07-10) �? Double b�? th�? "Công ngh�?" (và preset khác)
- **Root cause:** `dedupePresetDecks` ch�? gộp khi `name` khớp exact + `origin !== 'user'` �? bản UUID / tên lệch unicode / origin user vẫn hiện cạnh `preset:ielts:cong-nghe`
- **Fix:** bucket theo **stable slug** (`preset:group:slug`), gộp mọi deck trong group preset khớp seed (k�? c�? origin user); chuẩn hoá tên seed; phrase key NFD + collapse space
- M�? lại `/app/vocab` �? seed chạy dedupe �? ch�? còn 1 "Công ngh�?"

## Đã xong (2026-07-10) �? Listening ZIP: Answer Key + Audioscript �? transcript khi xem lại
- ZIP có th�? chứa `answer-key.pdf` / `audioscript.txt` / `tapescript.txt` (trước đây b�? skip)
- `listeningAudioscriptParse.ts` �? tách Audioscript, map s�? câu �? `ttsText`
...
- **Lưu ý:** PDF scan (không text layer) �? cần `audioscript.txt` hoặc PDF có ch�?; preview import hiện warning
...
## Đã xong (2026-07-10) �? Library: 3× Test 1 �? ch�? gi�? 7-part
...
- **Nguyên nhân h�? thống:** publish đẩy card/deck UUID random �? re-publish / user pull `bulkPut` id mới �? double
- **Fix phòng ngừa (c�? 2 phía):**
  ...
  - `mergeVocab` (user pull) normalize lại payload (k�? c�? bản publish cũ UUID) + `ensureSrs` + dedupe legacy
  - `stablePresetCardId` / `phraseKeyForCard` trong `vocabSeedDecks.ts`
- **Sau này:** Admin Publish vocab idempotent �? cùng t�? = cùng id, không nhân đôi
- Verify: `tsc --noEmit` pass
- **Next admin:** Publish lại 1 lần vocab đ�? cloud payload dùng id ổn định (khuyến ngh�?)
...
- [x] Link checklist t�? master + 10 Universal (KET…CPE L/R)
...
### Listening transcript t�? `audioscript.txt`
- [x] **UI xem:** Nộp bài �? kết qu�? `Transcript:` · hoặc **Xem cùng đ�? bài** �? khối **Transcript · Câu N** / bar review
- [x] Bug: ZIP `ket-listening-test2.zip` **thiếu** `audioscript.txt` (ch�? nested folder) �? rebuild ZIP flat + file script
...
- [x] Admin xóa đ�?: local Dexie + unpublish cloud published tables
...
- [x] `sync.ts`: nhận ghost deck (cùng group+tên preset) �? **không pull**; **soft-delete** ghost deck/card cloud; ch�? push UUID user; push **chunked** + partial fail không “chết�? sync
...
- [x] UI `/app/vocab`: nút **「Dọn th�? trùng�?**
- [x] Version web **0.2.3** · **deploy prod** https://ryanenglishv2.vercel.app
- [x] User: hard refresh (Ctrl+Shift+R) �? sync �? **Dọn th�? trùng**

### Lỗi còn tồn tại / theo dõi
- [ ] User verify sau deploy: s�? th�? từng b�? preset (~100) hết x2/x3; sync sidebar không còn đ�? RLS
- [ ] Admin **Publish lại vocab 1 lần** (payload cloud id ổn định `preset:` / `pcard:`) nếu cloud còn UUID cũ
- [ ] Nếu unpublish/xóa đ�? cloud fail: kiểm tra RLS DELETE trên `reading_exam_published` / `listening_exam_published`
- [ ] Re-import KET Listening Test 2 ZIP mới (có audioscript) nếu đ�? cũ local chưa có `ttsText`
...
- **Bug:** `listening-import-*` publish ch�? JSON; `stripLocalMediaKeys` xóa `audioKey` �? Firefox/user khác **không có MP3**
...
- User Admin: **Publish lại** đ�? import trên máy có blob local; Firefox hard refresh
- URL ví d�?: `/app/exam/listening/listening-import-…` load t�? `listening_exam_published`

## Đã xong (2026-07-11) �? Fix theo screenshot Error/error1+error2

- **error2 Part 2:** passage ch�? còn title + 3 ảnh (không label/text) �? UI `sign-box` khung lớn, mất Angus/Frank/Zac
  - `repairKetPart2Passage` ghép lại 3 profile + portrait theo th�? t�?
...
- **Root cause:** Publish t�? local import ch�? có `imageKey` blob �? `stripLocalMediaKeys` xóa key �? user không có `imageUrl`. Published ghi đè catalog (có `/catalog/...`). Part 2 portrait + nhánh render ch�? ảnh �? nuốt text khi thiếu label/text.
- **Fix:** `fillReadingExamMedia.ts` �? vá `imageUrl`/`text`/`label` t�? catalog khi resolve + trước publish
- Render Part 2: luôn hiện text kèm portrait
- Test: `scripts/test-fill-reading-media.mts` PASS
- User hard refresh là thấy (không bắt buộc re-publish); Admin **Publish lại** đ�? JSON cloud sạch

## Đã xong (2026-07-11) �? User không thấy P6/P7 + ảnh KET A2

- **Root cause P6/P7:** catalog builtin `reading-ket-a2-test1` ch�? **5 parts**; Admin import local 7-part �? User ch�? thấy catalog
...
- **Ảnh Part 2:** user ch�? thấy nếu upload gắn **cùng examId** user m�? (vd. `catalog-reading-ket-a2-test1`); sau upload cập nhật published JSON nếu đã publish
- Admin: import ảnh portrait **trên đ�? catalog** (hoặc Publish lại sau upload), không ch�? trên `reading-import-*`

## Đã xong (2026-07-11) �? KET A2 Part 2 portrait image (Admin import)

- 3 profile (label Angus/Frank/Zac�?): trước mỗi đoạn ô **2.5cm × 3.5cm**
- **Admin** thấy ô import / Đổi / Xóa; **user** ch�? thấy ảnh khi đã có
...
## Đã xong (2026-07-11) �? Fix KET A2 Part 2 b�? ép YNNG (false positive)
...
- **Root cause:** `coerceTriStateGroupType` / `optionsLookTriStateOnly` trong `readingExamSanitize.ts`
  ...
  - Option id A/B/C �? b�? coi tri-state; label ngắn (`Angus` < 12 ký t�?) không vượt `hasRealMcOpts`
...
- User: hard refresh dev �? **import lại** ZIP (hoặc m�? lại đ�?) �? không cần sửa JSON
...
- [x] `DictionaryModal` empty-state hiển th�? PV / Idiom / Colloc counts
- [x] **IPA US/UK** cho multi-word: `scripts/enrich-dict-multi-ipa.mjs` (CMUdict ARPABET→IPA + weak forms)
  - Phrasal **2000/2000**, Idioms **3000/3000**, Colloc **~9985/10000** (còn ~15 Latin/k�? thuật hiếm)
  - Rebuild multi t�? gọi enrich IPA �? cuối `build-dict-multi.mjs`
- [x] `pnpm --filter web exec tsc --noEmit` �? pass
...
### Ghi chú chất lượng
- Một phần idiom/collocation là pattern-generated (nghĩa VI generic) �? đ�? volume tra offline; có th�? tinh chỉnh curated sau.
- Bundle JSON lớn (~3 MB multi) �? import Vite JSON OK; typecheck pass.

## 2026-07-15 �? Fix: popup �?31k t�? cần ôn lại�? khi chưa rating

**Nguyên nhân:** Seed gán mọi th�? `dueAt = now`, `state = 'new'`. Code đếm `dueAt �? now` �? ~30k th�? “cần ôn lại�?.

**Fix:** `isSrsReviewDue` / `isSrsNew` trong `packages/core` �? “ôn lại�? ch�? th�? đã learning/review + đến hạn. Popup, hub, deck badge, daily goal, notification dùng predicate này.
...
- [x] `tts.ts` �? `speak()` t�? dùng giọng đã chọn theo lang `a`/`b`
- [x] Settings �? Tài khoản �? **Giọng đọc Kokoro local** �? dropdown US/UK + **Th�? nghe**
...
- speak() inject preferred voice; cần Bat-Kokoro.bat đ�? nghe Kokoro.

NEXT (optional):
1) Smoke Settings �? Th�? nghe (cần TTS gateway :8787)
2) Deploy web nếu ship dict multi + voice picker
```

## 2026-07-11 �? HDSD Universal: đ�? PDF **hoặc** TXT

- Master L/R + checklist + 10 Universal: § **Nguồn đ�? PDF hoặc TXT**
...
- KET Listening Universal: ví d�? Q8 `8/eight` + bảng lỗi thường gặp

## 2026-07-11 �? Listening: local media > catalog (triệt đ�?)

- **Bug:** Import KET Test 3 ZIP �? merge catalog gắn `/catalog/listening/ket-a2-test1` �? phát nhầm audio Test 1
- **Policy:** `listeningLocalMediaPolicy.ts` �? blob local thắng; twin ch�? cùng s�? Test; không default Test 1 cho Test N
- **Load path:** `resolveListeningExam` �? merge + `preferLocalListeningMedia` (sửa đ�? Dexie cũ, không bắt re-import)
...
## 2026-07-11 �? Tainguyen ra ngoài repo (deploy nh�? hơn)
...
- **Gi�?:** `apps/web/public/catalog` (~830 MB audio) �? vẫn là bottleneck upload nếu chưa CDN

## 2026-07-11 �? Check-in (điểm danh) sync theo tài khoản

- **Vấn đ�?:** Chuỗi điểm danh ch�? trong IndexedDB (`reviewLog` mode=checkin); publish không xóa; đổi máy / clear site data / browser khác = mất streak. Sync cloud trước đó **không** gồm reviewLog.
- **Fix:**
...
## 2026-07-12 - Publish prune cho toàn b�? nội dung Admin

- Lessons, Translation, Sentence Structures và Writing Prompts gi�? prune các ID từng nhận t�? Admin Publish nhưng đã vắng trong payload mới.
- Batch Publish Reading/Listening gi�? dọn các exam cloud không còn trong danh sách publishable local.
- D�? liệu cá nhân không nằm trong danh sách Admin Publish nên không b�? xóa.

## 2026-07-12 �? Fix MindMap xoá rồi sống lại (sync legacy bypass tombstone)

- **Root cause:** `apps/web/src/features/auth/useSync.ts` chạy song song với `useSyncManager` và khi `db.decks.count() === 0` gọi `syncCloudToLocal` �? hàm này pull thẳng `SELECT * FROM mindmaps` rồi `bulkPut`, **không đọc `mindmapTombstones`**. Mọi mindmap đã xoá local nhưng cloud chưa kịp xoá đều được kéo v�? "sống lại".
- **Fix:**
  ...
  - B�? 2 tên đó khỏi export �? `packages/db/src/index.ts`.
- **Verify:** `pnpm --filter web exec tsc --noEmit` PASS.
- **Ghi chú:** gi�? ch�? còn 1 đường sync duy nhất qua `syncBidirectional`. Nếu sau này cần "pull-only cho máy mới" thì phải viết lại có filter tombstone (mindmaps, và các bảng khác v�? sau).

## 2026-07-12 �? Fix Deck/Card xoá rồi sống lại (thiếu tombstone)

- **Root cause:** `deckRepo.delete()` và `cardRepo.delete()` ch�? xoá thuần local (`db.decks.delete` / `db.cards.delete`), không viết tombstone. `syncBidirectional` sau đó thấy row còn trên cloud mà không có local �? coi như "remote mới" �? `bulkPut` kéo v�?, sống lại toàn b�? deck + cards + SRS (hoặc card đơn l�?).
- **Fix:**
  ...
  - `deckRepo.delete()` �? transaction: put tombstone �? xoá srs/reviewLog/cards/deck local (gi�? nguyên preset guard).
  - `cardRepo.delete()` �? transaction: put tombstone �? xoá srs + card local.
  - `packages/db/src/cloud/sync.ts` (`syncBidirectional`):
    - Load `deckTombstones` + `cardTombstones` cùng lúc, lọc UUID hợp l�?.
    - `cloudDecksLive` / `cloudCardsLive` filter thêm tombstone set (không pull ngược ngay c�? khi push xoá cloud lỗi).
    - Push hard-delete cloud (`.from('decks').delete().eq(user_id).in(id, chunk)` + tương t�? cho cards), chunk 80. Cloud có FK `on delete cascade` nên xoá deck �? cards + srs t�? dọn.
    - Xoá tombstone local sau khi cloud xác nhận thành công.
- **Verify:** `pnpm --filter web exec tsc --noEmit` PASS.
- **Ghi chú:**
  - Deck/card đã b�? xoá trước bản vá này (khi chưa có tombstone) vẫn có th�? sống lại 1 lần t�? cloud vào sync k�? tiếp �? không cứu lại được vì không có dấu vết. T�? gi�? tr�? đi thì sạch.
  - Không cần migration Supabase �? cloud schema đã có `on delete cascade` sẵn.

## 2026-07-12 �? Prune đ�? Reading/Listening Admin đã xoá (theo pattern mindmap)

- **Vấn đ�?:** Admin xoá đ�? trong `reading_exam_published` / `listening_exam_published` thì `listAllReadingExams`/`listAllListeningExams` (đọc thẳng cloud) t�? b�? khỏi Library. Nhưng một s�? flow ph�? như `readingExamCloudImages.persistReadingPartImage` có gọi `examRepo.create(..., 'cloud-images')` �? cache lại bản local với cùng id. Nếu Admin xoá cloud, `resolveReadingExam` (`local || published || builtin`) vẫn còn bản local �? đ�? "sống lại" khi m�? chi tiết.
- **Fix (kênh A: ch�? Admin publish, không đụng đ�? user t�? import):**
  - Mới: `apps/web/src/features/admin/syncAdminPublishedExams.ts`
    - `syncPublishedReading()` / `syncPublishedListening()` �? SELECT id t�? 2 bảng publish cloud, so với danh sách id đã lưu trong settings key `admin_published_reading_exam_ids` / `admin_published_listening_exam_ids`, prune bản local (`examRepo.delete` / `listeningExamRepo.delete`) + audio blob prefix (`reading-exam:${id}:`, `listening-exam:${id}:`) cho id đã biến mất khỏi cloud.
    - Ghi lại danh sách id hiện tại vào settings cho lần sau. Đ�? user t�? import (không nằm trong id publish) không b�? đụng.
  - Nối vào `useSyncManager.runSync` sau bước check-in, non-fatal.
- **Verify:** `pnpm --filter web exec tsc --noEmit` PASS.
- **Ghi chú:**
  - Không giải quyết bài toán "48 đ�? IELTS user t�? import b�? mất" �? kênh B (sync user-imported exams lên cloud) chưa làm.
  - Lần sync đầu tiên sau bản vá: settings chưa có key nên `previousIds` rỗng �? không prune gì hết, ch�? ghi baseline. T�? lần Admin publish/xoá tiếp theo mới bắt đầu prune đúng.
-
## 2026-07-12 �? IELTS Reading Cambridge catalog import

- Seed 47 đ�? Reading Cambridge IELTS Cam 9�?20, Test 1�?4 vào builtin catalog.
- Loại `reading-cam-11-2.json` vì payload có 41 câu; ghi chú tại `docs/known-issues.md`.
- Thêm `scripts/payload-to-catalog.mjs` và m�? rộng `scripts/build-catalog.mjs` đọc `out-reading/`.
- Fix layout câu hỏi: gi�? prompt thật, table completion, title, rows và gap t�? `reading_filtered.json`.
- Verify: catalog test 47 đ�?/3 passages/40 câu PASS; `pnpm -C apps/web build` PASS.

## 2026-07-14 �? Fix avatar User trong Admin

- **Root cause:** Admin đã render `profiles.avatar_url`, nhưng profile sync ch�? đọc `user_metadata.avatar_url`; một s�? Google identity ch�? có `picture`, và profile cũ chưa được backfill.
- `syncAuthProfile.ts`: đồng b�? email/tên/avatar t�? Auth vào `profiles` khi bootstrap/đăng nhập, h�? tr�? `avatar_url` + `picture`, không ghi `null` đè d�? liệu profile hiện có.
- `018_profile_avatar_sync.sql`: cập nhật trigger Auth và backfill avatar/tên cho user cũ t�? `auth.users`.
- `AdminPage.tsx`: ảnh dùng `referrerPolicy="no-referrer"`; URL hỏng t�? fallback sang avatar ch�? cái.
- Verify: `vitest run src/features/auth/syncAuthProfile.test.ts` �? 2 tests PASS; `pnpm --filter web exec tsc --noEmit` PASS.
- Deploy cần chạy `pnpm db:push` đ�? áp migration 017�?018 trước hoặc cùng lúc deploy web.

## 2026-07-14 �? KET A2 Listening practice 44 đ�? (pilot + multi-part audio)

### Mục tiêu
Import thêm ~44 đ�? KET Listening (crawl CSV + media) vào app, title theo Cambridge Book/Test (tiếp sau 10 đ�? sẵn).
...
### Đã làm
- [x] Survey cơ ch�? import KET (ZIP `exam.json` + media; catalog 1 đ�? builtin)
...
- App UI KET đã h�? tr�? 5 file part; published exams dùng `audioUrl` public per part
...
- Mỗi b�? id `tr-grammar-{genre}`, 25 câu VI→EN + hint
- `seedTranslationPacks` v2 �? auto upgrade (gi�? SRS theo sentence id); xóa pack import trùng genre
- Verify: vitest grammarBasic25 PASS; tsc PASS
- User: hard refresh �? `/app/writing/translate/grammar_basic` �? từng ch�? đ�? hiện **1 b�? · 25 câu**
...
- **Nguyên nhân:** Google OAuth dùng `picture` (không ch�? `avatar_url`); `<img>` thiếu `referrerPolicy="no-referrer"` �? Google chặn �? vòng tròn trắng; fallback ch�? dùng `--bg-primary` d�? sai contrast.
- **Fix:**
  - `userAvatar.ts` �? resolve `avatar_url` | `picture` | identity_data
  - `UserAvatar.tsx` �? component chung: `referrerPolicy`, `object-cover`, onError �? ch�? cái `--color-on-primary`
...
- Media đ�? + convert ZIP **44/44**
- User: test-01 đã import; bắt đầu t�? **Cam 3 Test 4** �? publish **02�?44** only
...
- Xác minh endpoint dev tr�? đúng `200`, `application/pdf`, `Content-Length: 1,018,904` và magic bytes `%PDF-1.4`; file vật lý không rỗng.
- Root cause nằm �? pipeline browser `fetch �? arrayBuffer �? Uint8Array �? getDocumentProxy`: PDF.js chuyển/detach backing buffer khi m�? document, tạo đường lỗi 0-byte trong lifecycle reader.
- B�? hoàn toàn fetch/arrayBuffer trung gian trong `BookReaderPage`; dùng PDF.js `getDocument({ url })` đ�? thư viện tải trực tiếp URL PDF.
- B�? helper `renderPageAsImage`; render `PDFPageProxy` trực tiếp lên `<canvas>`, có cleanup/cancel render task khi đổi trang hoặc unmount.
...
Hard refresh `/app/reading-corner/sach/read/cv01`; xác nhận trang 1 render lên canvas, b�? đếm `1 / 278`, nút trang sau m�? trang 2 và không còn lỗi PDF file empty.
## 2026-07-16 �? Fix SRS flip card b�? xuyên hai mặt �? dark theme

- Ảnh user cho thấy sau khi lật, nội dung mặt trước b�? soi gương/xuyên qua mặt sau.
...
- Mặt th�? gi�? có lớp nền kín `var(--bg-card)` dưới gradient theme, b�? backdrop-filter khỏi chính hai mặt 3D.
- Thêm visibility handoff tại nửa animation (0.275s): mặt trước ẩn cứng khi flipped, mặt sau ch�? hiện khi flipped; gi�? `backface-visibility` cho hiệu ứng xoay.
- Thêm stacking isolation cho `.vs-flip-inner`; không thay đổi handler lật/rating hoặc grid.
- Regression test đ�? trước fix �? xanh; 4 vocab CSS/backdrop tests PASS; `tsc --noEmit` PASS; production build PASS.

### Next session start prompt

Hard refresh `/app/vocab`, m�? Lặp lại ngắt quãng �? dark theme và lật nhiều th�?; xác nhận ch�? thấy đúng một mặt, không còn ch�? soi gương/xuyên lớp trong hoặc sau animation.
## 2026-07-16 �? Fix tiếp SRS: mặt sau biến mất sau bản vá xuyên mặt

- Ảnh user cho thấy trạng thái flipped có rating buttons nhưng toàn b�? card trống.
- Root cause chính xác: `isolation: isolate` được thêm cùng `.vs-flip-inner { transform-style: preserve-3d }`; isolation là grouping property khiến descendants b�? flatten, nên backface của mặt sau b�? loại b�?.
- G�? `isolation: isolate`, gi�? `position: relative` và `preserve-3d`.
- Gi�? nguyên các phần đúng của bản vá trước: nền card kín, không backdrop-filter trên face, visibility handoff �? nửa vòng xoay.
- Regression test yêu cầu flip inner có `preserve-3d` và tuyệt đối không có `isolation`.
- Verify: test đ�? trước fix �? xanh; 4 vocab tests PASS; `tsc --noEmit` PASS; production build PASS.

### Next session start prompt

Hard refresh `/app/vocab`, lật SRS card �? dark theme; xác nhận mặt sau hiện nghĩa/IPA/example, mặt trước không xuyên qua, lật v�? mặt trước vẫn bình thường.
## 2026-07-16 �? Security HIGH: Phase 1 code hoàn tất, Phase 2 quota đang triển khai

- Đọc toàn b�? `Security/SECURITY_HARDENING_PLAN.txt` và triển khai theo th�? t�?.
- Phase 1 code:
  - Migration `020_harden_published_exams.sql`: anon không đọc được đ�? publish; authenticated qua `can_read_published_exam`, free ch�? 4 demo, paid/admin toàn b�?.
  - `admin_published_modules` + `admin_publish_meta` chuyển t�? public sang authenticated read.
  ...
  - `books/` paid-only; `catalog/ielts-wizard/` admin-only; content-sign h�? tr�? PDF/SVG.
  ...
  - Upload script h�? tr�? books/PDF/SVG và dry-run không cần credential.
...
- Phase 2 code:
  ...
  - content-sign TTL 60s, quota 400/user/24h, alert t�? 300 request.
...
- Phase 5.1 Turnstile Spin:
  ...
  - Secret ch�? được truyền vào Worker qua `wrangler secret put`, không ghi vào repo.
  - Form đăng nhập email được gate bằng `success === true`; Google OAuth gi�? nguyên.
...
- Full `tsc` đang b�? chặn bởi duplicate properties trong user-owned `reading-corner/catalog.ts`, không thuộc patch security.
- Phase 4 code:
  ...
  - Reusable `TermsConsentCheckbox` đã sẵn sàng; app chưa có signup handler
    thực t�? nên chưa wire checkbox vào luồng đăng ký.
- Phase 3/6:
...
- Production đã áp:
  - Lấy service-role tạm qua Supabase Management API bằng PAT hiện có; key ch�?
  ...
  - Duy nhất `catalog/listening/cae-c1-test1/listening.mp3` (82.94MB) vượt giới
    hạn 50MB của Supabase Free, b�? b�? qua và s�? không kh�? dụng sau lockdown.
  ...
  - Audit production PASS: anon đọc 0 rows �? reading/listening/admin publish
    tables; answer leak false; `exam-media.public=false`; vault tồn tại;
    content-sign thiếu JWT tr�? 401.
- G�? duplicate aliases `media` và `housing` �? mapping `reading-corner/catalog.ts`
  (gi�? semantics runtime “last key wins�?), m�? khóa typecheck/build.
- Full `tsc --noEmit` PASS; production build PASS và strip toàn b�? private media
  khỏi dist; `security:check` 9/9 PASS.
- CSP production đã b�? `'unsafe-eval'` và script `'unsafe-inline'`.
- Blocker còn lại:
  - Vercel CLI token không hợp l�?: code web mới chưa deploy; chưa tạo/publish
    Firewall rules hoặc kiểm tra production logs.
  - Signup handler chưa tồn tại nên `TermsConsentCheckbox` chưa wire thực t�?.
  - Chưa có kênh email/Zalo gửi alert; hiện có queue + admin-only DB alerts.
  - PITR, legal review/đăng ký bản quyền và x�? lý file CAE >50MB là thao tác
    dashboard/kinh doanh còn lại.

### Next session start prompt

User chạy `vercel login` đ�? làm mới token, sau đó deploy production ngay (backend
đã lockdown), smoke `/terms`, `/privacy`, login Turnstile, rồi tạo/publish
Vercel Firewall draft. X�? lý audio CAE 82.94MB bằng nâng Supabase Pro hoặc nén
dưới 50MB và upload lại đúng path.

### Tạm dừng cuối ngày 2026-07-16

- User yêu cầu dừng và tiếp tục vào ngày mai.
- Không chạy lại upload, backfill hoặc migrations 018�?022: tất c�? đã áp dụng
...
- Việc đầu tiên ngày mai:
  ...
  3. Deploy frontend production đ�? đồng b�? với backend đã lockdown.
  4. Smoke test `/terms`, `/privacy`, Turnstile login, sách và media đ�? thi.
...
- Chưa x�? lý:
  ...
  - Alert mới dừng �? DB queue; chưa gửi email/Zalo.
  - PITR, legal review và đăng ký bản quyền cần thao tác dashboard/nghiệp v�?.
...
- Smoke test production (https://ryanenglishv2.vercel.app) �? ALL PASS:
  - `/terms`, `/privacy` render đầy đ�? (điều khoản cấm crawl, copyright footer)
  ...
  - Login: Turnstile widget hiển th�?, nút Đăng nhập disabled tới khi có token;
    không lỗi CSP
...
- **Vercel Hobby note:** không có rate-limit rule (cần Pro). Mức Hobby dùng:
  Attack Challenge Mode (bật khi b�? crawl) + 1 custom WAF rule challenge/deny
  theo UA bot (python-requests, scrapy, curl, wget, HeadlessChrome,
  Go-http-client). Daily quota 400/user/24h �? content-sign là lớp rate-limit
  chính �? ch�? nâng Pro/Cloudflare khi có bằng chứng b�? crawl
  (theo dõi `content_access_log` + Vercel Analytics).
- Còn lại (tay user): tạo 1 custom WAF rule UA-bot trên Vercel Dashboard;
  login thật + admin publish 1 đ�? đ�? confirm end-to-end.

### Session 2026-07-17 �? Speaking AI thành trang riêng, chat + thu âm

- Speaking AI chuyển t�? modal panel sang trang riêng `/app/speaking-ai`
...
- H�? tr�? song song gõ chat và thu âm micro: textarea + nút mic; transcript
  nhận dạng ghép với text gõ tay; lượt gõ tay ước lượng `durationSec` theo s�?
  t�? (words/2, kẹp 1�?60s) nên vẫn dùng edge function `speaking-ai` (DeepSeek)
...
- Sidebar: b�? nút m�? modal �? đầu toolbar; thêm NavLink "Speaking AI"
  (icon AudioLines) ngay dưới "Luyện Shadowing".
- Test `speakingAiMvp.test.ts` cập nhật theo trang mới �? 3/3 PASS;
  `tsc --noEmit` PASS.
- Chưa smoke UI trong trình duyệt (route cần đăng nhập, user chọn b�? qua).

### Session 2026-07-17 �? Nền lưới Writing subpages

- Mọi route con `/app/writing/*` dùng nền lưới xanh nhạt, không ribbon; trang
  hub `/app/writing` gi�? ribbon.
- B�? sung transparency cho `writing-shell`, cùng Translation Practice:
  danh sách/chi tiết, empty state và màn hình luyện tập toàn trang (`tp-shell`)
  đều đ�? l�? backdrop lưới; card và input vẫn gi�? nền riêng.
- Verify: `appShellBackdrop.test.ts` 62/62 PASS; `tsc --noEmit` PASS.

### Session 2026-07-17 �? Translation: nhận diện câu đã dịch

- Câu đã dịch có nền và viền màu primary rất nh�?, đồng thời badge `Đã dịch` có
  icon check đ�? phân biệt nhanh với câu chưa dịch mà không làm mất đ�? d�? đọc.

### Session 2026-07-17 �? Sửa overlay phiên luyện dịch

- `tp-shell` là lớp ph�? toàn trang của phiên luyện dịch, nên không được làm
  trong suốt theo backdrop grid. Đã b�? override này; grid ch�? hiện �? màn hình
  danh sách, còn phiên luyện tập có nền đặc và không còn l�? UI phía sau.
- Regression check `appShellBackdrop.test.ts`: red trước sửa (1/62 fail vì
  `tp-shell` b�? transparent), green sau sửa.

### Session 2026-07-17 �? Lưới trong phiên luyện dịch

- `tp-shell` có nền lưới xanh nhạt độc lập (opaque), vì vậy phiên luyện tập có
  đúng visual grid mà không làm l�? UI danh sách phía sau.
- Tăng đ�? nhận diện cho câu/badge `Đã dịch`: tint 11%, viền primary 45% và
  badge 30% đ�? trạng thái xanh nhìn thấy ngay �? light/mid/dark theme.

### Session 2026-07-17 �? Badge Đã dịch xanh lá

- Thêm token `--color-success` cho c�? light/dark/mid theme; ch�? badge check
  `Đã dịch` dùng token này, đ�? trạng thái hoàn thành được nhận diện bằng xanh lá.

### Session 2026-07-17 �? Trạng thái Sentence Structure

- Danh sách `/app/sentence-structure` đọc completion history: cấu trúc đã hoàn
  thành hiện badge xanh lá `�? Đã học` và gợi ý `�? Học lại`, click hàng vẫn m�?
  đúng phiên luyện tập hiện có.

### Session 2026-07-17 �? Font Sentence Structure practice

- B�? style serif/editorial cũ ch�? còn �? route luyện cấu trúc: header `Điền trắc
  nghiệm` tr�? v�? font ứng dụng chuẩn và không còn ch�? trang trí `GRAMMAR ATLAS`
  chồng lên màn hình.

### Session 2026-07-17 �? Sunny dạo chơi trên Home

- Sau 30 giây �? `/app/home`, mascot Sunny chuyển sang animation “dạo chơi/trốn
  tìm�? nh�? quanh bubble header; vẫn `pointer-events: none` và tắt hoàn toàn khi
  người dùng bật `prefers-reduced-motion`.

### Session 2026-07-17 �? Sunny �? các trang chính

- `AppShell` dùng một mascot góc phải trên cho T�? vựng, Viết, Nghe, Shadowing,
...
- Sau 30 giây, Sunny �? góc phải trên chơi trốn tìm: nép ra ngoài mép phải rồi
  quay lại theo chu k�? nh�?.

### Session 2026-07-17 �? Card IELTS theo Góc đọc Báo

- Hai card Listening/Reading �? `/app/exam/track/ielts` dùng variant riêng theo
...
- Áp surface card lấy cảm hứng t�? Góc đọc Báo cho card cấp một toàn app: Home,
  T�? vựng, Writing/Translation, Shadowing, Exam, Sentence Structure và Prompt
  Bank. Các card có viền primary text, nền theme, offset shadow, hover/focus rõ;
  không áp vào question card, modal hoặc input đ�? bảo toàn luồng học.
- B�? sung `study-heatmap` ("60 ngày học gần nhất") vào card surface chung.
...
- Shared card surface làm nền Writing Library sáng, trong khi copy cũ là trắng.
  Đã override title/description/CTA theo token theme đ�? card luôn đọc được �?
  Light, Mid và Dark.

### Session 2026-07-17 �? Card archive IELTS/Cambridge

- Card sách, header sách và hàng đ�? trong mọi trang con Reading/Listening của
  `/app/exam/track/ielts/*` và `/app/exam/track/cambridge/*` dùng surface card
  Góc đọc Báo. Ch�? đổi archive/library UI, không ảnh hưởng màn hình làm bài.

### Session 2026-07-17 �? Light contrast Cambridge exam cards

- Shared card surface đã thay gradient của card k�? năng Cambridge nhưng copy cũ
  vẫn màu trắng. Title, mô t�?, icon chip và badge gi�? dùng token theme, nên đọc
  rõ �? Light (và vẫn đúng �? Mid/Dark).

### Session 2026-07-17 �? Typography chung theo Góc đọc Báo

- Thống nhất font UI bằng stack native của `/app/reading-corner/bao` qua
  `--font-app`; controls k�? thừa đúng font. Các màn Writing, Vocab và paper
  IELTS/KET không còn gi�? Inter/Quicksand/Segoe riêng. Heading editorial dùng
  `--font-editorial` (Georgia), tương ứng typography headline của Báo; font mono
  cho IPA/code và lựa chọn font đọc đ�? của người dùng được gi�? nguyên.

### Session 2026-07-17 �? Typography Tổng quan

- `/app/home` còn override Cambria/Georgia �? tiêu đ�?, ch�? s�? và nhãn section
  nên nhìn chưa đồng b�?. Ba phần này nay dùng `--font-app` như Góc đọc Báo.

### Session 2026-07-17 �? Audit typography toàn app

- Rà toàn b�? CSS app và thay các font UI/decorative còn sót �? Reading Corner,
  Exam hub, Listening library, Sentence Structure, Login và KET Listening v�?
  `--font-app`. Ngoại l�? duy nhất gi�? lại là text IPA/code và font reading do
  học viên chọn trong trình làm đ�?; đây là d�? liệu/chức năng học, không phải
  typography giao diện.
- Landing và text trong mascot SVG cũng đã b�? Inter/Instrument Serif đ�? cùng
  h�? font; export MindMap vốn đã dùng system font tương thích.
...
- `/app/vocab` dùng grid-paper panel neo-brutalist, filter/tabs viền đậm và
  card giáo trình 2 cột dạng bìa sách + metadata/progress. Gi�? AppShell, d�?
  liệu deck và luồng học Ryan; không dùng hay import d�? liệu t�? website crawl.

### Session 2026-07-17 �? Vocab lesson theo Nhai TOPIK (UI-only reference)

- Khi m�? một deck, `/app/vocab` chuyển sang lesson canvas grid-paper với hard
  border/shadow; danh sách t�? tr�? thành 2-column word cards trên desktop và
  một cột trên mobile. SRS/study modes hiện có vẫn là luồng flashcard thật của
  Ryan, không đưa d�? liệu lesson t�? crawl vào app.

### Session 2026-07-17 �? Đồng b�? 9 mode học vocab theo lesson grid-paper

- `Lặp lại ngắt quãng`, `Trắc nghiệm`, `Đoán nghĩa`, `Nghe & Gõ`, `Speaking`, `T�? yếu`, `Ôn tập`, `Thống kê`, `S�? ghi chú` dùng chung b�? mặt paper-grid: viền đậm, hard-shadow, nút/card khối và màu xanh chọn như ảnh UI crawl.
- Thanh 9 mode là mode card rõ ràng (3 cột desktop, 1 cột mobile); luồng học và d�? liệu không thay đổi.

### Session 2026-07-17 �? Phát âm trong danh sách t�?

- Mỗi th�? t�? �? lesson `/app/vocab` có nút loa, gọi TTS phrase hiện có và có nhãn tr�? năng theo t�? đang phát.

### Session 2026-07-17 �? Khung flashcard SRS paper-grid

- B�? sung hard-border/hard-shadow trực tiếp vào 3D flip scene và loại b�? gradient/bóng lồng �? hai mặt th�?; mặt trước/sau gi�? cùng h�? paper-card với các mode vocab khác mà vẫn gi�? cơ ch�? lật.
- Nút `Lật thẻ` cạnh `Hỏi AI` cũng dùng surface paper-card, viền đậm và hard-shadow thay vì gradient cũ.
- Mặt trước SRS �? light theme khóa foreground của t�?, nhãn, ví d�? và gợi ý lật v�? `--text-primary` đ�? tương phản rõ trên nền xanh paper-card.
- B�? sung tương phản cho các badge ch�? đ�?/đến hạn và nút audio �? mặt trước; chúng không còn dùng ch�? nhạt dành cho dark theme.

### Session 2026-07-17 �? Listening cards theo style Góc đọc Báo

- Card Cambridge pack, lesson cá nhân và card Part bên trong `/app/listening` nay dùng nền theme, border rõ, offset hard-shadow và hover nâng nh�? theo h�? card Báo; thao tác/list/grid/compact vẫn gi�? nguyên.
...
- `/app/writing` b�? toàn b�? các hình tròn/vuông minh họa trong bốn card hub; card chuyển sang b�? cục một cột, gi�? gradient, nội dung và CTA.

### Next session start prompt

Kiểm tra trực quan Light/Mid/Dark tại `/app/vocab`, `/app/listening`, `/app/listening/:lessonId` và `/app/writing`: bảo đảm card paper/Báo có tương phản ch�? tốt, không còn shape hình học �? Writing và các thao tác học/SRS/Listening vẫn hoạt động. Các thay đổi UI hiện tại đã pass `pnpm --filter web exec -- tsc --noEmit`; working tree có thay đổi UI của user/session, không reset hoặc checkout.
...
- `/app/admin` now lets an administrator search a user, give an optional reason, confirm **Khóa**, and later **M�? khóa**. Admin accounts cannot be suspended from this UI.
...
- Replaced the old day-only scheduler with the visible learning cadence: **Quên** returns in 1 minute, **Khó** in 10 minutes, consecutive **Nh�?** ratings schedule 1 day then 4 days, and **D�?** schedules a new card for 4 days. Later intervals still grow from ease.
...
- The SRS title/empty state identifies this as “Ôn th�? đến hạn�?. Verify: SRS tests 6/6 and `pnpm --filter web exec tsc --noEmit` PASS.

## 2026-07-18 �? Mây nh�? đi cùng Sunny

- Gắn mây nh�? ☁️ vào Sunny �? header Tổng quan và mascot góc phải của các trang chính.
- Light hiển th�? Sunny đi cùng mây nh�? ☁️; Mid và Dark ẩn hoàn toàn mặt trời, thay bằng mặt trăng 🌙 nhưng vẫn gi�? mây nh�? đi cùng.
- Mây/mặt trăng nằm cùng wrapper chuyển động nên luôn theo Sunny khi nhún, dạo chơi và trốn tìm; vẫn tôn trọng `prefers-reduced-motion` và thu nh�? trên mobile.
- Verify: `pnpm --filter web exec tsc --noEmit` PASS; `git diff --check` PASS.

## 2026-07-18 �? Bird companion SVG mascot set

- Tạo 5 SVG thuần, nh�? tại `apps/web/public/mascots/`: idle, happy, sad/encourage, studying/thinking và flying/loading; mọi b�? phận chính có ID ổn định đ�? animate t�? React/CSS.
- Bird dùng thân mint, cánh xanh, bụng kem, m�? coral và nét viền nâu-đen mảnh; flying có wing-flap loop nội b�? và tôn trọng `prefers-reduced-motion`.
...
- Bird SVG được gi�? làm asset/demo tham khảo nhưng đã g�? khỏi UI theo yêu cầu; app dùng Sunny + mây ☁️ �? Light, mặt trăng 🌙 + mây ☁️ �? Mid/Dark.

## 2026-07-18 �? Audit local-first cho tải 1000 user đồng thời

- Audit read-only xác nhận `syncBidirectional`, `exam_progress` và `checkin_days` đang full-pull, không cursor/pagination; `supabase/config.toml` giới hạn `max_rows = 1000`, tạo rủi ro cắt d�? liệu im lặng cho user lớn.
- Push chính đã batch/chunk; sync chạy khi login, online và mỗi 5 phút nhưng chưa có jitter/backoff. LWW vẫn dùng timestamp t�? clock client trong khi một s�? bảng có trigger server `updated_at`, dẫn tới semantics không nhất quán.
...
- RLS own-row dùng `auth.uid()` trực tiếp và thường thiếu `to authenticated`; các helper admin/entitlement được gọi theo row, cần init-plan wrapper `(select ...)`. `speaking_messages` EXISTS có PK/index h�? tr�? nhưng vẫn cần EXPLAIN trên production data.
- Runtime browser/Edge dùng Supabase HTTP Data API, không m�? Postgres connection trực tiếp. Chuỗi tooling hiện có là Supavisor session port 5432; nếu sau này có serverless native Postgres client thì dùng transaction pooler 6543.
- Chưa chạy EXPLAIN/load test production vì audit không được cấp phép tạo tải; cần migration incremental-sync + indexes trước, sau đó k6 bằng token riêng và SQL `EXPLAIN (ANALYZE, BUFFERS)` trên d�? liệu đại diện.
...
- `/app/shadowing` có form dán URL YouTube; h�? tr�? watch, youtu.be, Shorts, embed, live URL và video ID. Backend t�? chọn caption tiếng Anh th�? công trước, fallback auto-generated, chuẩn hóa timestamp rồi m�? ngay lesson với player/tua câu/tốc đ�?/ghi âm/chấm nói/dictation/quiz hiện có.
- Thêm Edge Function `youtube-captions`: auth + Pro/Lifetime/Admin + suspension check, URL/video validation, 15s timeout, giới hạn video 2 gi�?/2.000 cue, lỗi rõ cho private/live/no-caption/YouTube upstream. Không dùng API key/OAuth �? client.
- Custom lesson cache local tối đa 20 video. Video phải public, cho phép embed và có caption tiếng Anh; caption của video bất k�? không dùng được YouTube Data API chính thức vì captions list/download yêu cầu OAuth/quyền liên quan video.
...
## 2026-07-18 �? Fix YouTube caption fetch b�? chặn trên Supabase Edge

- Tái hiện với video `_CwYFdjj63s`: URL/parser đúng và desktop request ngoài Edge tr�? HTTP 200, nhưng production báo `VIDEO_UNAVAILABLE` ngay tại request trang watch; nguyên nhân là nhánh cũ không có fallback và che mọi upstream non-2xx thành 404 chung.
- `youtube-captions` nay th�? `www.youtube.com/watch` trước rồi fallback sang `m.youtube.com/watch` với mobile User-Agent. Mobile watch đã được xác minh tr�? `ytInitialPlayerResponse` và `captionTracks` cho chính video lỗi.
- Khi c�? hai nguồn thất bại, function tr�? `YOUTUBE_BLOCKED` 502 và log chuỗi nguồn/status (`desktop:403`, `mobile:429`, v.v.) thay vì báo sai rằng video không tồn tại.
...
- Edge Function `youtube-captions` đã deploy production lên project `ntcagvtkwxwsmlxlumfo`. Cần người dùng Pro dán lại đúng URL đ�? smoke authenticated end-to-end; không cần redeploy frontend.

### Follow-up �? Supabase và Vercel đều b�? bot checkpoint

- Authenticated production retry xác nhận c�? desktop/mobile YouTube watch đều b�? chặn t�? Supabase, tr�? `YOUTUBE_BLOCKED`.
- Thêm Vercel Function nội b�? `/api/youtube-captions-relay`, ch�? nhận video ID hợp l�?, bảo v�? bằng `YOUTUBE_CAPTIONS_RELAY_SECRET`, giới hạn caption 5 MB và không nhận URL tùy ý. Relay tests + fallback tests 4/4 PASS; TypeScript PASS.
- Secret relay yếu do PowerShell API không tương thích �? lần deploy đầu đã được xoay ngay bằng `RandomNumberGenerator.Create().GetBytes`; Vercel production deployment mới `dpl_BymHVAFqGL8iTe4NUMDSJ7nUZ9RJ` READY và Supabase secret/function đã redeploy.
- Blocker hiện tại: Vercel Security Checkpoint tr�? 429 trước khi request tới Function; `vercel curl` cũng không t�? bypass vì project chưa tạo Protection Bypass for Automation. Không tắt firewall.
- Code Edge đã chuẩn b�? header `x-vercel-protection-bypass` t�? secret `VERCEL_AUTOMATION_BYPASS_SECRET`. User cần vào Vercel Project �? Settings �? Deployment Protection �? Protection Bypass for Automation, tạo token; đặt cùng giá tr�? vào Supabase Edge Function secret `VERCEL_AUTOMATION_BYPASS_SECRET`, rồi retry/deploy-smoke.
- User đã tạo automation bypass và Supabase secret, sau đó rotate vì token cũ l�? trong ảnh. Usage screenshot xác nhận quota không vượt (409 MB/100 GB, 28K/1M Edge Requests, CPU 4s/1h); nhận định quota là blocker đã được rút lại.
- Redeploy production sau khi rotate token đ�? deployment nhận bypass mới: `dpl_53GcojXeKPY8LfXgwSBC2K3VnbrW` READY, alias chính đã cập nhật. `vercel curl` của CLI 54 vẫn b�? Security Checkpoint và không có Function log, nên cần retry authenticated t�? app đ�? xác nhận Edge dùng secret Supabase trực tiếp; CLI automation lookup không phải bằng chứng secret Supabase sai.
## 2026-07-18 �? Mặt trời TID cho Exam Hub

- Thay minh họa lớn trong hero `/app/exam` bằng đúng hai lớp SVG thân/khuôn mặt được trích t�? `https://theieltsdictionary.com/about-tid`; mascot dạo chơi toàn app không đổi.
- Gi�? interaction gốc: thân mặt trời quay 9 đ�? mỗi 0,5 giây, khuôn mặt đứng yên; responsive và tôn trọng reduced-motion.
- Hai mắt được tách khỏi SVG mặt, gi�? đúng tọa đ�?/hình dáng gốc và liếc trái–phải theo nhịp 4,8 giây; reduced-motion gi�? mắt đứng yên.
- Thu mặt trời desktop t�? 29rem xuống 27rem và dịch sang phải 2,5rem đ�? không overlap tiêu đ�? Luyện thi; mobile gi�? trong viewport.
...
- Chẩn đoán route `listening-import-ket-a2-practice-12`: màn kết qu�? gọi `.toUpperCase()` trực tiếp trên option ID của d�? liệu import, nên option thiếu ID/ID không phải chuỗi làm React crash và vùng Exam trắng.
- Chuẩn hóa ID v�? string; nếu thiếu thì fallback A/B/C theo index. Luồng format đáp án nhiều lựa chọn cũng không còn gọi `.toUpperCase()` trên giá tr�? không chuẩn.
- Regression test tái hiện lỗi trước fix và bao ph�? c�? ID thiếu lẫn ID dạng s�?.
- Hardening tiếp sau khi Practice 12 vẫn trắng: loại b�? `.toUpperCase()` trực tiếp còn sót �? AI source và chuẩn hóa `question.answer`/`userAnswer`; câu thiếu answer key gi�? tr�? sai an toàn thay vì gọi `.trim()` trên `undefined` làm crash toàn báo cáo.
- Root cause điểm 0/25 và review không có đáp án: localhost coi `catalog/exams/**/*.answers.json` là Vite public asset, trong khi vault của đ�? restored/published ch�? tồn tại �? private Supabase Storage. `mustUseSignedMedia` gi�? luôn ký answer vault trên c�? dev và production; regression security test bao ph�? Listening/Reading.
- Harden toàn b�? Luyện thi: Reading result dùng formatter option an toàn như Listening; normalize answer/option/headings chịu được d�? liệu import thiếu/sai kiểu. C�? Reading và Listening không còn chấm 0 gi�? khi vault lỗi—hiện màn báo tải đáp án thất bại và nút th�? lại thay vì render report/review thiếu answer key.
- Fix tiếp “Xem cùng đ�? bài�? hiện `Đáp án đúng: —`: màn báo cáo đã dùng exam được merge answer vault nhưng khi chuyển v�? paper, component cha vẫn gi�? DTO cũ đã strip đáp án. `promoteHydratedExamForReview` nay đồng b�? snapshot đã hydrate v�? DTO mà paper đang render trước khi bật review mode, áp dụng chung cho Listening và Reading. Regression test 1/1; toàn b�? test liên quan 8/8, TypeScript và diff check PASS.
...
- Query pull nay chọn `card_id` riêng cho `srs`, gi�? `id` cho decks/cards/writing_docs/mindmaps; regression test khóa đúng mapping và test scalability được cập nhật theo query động.
...
- Tách relay Vercel thành project riêng `youtube-captions-relay.vercel.app`, bảo v�? bằng shared secret và b�? sung Android InnerTube Player API sau desktop/mobile watch page. Unit/regression tests 5/5 và TypeScript PASS.
- Smoke production xác nhận relay riêng không còn b�? Vercel Security Checkpoint, nhưng egress Vercel vẫn b�? YouTube chặn: endpoint tr�? 502 dù Android InnerTube hoạt động t�? máy local.
- Tạo và deploy Cloudflare Worker giới hạn đúng POST + video ID 11 ký t�?, secret header, timeout 15 giây và caption tối đa 5 MB tại `services/youtube-captions-worker`; Wrangler 4.112.0 types + deploy dry-run PASS. Endpoint: `ryan-youtube-captions-relay.ryan-license-worker.workers.dev`.
- Relay secret đã xoay bằng CSPRNG, đặt vào Worker qua stdin và đồng b�? cùng URL vào Supabase; Edge Function `youtube-captions` đã redeploy.
- Smoke production vẫn b�? YouTube chặn t�? Cloudflare: `desktop:429`, `mobile:429`, `innertube:LOGIN_REQUIRED`. Kết luận đây là chặn IP datacenter, không phải Vercel checkpoint hay lỗi parser. Cần residential proxy hoặc transcript API có key đ�? hoàn tất; app hiện vẫn có th�? báo YouTube chặn.
- Verify: YouTube fallback/relay tests 5/5 PASS; web TypeScript PASS; `git diff --check` PASS.

### Session 2026-07-19 �? G�? nhập YouTube khỏi Shadowing

- G�? form “Luyện với video YouTube của bạn�?, toàn b�? state/import/cache client và fallback custom lesson; thư viện Shadowing dựng sẵn cùng trình phát của các bài có sẵn vẫn gi�? nguyên.
...
- Whisper local gi�? backward-compatible plain text và lưu thêm segment timing đã validate tại `exam-listening-whisper-segments:{examId}:{partNumber}`. Transcript panel render từng segment, highlight/auto-scroll segment đang phát, b�? highlight khi pause và quy đổi đúng thời gian tương đối khi Part dùng một MP3 chung.
- Thêm `useAudioSync`: practice mode t�? chọn/scroll câu theo Whisper segment hoặc t�? l�? thời lượng; exam mode, submitted và review mode không auto-advance. Mọi pointer interaction trong runner tạm khóa auto-sync 3 giây đ�? thao tác th�? công thắng.
- Tích hợp KET, PET, FCE/CAE/CPE và IELTS runner. Mapping thuần x�? lý c�? audio riêng từng Part và shared-audio `startPct`/`endPct`.
...
- Lỗi còn tồn tại: chưa smoke tương tác có audio + Whisper thật trong browser đăng nhập; cần kiểm tra highlight, seek, đổi Part và manual override trên d�? liệu thực.
...
- Root cause transcript m�? ra nhưng không highlight: auto-question có fallback theo t�? l�? duration nên vẫn chạy, còn transcript highlight bắt buộc có Whisper segments. Transcript plain/catalog hoặc Whisper tạo trước bản vá ch�? có text; đồng thời UI cũ ẩn nút Whisper khi `ttsText` đã đ�?, khiến không th�? tạo timestamps.
- Panel nay hiện `Tạo đồng b�? transcript theo audio` bất c�? khi nào Part có audio URL nhưng chưa có segments, k�? c�? transcript text đã tồn tại. Chạy một lần s�? ghi segment timing và bật highlight/auto-scroll.
- Regression test khóa trạng thái plain transcript + zero segments. Verify: scoped tests 5/5, TypeScript và `git diff --check` PASS.
## 2026-07-21 �? Fix login b�? cắt trên màn hình MacBook thấp

- Repro Chrome tại `1024x640`: login card cao 697.5px nhưng `.login-page` fixed-height 640px vẫn `align-items: center`, đẩy card lên `y=-28.75px`; container không tạo vùng scroll hữu dụng nên đầu/cuối form b�? cắt.
- Breakpoint `max-height: 750px` nay dùng `align-items: flex-start`. Card bắt đầu dưới padding �? `y=12px`, login page có vùng cuộn 82px tại 1024x640; 1280x720 gi�? card đầy đ�? và ch�? có 2px overflow.
- Verify trực tiếp Chrome DevTools tại 1024x640 và 1280x720; cuộn được tới footer. TypeScript và `git diff --check` PASS.
- Next session start prompt: nếu cần, smoke thêm tab Đăng ký tại viewport thấp vì consent copy có th�? làm card cao hơn login.
## 2026-07-21 �? Publish Whisper segment timestamps lên cloud

- `ListeningPart` có field `transcriptSegments`; d�? liệu nằm trực tiếp trong JSON `listening_exam_published.parts`, không cần migration.
- Pipeline publish hydrate c�? plain transcript và segment timing t�? localStorage trước khi strip/push. Segment cloud đã có được ưu tiên, không b�? timing local cũ ghi đè.
- Transcript panel và `useAudioSync` ưu tiên `part.transcriptSegments` t�? cloud, ch�? fallback localStorage cho đ�? local/đ�? cũ. Vì vậy Vercel không cần chạy Whisper khi user học.
- Đ�? đã publish trước đây cần Admin m�? từng Part, chạy `Tạo đồng b�? transcript theo audio`, rồi publish/re-publish đ�? đ�? timestamps lên cloud.
...
- Lỗi còn tồn tại: chưa re-publish d�? liệu production, nên các đ�? cloud hiện tại chưa t�? có segments cho tới khi chạy quy trình generation + publish.
- Next session start prompt: chọn một Listening exam, tạo timing cho mọi Part, re-publish, rồi smoke trên domain production/khác origin đ�? xác nhận không gọi `/api/stt` nhưng highlight/scroll vẫn chạy.

## 2026-07-21 �? Lệnh batch tạo segment timestamps cho 54 đ�? KET

- Thêm lệnh root `pnpm ket:segments -- --yes` đ�? Claude chạy một lần cho toàn b�? KET A2 Listening đã publish. Script tải audio private t�? `exam-media`, gi�? một tiến trình `faster-whisper`/model duy nhất, ghi `transcript` + `transcriptSegments` vào `listening_exam_published.parts`, b�? qua Part đã có timing và tiếp tục báo lỗi theo từng Part.
- Lệnh t�? đọc `.env.deploy`, lấy service role trực tiếp hoặc qua `SUPABASE_ACCESS_TOKEN`, đồng thời t�? tìm venv Whisper có sẵn tại `C:\Users\lindv\whisper\.venv\Scripts\python.exe`. Có các tùy chọn `--list-only`, `--force`, `--limit`, `--only`, `--model`, `--python`; mọi thao tác ghi bắt buộc có `--yes`.
- Read-only production check ban đầu xác nhận đúng 54 KET exams, mỗi đ�? 5 Parts, tổng 270 Parts và 0/270 Parts có timestamps.
- User đã chạy batch ghi thật thành công: `Done: 54/54 exams updated, 270 Parts timed, 0 Parts failed.`
- Caveat: 10 đ�? Cam/Book cũ có th�? dùng một file audio chung cho c�? 5 Parts. Batch tái s�? dụng transcript/timestamps tuyệt đối của file chung; highlight theo audio hoạt động, nhưng panel có th�? hiện transcript toàn bài và mapping câu hỏi ch�? gần đúng nếu Part không có mốc phần trăm thời gian.
...
- Lỗi còn tồn tại: cần smoke production ít nhất một đ�? per-Part audio cùng một đ�? shared-audio; xác nhận transcript highlight/auto-scroll, seek, pause và khóa thao tác th�? công 3 giây.
- Next session start prompt: smoke production đ�? “KET A2_Test 2 - Luyện Nghe Tiếng Anh A2 Có đáp án và dịch nghĩa�?, sau đó kiểm tra thêm một đ�? KET practice dùng audio riêng từng Part.

## 2026-07-21 �? Production web release v0.2.10

- Bump `apps/web` t�? 0.2.9 lên 0.2.10 và deploy frontend auto-sync Listening, cloud transcript segments cùng fix login viewport thấp lên Vercel production.
...
- Next session start prompt: smoke authenticated production đ�? KET A2_Test 2 và một KET practice; xác nhận transcript highlight/auto-scroll, seek, pause và manual override 3 giây.

## 2026-07-21 �? Fix auto-sync cho Cambridge shared audio

- Root cause Cam 1 Test 3 không t�? chuyển câu/Part: `useAudioSync` ch�? map audio vào câu của `currentPart`, trong khi 10 đ�? KET Book 1 Test 1 đến Book 3 Test 2 dùng một MP3 chung, không có `audioStartPct/audioEndPct`.
- Hook chung nay t�? nhận dạng theo cấu trúc d�? liệu: mọi Part cùng nguồn audio và không có range thì sync trên toàn b�? câu, đồng thời đổi `partIndex`; audio riêng từng Part hoặc shared audio có range tiếp tục sync trong Part hiện tại.
- Áp dụng chung cho runner KET, PET, FCE/CAE/CPE và IELTS, nên các lần import Cambridge A2-C2 sau t�? h�? tr�? c�? kiểu một audio lẫn nhiều audio, không hardcode ID/Cam.
...
- Regression tests bao ph�? shared-unbounded, per-Part và shared-ranged. Verify: Listening tests 11/11 PASS; TypeScript PASS; `git diff --check` PASS.
...
## Preserved main-line session summary before FCE B2 consolidation
