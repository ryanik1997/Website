...
- **Session:** 2026-07-03 — IELTS batch Cam9–20 (48 đề) + catalog v14 (map/diagram crop auto)
- **Production:** https://ryanenglishv2.vercel.app — **chưa deploy** catalog v14 (48 IELTS + MP3); local OK sau `pnpm build:catalog`
...
- [x] `GLOBAL_CATALOG_VERSION` **12 → 13 → 14**

### Lỗi còn tồn tại (IELTS import)

- Cam9 Test 1: nguồn Python `build-ielts-listening-tests.py` (legacy single-file; các đề khác dùng bundle)
- Cam9 Test 4 P4: layout `table` (đề gốc dạng bảng — không lecture notes)
- Production chưa deploy catalog v14
...
Session tiếp — deploy catalog v14.

Đã xong (2026-07-03):
- 48 đề IELTS Cam9–20: P1 + P4 note lines (bullet •/–/+) + map/diagram crop (chỉ hình, không cả trang)
- `scripts/extract-all-ielts-plan-images.py` — auto 15 plan images
- GLOBAL_CATALOG_VERSION=14

User cần:
1. pnpm dev → hard refresh → Cam19 T1 P2 map (case7.jpg), Cam20 T2 P1 notes (case5/6)
2. pnpm deploy:prod khi OK local
```
